...
function ApplyTemplateChanges()
{
	...
	var desc_len = 0;
	var textFields = doAction("REQ_GET_FORMVALUE", "TextFields", "TextFields");
	if (textFields)
	{
		textFields = textFields.split (gSEPERATOR);
		for (var n = 0; n < textFields.length; n++)
		{
			var temp = doAction("REQ_GET_FORMVALUE", textFields[n], textFields[n]);
			if (temp)
			{
				bizInfo[textFields[n]] = temp;
				desc_len++;
			}
		}
	}
	...
	var HeaderText = "Contact Us"; // not changable by user???
	...
	if (bizInfo && desc_len > 0)
	{
		width = textWidth;
		height = 0;
		for (var n = 0; n < textFields.length; n++)
		{
			if (bizInfo[textFields[n]])
			{
				height += getTextHeight(bizInfo[textFields[n]], width);
			}
		}
		posObj.setStartX(width);
		ApplyBizElementChange(pageElements, pageName, 'BusinessText', bizInfo, posObj, width, height);
		posObj.startY += height + 5;
	}
