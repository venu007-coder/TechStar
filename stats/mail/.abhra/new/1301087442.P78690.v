Return-Path: sanjeev@xperttech.com
Received: from mx60.stngva01.us.mxservers.net (204.202.242.130)
	by mail19g.g19.rapidsite.net (RS ver 1.0.95vs) with SMTP id 1-0915767441
	for <abhra@altechstar.com>; Fri, 25 Mar 2011 17:10:41 -0400 (EDT)
Received: from unknown [76.74.252.24] (EHLO rs78.luxhv.com)
	by va1-mx60.stngva01.us.mxservers.net (mxl_mta-3.1.0-05)
	with ESMTP id 0d40d8d4.2530200480.22113.00-005.va1-mx60.stngva01.us.mxservers.net (envelope-from <sanjeev@xperttech.com>);
	Fri, 25 Mar 2011 17:10:40 -0400 (EDT)
Received: from ip-10-196-81-219.ec2.internal (ec2-184-72-246-77.compute-1.amazonaws.com [184.72.246.77])
	(authenticated bits=0)
	by rs78.luxhv.com (8.13.8/8.13.8) with ESMTP id p2PLAaVc011827;
	Fri, 25 Mar 2011 16:10:37 -0500
Date: Fri, 25 Mar 2011 16:10:34 -0500 (GMT-05:00)
From: Sanjeev Coduru <sanjeev@xperttech.com>
To: sanjeev@xperttech.com
Message-ID: <1301087434391.06162755243022966.sanjeev@xperttech.com>
Subject: Need SQL DBA/Developer@TX
MIME-Version: 1.0
Content-Type: multipart/mixed; 
	boundary="=_reb-r4D74772A-t4D8D04D1"
Importance: Normal
X-Lux-Comment: Message p2PLAaVc011827 sent by user #68993; report Spam, UCE, or Abuse to abuse@luxsci.us
X-Lux-Comment: LuxSci High Volume Email: http://luxhv.com
X-Processed-By: Rebuild v2.0-0
X-Spam: [F=0.2000000000; B=0.500(0); S=0.200(2010122901); MH=0.500(2011032528)]
X-MAIL-FROM: <sanjeev@xperttech.com>
X-SOURCE-IP: [76.74.252.24]
X-SF-Loop: 1

This is a multi-part MIME message.

--=_reb-r4D74772A-t4D8D04D1
Content-Type: text/html; charset=us-ascii
Content-Transfer-Encoding: 7bit

<div style="padding:3px" width="100%" align="right" id="removeTopLink"><a href="http://portal.zoniacswift.com//servlet/ZoniacServlet?api=getOutsideUnsubscribeContact&e66Ve62i57=true&m9UCc1ucRi=540&h2BNp44yL7=109490&h5p8pKsyMi8l0=0&h2ZNp64yM8=null&h2TNp86wP9=5&h2GQy54sI2=759" style="font: 11px verdana;"><font style="font: 11px verdana;" color="blue">Click here </font></a><span style="font: 11px verdana;">to unsubscribe.</span></div>


<style type="text/css">

		.font11 {color: black; font: 11px verdana}
		.font11bold {color: black; font: bold 11px verdana}	
		TABLE {font: 12px verdana}
	</style><br>
<table border="1" align="center" width="100%" cellpadding="10" cellspacing="0" style="border-left-color: #006aaf; border-bottom-color: #006aaf; border-top-color: #006aaf; border-collapse: collapse; border-right-color: #006aaf">
    <tbody>
        <tr>
            <td width="100%">
            <div id="customerLogo"><img src="cid:fv6aWuSn0a" alt="" border="0"></div>
            <div style="font: 12px verdana" id="messagebody">&nbsp;</div>
            <div style="font: 12px verdana" id="Signature"><br>
            <br>
            <div style="margin: 0in 0in 0pt"><span style="font-size: 11pt; font-family: Calibri"><font size="3">Hi,</font></span></div>
            <div style="margin: 0in 0in 0pt"><strong><font size="3" face="Calibri">This is Sanjeev from Xperttech, Inc</font></strong></div>
            <div style="margin: 0in 0in 0pt"><strong><font size="3">&nbsp;</font></strong></div>
            <div style="margin: 0in 0in 0pt"><strong><font size="3" face="Calibri">Our client is looking for SQL Server DBA/ Developer at Houston, TX for Contact to hire, please go thru the position below Job Description and let me know</font></strong></div>
            <div style="margin: 0in 0in 0pt">&nbsp;</div>
            <div style="margin: 0in 0in 0pt"><strong><span style="font-size: 11pt; color: #003366; font-family: Calibri">SQL Database developer (SQL Server)</span></strong></div>
            <div style="margin: 0in 0in 0pt"><strong><span style="font-size: 11pt; color: #003366; font-family: Calibri">Location: Houston, TX</span></strong></div>
            <div style="margin: 0in 0in 0pt"><strong><span style="font-size: 11pt; color: #003366; font-family: Calibri">Duration: 4 Months</span></strong></div>
            <div style="margin: 0in 0in 0pt"><strong><span style="font-size: 11pt; color: #003366; font-family: Calibri">Rate: $40-$42/hr max we can pay to consultant</span><span style="font-size: 11pt; font-family: Calibri"><br>
            <br>
            <strong><u>Job Description:</u></strong></span></strong></div>
            <div style="margin: 0in 0in 0pt"><span style="font-size: 11pt; font-family: Calibri"><br>
            The Database Developer position will be responsible for the development of database objects and procedural code. Responsibilities will include design and development of efficient database stored procedures, triggers, constraints, and ETL (extraction, transformation and data loading) programs. Responsibilities also include overall development success (inclusive of peer/code reviews, adherence to corporate standards, build process guidelines, maintainability, and unit testing). Responsible to provide status of assigned tasks regularly and on request to the project manager. </span></div>
            <div style="margin: 0in 0in 0pt">&nbsp;</div>
            <div style="margin: 0in 0in 0pt"><strong><u><span style="font-size: 11pt; font-family: Calibri">Knowledge, Skills, and Abilities Requirements:</span></u></strong></div>
            <div style="margin: 0in 0in 0pt"><span style="font-size: 11pt; font-family: Calibri"><br>
            &bull;List unique knowledge, skills, or problem-solving requirements.<br>
            &bull;Strong SQL programming skills (developing stored procedure, trigger and constraint development)<br>
            &bull;Experience developing ETL (Extract-Transform-Load) programs<br>
            &bull;SQL query performance tuning experience<br>
            &bull;Experience working with large databases (+) <br>
            &nbsp;&nbsp; Structured troubleshooting skills Toolset: <br>
            &bull;Microsoft SQL Server 2005 RDBMS<br>
            &bull;Microsoft SQL Server 2005 Integration Services (SSIS)&nbsp;<br>
            <br>
            Sanjeev Coduru<br>
            sanjeev@xperttech.com<br>
            781-767-7573</span></div>
            </div>
            </td>
        </tr>
    </tbody>
</table>
<div style="padding:3px" width="100%" align="center" id="removeLink"><a href="http://portal.zoniacswift.com//servlet/ZoniacServlet?api=getOutsideUnsubscribeContact&e66Ve62i57=true&m9UCc1ucRi=540&h2BNp44yL7=109490&h5p8pKsyMi8l0=0&h2ZNp64yM8=null&h2TNp86wP9=5&h2GQy54sI2=759" class="font10"><font color="blue">Click here </font></a><span class="font10">to unsubscribe from our mailing list.</span><br><span class="font10">XPERTTECH  Inc, 400 W Cummings Park, Suite #2850, Woburn, Massachusetts, 01801      Phone: 781 935 3775 Fax: 978-405-5040</span></div>
<div style="display:block;" width="100%" align="center"><table width="100%" align="center"><tr><td align="right" width="55%"><a target="_new" href="http://www.zoniac.com" style="vertical-align: text-top;"><font style="vertical-align: text-top;">Powered by Zoniac</font></a></td><td align="left" width="45%"><IMG align="bottom" height="20" width="20" border="0" SRC="cid:19jWAUPxEF"></td></tr></table></div><div id="removeLinkDummy1"></div><div id="removeLinkDummy2"></div><div id="removeLinkDummy3"></div><div id="removeLinkDummy4"></div><div id="removeLinkDummy5"></div><div id="removeLinkDummy6"></div>

--=_reb-r4D74772A-t4D8D04D1
Content-Type: image/gif; name="/Companies/540//custom/customerLogo.gif"
Content-Transfer-Encoding: base64
Content-Disposition: inline; 
	filename="/Companies/540//custom/customerLogo.gif"
Content-ID: <fv6aWuSn0a>



--=_reb-r4D74772A-t4D8D04D1
Content-Type: image/gif; name="/custom/UnsubscribedPoweredByImg.gif"
Content-Transfer-Encoding: base64
Content-Disposition: inline; filename="/custom/UnsubscribedPoweredByImg.gif"
Content-ID: <19jWAUPxEF>



--=_reb-r4D74772A-t4D8D04D1--

