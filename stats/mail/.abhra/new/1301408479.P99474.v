Return-Path: selva@jjcpl.net
Received: from mx82.stngva01.us.mxservers.net (204.202.242.81)
	by mail19a.g19.rapidsite.net (RS ver 1.0.95vs) with SMTP id 4-0903008479
	for <abhra@altechstar.com>; Tue, 29 Mar 2011 10:21:19 -0400 (EDT)
Received: from unknown [216.151.164.100] (EHLO mail.njtech.com)
	by va1-mx82.stngva01.us.mxservers.net (mxl_mta-3.1.0-05)
	with ESMTP id edae19d4.1325353888.371268.00-006.va1-mx82.stngva01.us.mxservers.net (envelope-from <selva@jjcpl.net>);
	Tue, 29 Mar 2011 10:21:18 -0400 (EDT)
Received: (qmail 1699 invoked from network); 29 Mar 2011 14:21:17 -0000
Received: from unknown (HELO NEW2-PC) ([122.164.25.131])
  by 10.97.0.65 with SMTP; 29 Mar 2011 14:21:15 -0000
From: "selva" <selva@jjcpl.net> 
To: "abhra@altechstar.com" <abhra@altechstar.com>
Reply-To: selva@jjcpl.net
Date: Tue, 29 Mar 2011 09:21:01 -0500
Subject: Data Analyst 
MIME-Version: 1.0
Content-Type: multipart/related;
	type="multipart/alternative";
	boundary="=_reb-r4FF8B3BB-t4D91EADF"
X-Mailer: aspNetEmail ver 3.6.0.66
Message-ID: <NEW2-PC00880428236642e19ade2998bc7f5922@NEW2-PC>
X-Processed-By: Rebuild v2.0-0
X-Spam: [F=0.2000000000; B=0.500(0); S=0.200(2010122901); MH=0.500(2011032914)]
X-MAIL-FROM: <selva@jjcpl.net>
X-SOURCE-IP: [216.151.164.100]
X-SF-Loop: 1

This is a multi-part MIME message.

--=_reb-r4FF8B3BB-t4D91EADF
Content-Type: multipart/alternative;
	boundary="=_reb-r5ADC7303-t4D91EADF"

This is a multi-part MIME message.

--=_reb-r5ADC7303-t4D91EADF
Content-Type: text/plain;
	charset="iso-8859-1"
Content-Transfer-Encoding: quoted-printable

Thanks for choosing JJCPL

Regards

R.Selva Kumar

"we forecast you"=20

469 327 3112

http://www.jjcpl.net=20

Skype: jjcpl.india

Gtalk : jjcpl.rpo



--=_reb-r5ADC7303-t4D91EADF
Content-Type: text/html;
	charset="iso-8859-1"
Content-Transfer-Encoding: quoted-printable

<!DOCTYPE HTML PUBLIC "-//W3C//DTD HTML 4.0 Transitional//EN">
<HTML xmlns:o=3D"urn:schemas-microsoft-com:office:office"><HEAD>

</HEAD>
<BODY><SPAN style=3D"mso-fareast-font-family: 'Times New Roman'; mso-fareas=
t-theme-font: minor-fareast; mso-no-proof: yes"><FONT face=3D"Calibri"><SPA=
N style=3D"mso-fareast-font-family: 'Times New Roman'; mso-fareast-theme-fo=
nt: minor-fareast; mso-no-proof: yes"><FONT face=3D"Calibri"><SPAN style=3D=
"mso-fareast-font-family: 'Times New Roman'; mso-fareast-theme-font: minor-=
fareast; mso-no-proof: yes"><FONT face=3D"Calibri">
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt"><SPAN style=3D"FONT-FA=
MILY: 'Times New Roman','serif'; FONT-SIZE: 12pt">Hi=20
abhra,<o:p></o:p></SPAN></P>
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt"><SPAN style=3D"FONT-FA=
MILY: 'Times New Roman','serif'; FONT-SIZE: 12pt"><o:p>&nbsp;</o:p></SPAN><=
/P>
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt"><SPAN style=3D"FONT-FA=
MILY: 'Times New Roman','serif'; FONT-SIZE: 12pt">Hope doing=20
good<o:p></o:p></SPAN></P>
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt"><SPAN style=3D"FONT-FA=
MILY: 'Times New Roman','serif'; FONT-SIZE: 12pt"><o:p>&nbsp;</o:p></SPAN><=
/P>
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt"><SPAN style=3D"FONT-FA=
MILY: 'Times New Roman','serif'; FONT-SIZE: 12pt">Please check the=20
below JD if you have any suitable consultant revert me=20
ASAP.<o:p></o:p></SPAN></P>
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt"><SPAN style=3D"FONT-FA=
MILY: 'Times New Roman','serif'; FONT-SIZE: 12pt"><o:p>&nbsp;</o:p></SPAN><=
/P>
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt"><SPAN style=3D"FONT-FA=
MILY: 'Times New Roman','serif'; FONT-SIZE: 12pt">Role - Data=20
Analyst <BR>Location - Portland,ME <BR>Duration - 9 months <BR>Rate -=20
&nbsp;&nbsp;DOE<o:p></o:p></SPAN></P>
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt"><SPAN style=3D"FONT-FA=
MILY: 'Times New Roman','serif'; FONT-SIZE: 12pt"><o:p>&nbsp;</o:p></SPAN><=
/P>
<P style=3D"MARGIN: 0in 0in 0pt"><FONT face=3D"Times New Roman"><SPAN style=
=3D"BACKGROUND: lime; FONT-SIZE: 24pt; mso-highlight: lime">F2F is a must=
=20
after telephonic interview</SPAN><SPAN style=3D"FONT-SIZE: 24pt">&nbsp;<o:p=
></o:p></SPAN></FONT></P>
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt"><SPAN style=3D"FONT-FA=
MILY: 'Times New Roman','serif'; FONT-SIZE: 12pt"><BR>Technical=20
Environment: Informatica 8.6.x, Teradata, AIX UNIX <BR><BR>Responsibilities=
:=20
<BR><BR>Uses the Informatica Data Integration platform to extract, transfor=
m,=20
and load data <BR>Uses the Informatica Data Transformation technologies pla=
tform=20
to parse, map, serialize and split data in unstructured, semi-structured an=
d=20
complex XML formats <BR>Works with Data Integration developers to develop D=
ata=20
Integration Workflows and load processes <BR>Develops Informatica data=20
transformation designs <BR>Ensures adherence to proprietary and open data f=
ormat=20
standards for all developed components <BR>Ensures adherence to locally def=
ined=20
standards for all developed components <BR>Performs data analysis for sourc=
e and=20
target data <BR>May create external data transformation components using Ja=
va,=20
.Net, XSLT or other technologies <BR>Provides technical documentation of=20
transformation components <BR>Supports the development and design of the=20
internal data integration framework <BR>Participates in design and developm=
ent=20
reviews <BR>Works with system owners to resolve data transformation issues =
and=20
to refine transformation components <BR>Ensures performance metrics are met=
 and=20
tracked <BR>Collaborates with ends users and other stakeholders to define=
=20
detailed requirements <BR>Designs business intelligence solutions that meet=
 user=20
requirements for accessing and analyzing data <BR>Works with front-end busi=
ness=20
intelligence tools to design the reporting environment <BR>Works with the D=
BA=20
and Data Architect to optimize reporting performance <BR>Develops supportin=
g=20
documentation for the application <BR>Participates in the full testing cycl=
e=20
<BR>Develops technical design documents <BR><BR>&nbsp; <BR>Expertise:=20
<BR><BR>Has Strong Informatica Data Integration Platform experience (5 to 7=
=20
Years of Experience) <BR>Has Strong Teradata RDBMS skills including Loader=
=20
utilities (MLOAD, FASTLOAD, TPUMP) and SQL experience (3 to 5 Years of=20
Experience) <BR>Has experience on UNIX platform <BR>Solid understanding of=
=20
metadata concepts and the Data Warehouse/Data Mart model <BR>Strong=20
understanding of data integration concepts <BR>Ability to translate functio=
nal=20
requirements into technical design specifications <BR>Experience in optimiz=
ing=20
data loads and data transformations <BR>Strong Business Analysis and proble=
m=20
solving skills <BR>Aptitude with front-end business intelligence tools (i.e=
.,=20
Cognos, Informatica Data Analyzer) <BR>Excellent problem solving and=20
trouble-shooting skills <BR>Solid interpersonal skills and ability to work =
with=20
business and system owners to obtain requirements and manage expectations=
=20
<BR>Understanding of industry standard data integration architectures=20
<BR>Familiarity with appropriate modeling techniques such as UML and ER mod=
eling=20
as appropriate <BR>Capable of expressing technical concepts in business ter=
ms=20
<BR>Understands data integration processes and how to tune for performance=
=20
<BR>Has the ability to develop work plans and follow through on assignments=
 with=20
minimal guidance <BR>Has the ability to work with business and system owner=
s to=20
obtain requirements and manage expectations <BR>Solid understanding of XML =
and=20
XML Schema technologies <BR>Understands data transformation (e.g., parsing,=
=20
mapping and serialization processes) <BR>Possesses excellent communications=
=20
skills <BR>Software development expertise (previous development experience =
of=20
the application type) <BR>Deep understanding of all technical components of=
 the=20
application solution<o:p></o:p></SPAN></P>
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt"></FONT></SPAN></FONT><=
/SPAN></FONT></SPAN><SPAN style=3D"mso-fareast-font-family: 'Times New Roma=
n'; mso-fareast-theme-font: minor-fareast; mso-no-proof: yes"><FONT face=3D=
"Calibri"></FONT></SPAN></P>
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt"><SPAN style=3D"mso-far=
east-font-family: 'Times New Roman'; mso-fareast-theme-font: minor-fareast;=
 mso-no-proof: yes"><FONT face=3D"Calibri"></FONT></SPAN>&nbsp;</P>
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt"><SPAN style=3D"mso-far=
east-font-family: 'Times New Roman'; mso-fareast-theme-font: minor-fareast;=
 mso-no-proof: yes"><FONT face=3D"Calibri"></FONT></SPAN>&nbsp;</P><SPAN st=
yle=3D"mso-fareast-font-family: 'Times New Roman'; mso-fareast-theme-font: =
minor-fareast; mso-no-proof: yes"><FONT face=3D"Calibri">
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt"><SPAN style=3D"mso-far=
east-font-family: 'Times New Roman'; mso-fareast-theme-font: minor-fareast;=
 mso-no-proof: yes"><FONT face=3D"Calibri">Thanks for choosing JJCPL<o:p></=
o:p></FONT></SPAN></P>
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt"><SPAN style=3D"mso-far=
east-font-family: 'Times New Roman'; mso-fareast-theme-font: minor-fareast;=
 mso-no-proof: yes"><FONT face=3D"Calibri">Regards<o:p></o:p></FONT></SPAN>=
</P>
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt"><B><I><SPAN style=3D"F=
ONT-FAMILY: 'Century Gothic','sans-serif'; COLOR: #215868; FONT-SIZE: 12pt;=
 mso-fareast-font-family: 'Times New Roman'; mso-fareast-theme-font: minor-=
fareast; mso-no-proof: yes">Selva</SPAN></I></B></P>
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt"><IMG height=3D"35" wid=
th=3D"138" src=3D"cid:image_0000" alt=3D""></P>
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt"><B><SPAN style=3D"FONT=
-FAMILY: 'Tahoma','sans-serif'; COLOR: #f2f2f2; FONT-SIZE: 7pt; mso-fareast=
-font-family: 'Times New Roman'; mso-fareast-theme-font: minor-fareast; mso=
-no-proof: yes"></SPAN></B><SPAN style=3D"COLOR: red; mso-fareast-font-fami=
ly: 'Times New Roman'; mso-fareast-theme-font: minor-fareast; mso-no-proof:=
 yes"><FONT face=3D"Calibri">&#8220;<I style=3D"mso-bidi-font-style: normal=
">we forecast you</I>&#8221; </P>
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt"></FONT></SPAN><SPAN st=
yle=3D"FONT-FAMILY: 'Bookman Old Style','serif'; FONT-SIZE: 9pt; mso-fareas=
t-font-family: 'Times New Roman'; mso-fareast-theme-font: minor-fareast; ms=
o-no-proof: yes"><o:p>469=20
327 3112</o:p></SPAN></P>
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt"><SPAN style=3D"mso-far=
east-font-family: 'Times New Roman'; mso-fareast-theme-font: minor-fareast;=
 mso-no-proof: yes"><A href=3D"http://www.jjcpl.net/"><SPAN style=3D"COLOR:=
 blue"><FONT face=3D"Calibri">http://www.jjcpl.net</FONT></SPAN></A><FONT f=
ace=3D"Calibri">=20
<o:p></o:p></FONT></SPAN></P>
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt"><SPAN style=3D"mso-far=
east-font-family: 'Times New Roman'; mso-fareast-theme-font: minor-fareast;=
 mso-no-proof: yes"><FONT face=3D"Calibri">Skype: jjcpl.rpo</FONT></SPAN></=
P>
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt"><SPAN style=3D"mso-far=
east-font-family: 'Times New Roman'; mso-fareast-theme-font: minor-fareast;=
 mso-no-proof: yes"><FONT face=3D"Calibri">Gtalk : jjcpl.rpo</FONT></SPAN><=
/P><SPAN style=3D"mso-fareast-font-family: 'Times New Roman'; mso-fareast-t=
heme-font: minor-fareast; mso-no-proof: yes">
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt"><B><SPAN style=3D"COLO=
R: #31849b; FONT-SIZE: 11pt"><FONT face=3D"Times New Roman" color=3D"#0000f=
f"><A href=3D"http://jjcplrpo.blogspot.com">http://jjcplrpo.blogspot.com</A=
>=20
</FONT></SPAN></B></P></SPAN><SPAN style=3D"mso-fareast-font-family: 'Times=
 New Roman'; mso-fareast-theme-font: minor-fareast; mso-no-proof: yes">
<P class=3D"yiv1141086511msonormal" style=3D"MARGIN: 0in 0in 0pt"><STRONG><=
SPAN style=3D"FONT-FAMILY: 'Calibri','sans-serif'; COLOR: gray">Note: We re=
spect your=20
online privacy. This is not an unsolicited e-mail. If you are not intereste=
d in=20
receiving our e-mails then please reply with a</SPAN></STRONG><SPAN style=
=3D"FONT-FAMILY: 'Calibri','sans-serif'; COLOR: gray; FONT-SIZE: 10pt">=20
</SPAN><STRONG><SPAN style=3D"FONT-FAMILY: 'Calibri','sans-serif'; COLOR: r=
ed">"REMOVE"</SPAN></STRONG><SPAN style=3D"FONT-FAMILY: 'Calibri','sans-ser=
if'; COLOR: gray; FONT-SIZE: 10pt">=20
</SPAN><STRONG><SPAN style=3D"FONT-FAMILY: 'Calibri','sans-serif'; COLOR: g=
ray">in=20
the subject line. All removal requests will be honored ASAP. We sincerely=
=20
apologize for any inconvenience caused to you</SPAN></STRONG><SPAN style=3D=
"FONT-FAMILY: 'Calibri','sans-serif'; FONT-SIZE: 10pt"><o:p></o:p></SPAN></=
P></SPAN></FONT></SPAN></BODY></HTML>


--=_reb-r5ADC7303-t4D91EADF--


--=_reb-r4FF8B3BB-t4D91EADF
Content-Type: image/jpeg; name="logo%20new%201.jpg"
Content-Transfer-Encoding: base64
Content-ID: <image_0000>
Content-Disposition: inline



--=_reb-r4FF8B3BB-t4D91EADF--

