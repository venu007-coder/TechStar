Return-Path: sameer@xperttech.com
Received: from mx99.stngva01.us.mxservers.net (198.173.112.36)
	by mail19i.g19.rapidsite.net (RS ver 1.0.95vs) with SMTP id 3-0524309378
	for <abhra@altechstar.com>; Thu,  3 Mar 2011 11:22:58 -0500 (EST)
Received: from unknown [76.74.252.24] (EHLO rs78.luxhv.com)
	by va1-mx99.stngva01.us.mxservers.net (mxl_mta-3.1.0-05)
	with ESMTP id 160cf6d4.2454715296.296756.00-010.va1-mx99.stngva01.us.mxservers.net (envelope-from <sameer@xperttech.com>);
	Thu, 03 Mar 2011 11:22:57 -0500 (EST)
Received: from ip-10-196-81-219.ec2.internal (ec2-184-72-246-77.compute-1.amazonaws.com [184.72.246.77])
	(authenticated bits=0)
	by rs78.luxhv.com (8.13.8/8.13.8) with ESMTP id p23GMhdP017038;
	Thu, 3 Mar 2011 10:22:43 -0600
Date: Thu, 3 Mar 2011 11:22:42 -0500 (GMT-05:00)
From: Sameer Maharana <sameer@xperttech.com>
To: sameer@xperttech.com
Message-ID: <1299169362820.1698840419739952.sameer@xperttech.com>
Subject: Sr. Biztalk Developer // Seattle WA // 3+ months rolling contract
MIME-Version: 1.0
Content-Type: multipart/mixed; 
	boundary="=_reb-r1AFD4A66-t4D6FC062"
Importance: High
X-Lux-Comment: Message p23GMhdP017038 sent by user #68993; report Spam, UCE, or Abuse to abuse@luxsci.us
X-Lux-Comment: LuxSci High Volume Email: http://luxhv.com
X-Processed-By: Rebuild v2.0-0
X-Spam: [F=0.2000000000; B=0.500(0); S=0.200(2010122901); MH=0.500(2011030314)]
X-MAIL-FROM: <sameer@xperttech.com>
X-SOURCE-IP: [76.74.252.24]
X-SF-Loop: 1

This is a multi-part MIME message.

--=_reb-r1AFD4A66-t4D6FC062
Content-Type: text/html; charset=us-ascii
Content-Transfer-Encoding: 7bit

<div style="padding:3px" width="100%" align="right" id="removeTopLink"><a href="http://portal.zoniacswift.com//servlet/ZoniacServlet?api=getOutsideUnsubscribeContact&e66Ve62i57=true&m9UCc1ucRi=540&h2BNp44yL7=108734&h5p8pKsyMi8l0=0&h2ZNp64yM8=null&h2TNp86wP9=5&h2GQy54sI2=1746" style="font: 11px verdana;"><font style="font: 11px verdana;" color="blue">Click here </font></a><span style="font: 11px verdana;">to unsubscribe.</span></div>


<style type="text/css">

		.font11 {color: black; font: 11px verdana}
		.font11bold {color: black; font: bold 11px verdana}	
		TABLE {font: 12px verdana}
	</style><br>
<table border="1" align="center" width="100%" cellpadding="10" cellspacing="0" style="border-left-color: #006aaf; border-bottom-color: #006aaf; border-top-color: #006aaf; border-collapse: collapse; border-right-color: #006aaf">
    <tbody>
        <tr>
            <td width="100%">
            <div id="customerLogo"><img alt="" border="0" src="cid:7EviR32mMc"></div>
            <div style="font: 12px verdana" id="messagebody">&nbsp;</div>
            <div style="font: 12px verdana" id="Signature">
            <div style="margin: 0in 0in 0pt"><span style="font-size: 11pt; font-family: Candara"><font size="3">Hi, </font></span></div>
            <div style="margin: 0in 0in 0pt"><font size="3">&nbsp;</font></div>
            <div style="margin: 0in 0in 0pt"><span style="font-size: 11pt; font-family: Candara"><font size="3">I have an immediate requirement opening for a <span style="background: yellow">Sr. Biz Talk Developer position in Seattle WA. This is a 3+ months rolling contract position</span>. I would appreciate if you could send your updated word format resume along with best contact details ASAP.</font></span></div>
            <div style="margin: 0in 0in 0pt"><font size="3">&nbsp;</font></div>
            <div style="margin: 0in 0in 0pt"><span style="font-size: 11pt; font-family: Candara"><font size="3">Here are the requirement details:</font></span></div>
            <div style="margin: 0in 0in 0pt"><font size="3">&nbsp;</font></div>
            <div style="margin: 0in 0in 0pt"><span style="font-size: 11pt; color: blue; font-family: Candara"><font size="3">Position &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; : Sr. Biz Talk Developer</font></span></div>
            <div style="margin: 0in 0in 0pt"><span style="font-size: 11pt; color: blue; font-family: Candara"><font size="3">Location&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; : Seattle WA</font></span></div>
            <div style="margin: 0in 0in 0pt"><span style="font-size: 11pt; color: blue; font-family: Candara"><font size="3">Duration &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; : 3+ months rolling Extension project</font></span></div>
            <div style="margin: 0in 0in 0pt"><font size="3">&nbsp;</font></div>
            <div style="margin: 0in 0in 0pt"><u><span style="font-size: 11pt; font-family: Candara"><font size="3">Responsibilities: </font></span></u></div>
            <ul type="disc" style="margin-top: 0in">
                <li style="margin: 0in 0in 0pt"><span style="font-size: 11pt; font-family: Candara"><font size="3">Design and develop integration/messaging solutions. </font></span></li>
                <li style="margin: 0in 0in 0pt"><span style="font-size: 11pt; font-family: Candara"><font size="3">Design and develop BizTalk Pipeline components, Maps </font></span></li>
                <li style="margin: 0in 0in 0pt"><span style="font-size: 11pt; font-family: Candara"><font size="3">Prepare and review design documents </font></span></li>
                <li style="margin: 0in 0in 0pt"><span style="font-size: 11pt; font-family: Candara"><font size="3">Design and develop.NET components. </font></span></li>
                <li style="margin: 0in 0in 0pt"><span style="font-size: 11pt; font-family: Candara"><font size="3">Document technical implementations. </font></span></li>
                <li style="margin: 0in 0in 0pt"><span style="font-size: 11pt; font-family: Candara"><font size="3">Unit test technical solutions.</font></span></li>
            </ul>
            <div style="margin: 0in 0in 0pt"><span style="font-size: 11pt; font-family: Candara"><font size="3">&nbsp;</font></span></div>
            <div style="margin: 0in 0in 0pt"><u><span style="font-size: 11pt; font-family: Candara"><font size="3">Skills Needed: </font></span></u></div>
            <ul type="disc" style="margin-top: 0in">
                <li style="margin: 0in 0in 0pt"><span style="font-size: 11pt; font-family: Candara"><font size="3">Minimum 4 years experience with Integration Projects using BizTalk Server, 2006R2, 2009 and/or 2010 with overall experience of 8 Years in development on Microsoft technologies such as BizTalk Server, C#, SQL Server. </font></span></li>
                <li style="margin: 0in 0in 0pt"><span style="font-size: 11pt; font-family: Candara"><font size="3">Experience working on integration projects using EDI, AS2 is a must. </font></span></li>
                <li style="margin: 0in 0in 0pt"><span style="font-size: 11pt; font-family: Candara"><font size="3">Very well versed with all the basic BizTalk adapters including WCF Custom SQL, WCF Custom SAP adapters, Party Configuration, Creating custom pipeline components, Business Activity Monitoring, Business Rule Engine, etc. </font></span></li>
                <li style="margin: 0in 0in 0pt"><span style="font-size: 11pt; font-family: Candara"><font size="3">Secondary Skill sets required - NET development: C#, Visual Studio 2005/2008 </font></span></li>
                <li style="margin: 0in 0in 0pt"><span style="font-size: 11pt; font-family: Candara"><font size="3">SQL Server 2005/2008 </font></span></li>
                <li style="margin: 0in 0in 0pt"><span style="font-size: 11pt; font-family: Candara"><font size="3">Good oral and written communication skills</font></span></li>
            </ul>
            <div style="margin: 0in 0in 0pt"><font size="3">&nbsp;</font></div>
            <div style="margin: 0in 0in 0pt"><a name="_MailAutoSig"><span style="font-size: 10pt; color: black; font-family: Cambria"><font size="3">Thanks/Regards,</font></span></a></div>
            <div style="margin: 0in 0in 0pt"><strong><span style="font-size: 16pt; color: green; font-family: 'Trebuchet MS'"><font size="3">SAMEER</font></span><span style="color: navy; font-family: Cambria"><font size="3">,</font></span><span style="color: navy; font-family: Cambria"><br>
            </span></strong></div>
            <div style="margin: 0in 0in 0pt"><span style="font-size: 10pt; color: black; font-family: Cambria"><font size="3">400 W Cummings Park,</font></span></div>
            <div style="margin: 0in 0in 0pt"><span style="font-size: 10pt; color: black; font-family: Cambria"><font size="3">Suite #<span style="font-size: 10pt; color: black; font-family: Cambria"> 2850</span></font></span></div>
            <div style="margin: 0in 0in 0pt"><span style="font-size: 10pt; color: black; font-family: Cambria"><font size="3">Woburn, MA-01801</font></span></div>
            <div style="margin: 0in 0in 0pt"><font size="3"><span style="font-size: 10pt; color: black; font-family: Cambria">Email:</span><u><span style="font-size: 10pt; color: blue; font-family: Cambria">sameer@xperttech.com</span></u></font></div>
            <div style="margin: 0in 0in 0pt"><span style="font-size: 10pt; color: black; font-family: Cambria"><font size="3">Ph# 781-399-4354 | Fax # 781 207 0709 | </font></span><a href="http://www.xperttech.com/" title="blocked::http://www.xperttech.com/"><span style="font-size: 10pt; font-family: Cambria"><font size="3">www.xperttech.com</font></span></a></div>
            <div style="margin: 0in 0in 0pt"><strong><font size="3">&nbsp;</font></strong></div>
            <div style="margin: 0in 0in 0pt"><strong><span style="font-size: 8pt; color: #1f497d; font-family: Arial"><font size="3">IMPORTANT NOTE</font></span><font size="3"><span style="font-size: 8pt; color: #1f497d; font-family: Arial">: If you have received this email in error or prefer not to receive such emails in future, please reply with &quot;&quot;</span><span style="font-size: 8pt; color: red; font-family: Arial">Remove</span><span style="font-size: 8pt; color: #1f497d; font-family: Arial">&quot;&quot; in the subject line and the email-id(s) to be removed. All removal requests will be honored ASAP.</span></font></strong></div>
            <div style="margin: 0in 0in 0pt"><font size="3"><span style="font-size: 8pt; color: green; font-family: Webdings">P </span><span style="font-size: 8pt; color: green; font-family: Calibri">Please consider the environment - do you really need to print this email?</span></font></div>
            <div><font size="3">&nbsp;</font></div>
            <div style="margin: 0in 0in 0pt"><font size="3">&nbsp;</font></div>
            </div>
            </td>
        </tr>
    </tbody>
</table>
<div style="padding:3px" width="100%" align="center" id="removeLink"><a href="http://portal.zoniacswift.com//servlet/ZoniacServlet?api=getOutsideUnsubscribeContact&e66Ve62i57=true&m9UCc1ucRi=540&h2BNp44yL7=108734&h5p8pKsyMi8l0=0&h2ZNp64yM8=null&h2TNp86wP9=5&h2GQy54sI2=1746" class="font10"><font color="blue">Click here </font></a><span class="font10">to unsubscribe from our mailing list.</span><br><span class="font10">XPERTTECH  Inc, 400 W Cummings Park, Suite #2850, Woburn, Massachusetts, 01801      Phone: 781 935 3775 Fax: 978-405-5040</span></div>
<div style="display:block;" width="100%" align="center"><table width="100%" align="center"><tr><td align="right" width="55%"><a target="_new" href="http://www.zoniac.com" style="vertical-align: text-top;"><font style="vertical-align: text-top;">Powered by Zoniac</font></a></td><td align="left" width="45%"><IMG align="bottom" height="20" width="20" border="0" SRC="cid:NE3cY231uG"></td></tr></table></div><div id="removeLinkDummy1"></div><div id="removeLinkDummy2"></div><div id="removeLinkDummy3"></div><div id="removeLinkDummy4"></div><div id="removeLinkDummy5"></div><div id="removeLinkDummy6"></div>

--=_reb-r1AFD4A66-t4D6FC062
Content-Type: image/gif; name="/Companies/540/custom/customerLogo.gif"
Content-Transfer-Encoding: base64
Content-Disposition: inline; 
	filename="/Companies/540/custom/customerLogo.gif"
Content-ID: <7EviR32mMc>



--=_reb-r1AFD4A66-t4D6FC062
Content-Type: image/gif; name="/custom/UnsubscribedPoweredByImg.gif"
Content-Transfer-Encoding: base64
Content-Disposition: inline; filename="/custom/UnsubscribedPoweredByImg.gif"
Content-ID: <NE3cY231uG>



--=_reb-r1AFD4A66-t4D6FC062--

