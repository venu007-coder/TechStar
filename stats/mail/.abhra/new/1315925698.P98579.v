Return-Path: sonali@vlinkinfo.com
Received: from mx121.stngva01.us.mxservers.net (198.173.112.50)
	by mail19e.g19.rapidsite.net (RS ver 1.0.95vs) with SMTP id 2-021295697
	for <abhra@altechstar.com>; Tue, 13 Sep 2011 10:54:57 -0400 (EDT)
Received: from unknown [76.74.252.24] (EHLO rs78.luxhv.com)
	by va1-mx121.stngva01.us.mxservers.net (mxl_mta-3.1.0-05)
	with ESMTP id 1ce6f6e4.2814352288.1573512.00-016.va1-mx121.stngva01.us.mxservers.net (envelope-from <sonali@vlinkinfo.com>);
	Tue, 13 Sep 2011 10:54:57 -0400 (EDT)
Received: from ip-10-218-41-168.ec2.internal (ec2-184-72-246-77.compute-1.amazonaws.com [184.72.246.77])
	(authenticated bits=0)
	by rs78.luxhv.com (8.13.8/8.13.8) with ESMTP id p8DEspSS024194;
	Tue, 13 Sep 2011 09:54:51 -0500
Date: Tue, 13 Sep 2011 10:54:50 -0400 (EDT)
From: Sonali Jain <sonali@vlinkinfo.com>
To: sonali@vlinkinfo.com
Message-ID: <1315925690766.766109470323865.sonali@vlinkinfo.com>
Subject: DIRECT CLIENT POSITION: Java/ Flex/ Javascript / Web Developer
 Immediate Interview
MIME-Version: 1.0
Content-Type: multipart/mixed; 
	boundary="=_reb-r0D16645B-t4E6F6EC1"
Importance: Normal
X-Processed-By: Rebuild v2.0-0
X-Spam: [F=0.2000000000; B=0.500(0); S=0.200(2010122901); MH=0.500(2011091313)]
X-MAIL-FROM: <sonali@vlinkinfo.com>
X-SOURCE-IP: [76.74.252.24]
X-SF-Loop: 1

This is a multi-part MIME message.

--=_reb-r0D16645B-t4E6F6EC1
Content-Type: text/html; charset=us-ascii
Content-Transfer-Encoding: 7bit

<div style="padding:3px" width="100%" align="right" id="removeTopLink"><a href="http://portal.zoniacswift.com//servlet/ZoniacServlet?api=getOutsideUnsubscribeContact&e66Ve62i57=true&m9UCc1ucRi=693&h2BNp44yL7=292640&h2ZNp64yM8=null&h2TNp86wP9=4" style="font: 11px verdana;"><font style="font: 11px verdana;" color="blue">Click here </font></a><span style="font: 11px verdana;">to unsubscribe.</span></div>


<style type="text/css">

		.font11 {color: black; font: 11px verdana}
		.font11bold {color: black; font: bold 11px verdana}	
		TABLE {font: 12px verdana}
	</style><br>
<table align="center" width="100%" cellpadding="10" cellspacing="0" border="1" style="border-bottom-color: #006aaf; border-top-color: #006aaf; border-collapse: collapse; border-right-color: #006aaf; border-left-color: #006aaf">
    <tbody>
        <tr>
            <td width="100%">
            <div id="customerLogo"><img src="cid:SPAylxbWgY" alt="" border="0"><br>
            <br>
            <br>
            <span style="font-family: 'Verdana','sans-serif'; background: white; color: black; font-size: 10pt">Hello Partner,</span><span style="font-family: 'Verdana','sans-serif'; background: white; color: black; font-size: 10pt"><br>
            <br>
            Please send qualified resumes for the following position.&nbsp; MAX pay rate is $&nbsp; /hr.&nbsp;for this position.&nbsp; Be sure to include&nbsp;rate and availability information. Rate is not issued for right candidates.<br>
            <br>
            Regards,<br>
            Sonali&nbsp;<br>
            IT Resource Specialist<br>
            860-247-1400 Ext 110</span>
            <div id="messagebody">
            <div>&nbsp;</div>
            </div>
            <br>
            <font face="Verdana" size="2">We are looking for an outstanding <strong>Java/Flex/Javascript Web Developer,</strong>someone with a passion for software development, charged with designing,implementing and testing high-availability, multi-tier server systems in aJava/Oracle environment. A top tier engineer who brings a deep understandingof not only the language, but has a solid theoretical foundation in computersciences such as data structures, computational algorithms and operatingsystems. Being a part of a small team, a willingness to work in all parts ofthe system is needed.<br>
            <br>
            </font><font face="Verdana"><font size="2"><strong>Responsibilities:<br>
            </strong><br>
            Participate in the analysis, design, development, and testing of WebApplications for consumer-facing web sites<br>
            Develop new products and perform upgrades, enhancements or maintenance toexisting products<br>
            Programming using Flex (ActionScript), JavaScript, AJAX, Java, Servlets/JSP,DOJO, XML, HTML/DHTML and other technical skills.<br>
            Participate effectively in both functional/technical group discussions withfellow developers and also in requirements definition meetings with theProduct team.<br>
            Explore product requirements to figure out technical difficulties, recommendimplementation approaches, and draft functional specifications (FRDs).<br>
            Perform unit testing and provide guidance/support during QA/system testing<br>
            Resolve defects during QA testing and in post-release patches<br>
            Work and communicate effectively with cross-functional teams&nbsp; as needed,including collaborating with team members.Other duties as assigned<br>
            <br>
            </font></font><font face="Verdana"><font size="2"><strong>Qualifications:<br>
            </strong>Bachelor's degree in Computer Science.<br>
            Master's degree in Computer Science (preferred) or equivalent.<br>
            Experience in Flex (ActionScript) and JavaScript and Java programminglanguages.<br>
            Experience with JavaScript (OOP in JavaScript, ability to work with andimprove JavaScript frameworks), CSS, and HTML.<br>
            Experience with strong working knowledge of Object Oriented Programming concepts and design patterns.<br>
            Experience with MVC frameworks such as Struts, Spring MVC or Echo<br>
            Solid understanding of J2EE, XML, XSD, XSLT, SOAP, web services, andrelevant design patterns.<br>
            Extensive experience with programming in a multithreaded environment.<br>
            Knowledge and understanding of the DOM as it relates to multiple platforms.<br>
            Intimate familiarity with the HTTP protocol.<br>
            Experience with server side UI component frameworks (JSF) is preferred.<br>
            Working knowledge of Perl is preferred.<br>
            <br>
            </font></font><font face="Verdana"><font size="2"><strong>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; Location&nbsp;: Elmwood Park, NJ<br>
            <br>
            </strong><br>
            <br>
            </font></font>
            <p><font face="Verdana"><font size="2"><span style="background: white; color: black">Best regards,<br>
            Sonali<br>
            IT Resource Specialist</span><o:p></o:p></font></font></p>
            <p><span style="background: white; color: black"><font face="Verdana"><font size="2">VLink Inc | Phone: 860-247-1400&nbsp; x110&nbsp;| Fax: 860 256 8484 |<span class="apple-converted-space">&nbsp;</span></font></font></span><a target="_blank" href="mailto:sonali@vlinkinfo.com"><span style="background: white"><font face="Verdana" size="2">sonali@vlinkinfo.com</font></span></a><font face="Verdana"><font size="2"><span class="apple-converted-space"><span style="background: white; color: black">&nbsp;</span></span><span style="background: white; color: black">|<span class="apple-converted-space">&nbsp;</span></span></font></font><a target="_blank" href="http://www.vlinkinfo.com/"><span style="background: white"><font face="Verdana" size="2">www.vlinkinfo.com</font></span></a><span style="background: white; color: black"><br>
            <br>
            <font face="Verdana"><font size="2">LEGAL NOTICE:<br>
            Unless expressly stated otherwise, the information contained in this message is confidential, may be privileged, and protected from disclosure. If the reader of this message is not the intended recipient, or an employee or agent responsible for delivering this message to the intended recipient, you are hereby notified that any dissemination, distribution, or copying of this communication is strictly prohibited. If you have received this communication in error, please notify the sender immediately by replying to the message, permanently deleting it from your computer and/or destroy any copies/printouts of this message.&nbsp;<o:p></o:p></font></font></span></p>
            </div>
            <div id="messagebody" style="font: 12px verdana">&nbsp;</div>
            <div id="Signature" style="font: 12px verdana">&nbsp;</div>
            </td>
        </tr>
    </tbody>
</table>
<div style="padding:3px" width="100%" align="center" id="removeLink"><a href="http://portal.zoniacswift.com//servlet/ZoniacServlet?api=getOutsideUnsubscribeContact&e66Ve62i57=true&m9UCc1ucRi=693&h2BNp44yL7=292640&h2ZNp64yM8=null&h2TNp86wP9=4" class="font10"><font color="blue">Click here </font></a><span class="font10">to unsubscribe from our mailing list.</span><br><span class="font10">VLINK INC, 635 Farmington Ave, , Hartford, Connecticut, 06105      Phone: 860.247.1400 Fax: </span></div>
<div style="display:block;" width="100%" align="center"><table width="100%" align="center"><tr><td align="right" width="55%"><a target="_new" href="http://www.zoniac.com" style="vertical-align: text-top;"><font style="vertical-align: text-top;">Powered by Zoniac</font></a></td><td align="left" width="45%"><IMG align="bottom" height="20" width="20" border="0" SRC="cid:S36R7qrOPe"></td></tr></table></div><div id="removeLinkDummy1"></div><div id="removeLinkDummy2"></div><div id="removeLinkDummy3"></div><div id="removeLinkDummy4"></div><div id="removeLinkDummy5"></div><div id="removeLinkDummy6"></div>
<div style="padding:3px" width="100%" align="left" id="showDisclaimerData"></div>

--=_reb-r0D16645B-t4E6F6EC1
Content-Type: image/gif; name="/Companies/693//custom/customerLogo.gif"
Content-Transfer-Encoding: base64
Content-Disposition: inline; 
	filename="/Companies/693//custom/customerLogo.gif"
Content-ID: <SPAylxbWgY>



--=_reb-r0D16645B-t4E6F6EC1
Content-Type: image/gif; name="/custom/UnsubscribedPoweredByImg.gif"
Content-Transfer-Encoding: base64
Content-Disposition: inline; filename="/custom/UnsubscribedPoweredByImg.gif"
Content-ID: <S36R7qrOPe>



--=_reb-r0D16645B-t4E6F6EC1--

