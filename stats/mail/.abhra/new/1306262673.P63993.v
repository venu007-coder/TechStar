Return-Path: ansha@jjcpl.net
Received: from mx121.stngva01.us.mxservers.net (198.173.112.50)
	by mail19d.g19.rapidsite.net (RS ver 1.0.95vs) with SMTP id 2-0309092672
	for <abhra@altechstar.com>; Tue, 24 May 2011 14:44:33 -0400 (EDT)
Received: from unknown [216.151.164.100] (EHLO mail.njtech.com)
	by va1-mx121.stngva01.us.mxservers.net (mxl_mta-3.1.0-05)
	with ESMTP id 09cfbdd4.2893409184.967431.00-010.va1-mx121.stngva01.us.mxservers.net (envelope-from <ansha@jjcpl.net>);
	Tue, 24 May 2011 14:44:32 -0400 (EDT)
Received: (qmail 2207 invoked from network); 24 May 2011 18:44:32 -0000
Received: from unknown (HELO NEW2-PC) ([122.174.88.15])
  by 10.97.0.65 with SMTP; 24 May 2011 18:44:26 -0000
From: "Ansha" <ansha@jjcpl.net> 
To: "abhra@altechstar.com" <abhra@altechstar.com>
Reply-To: ansha@jjcpl.net
Date: Tue, 24 May 2011 13:44:12 -0500
Subject: Need a QA Lead 
MIME-Version: 1.0
Content-Type: multipart/related;
	type="multipart/alternative";
	boundary="=_reb-r44348D41-t4DDBFC90"
X-Mailer: aspNetEmail ver 3.6.0.66
Message-ID: <NEW2-PCde3e9d1319044e0ab43a54d2c383872e@NEW2-PC>
X-Processed-By: Rebuild v2.0-0
X-Spam: [F=0.3170731707; B=0.500(0); S=0.200(2010122901); MH=0.650(2011052430)]
X-MAIL-FROM: <ansha@jjcpl.net>
X-SOURCE-IP: [216.151.164.100]
X-SF-Loop: 1

This is a multi-part MIME message.

--=_reb-r44348D41-t4DDBFC90
Content-Type: multipart/alternative;
	boundary="=_reb-r7E3014F4-t4DDBFC90"

This is a multi-part MIME message.

--=_reb-r7E3014F4-t4DDBFC90
Content-Type: text/html;
	charset="iso-8859-1"
Content-Transfer-Encoding: quoted-printable

<!DOCTYPE HTML PUBLIC "-//W3C//DTD HTML 4.0 Transitional//EN">
<HTML xmlns:o=3D"urn:schemas-microsoft-com:office:office"><HEAD>

</HEAD>
<BODY><SPAN style=3D"mso-fareast-font-family: 'Times New Roman'; mso-fareas=
t-theme-font: minor-fareast; mso-no-proof: yes"><FONT face=3D"Calibri"><SPA=
N style=3D"mso-fareast-font-family: 'Times New Roman'; mso-fareast-theme-fo=
nt: minor-fareast; mso-no-proof: yes"><FONT face=3D"Calibri"><SPAN class=3D=
"Apple-style-span" style=3D"WIDOWS: 2; TEXT-TRANSFORM: none; TEXT-INDENT: 0=
px; BORDER-COLLAPSE: separate; FONT: medium 'Times New Roman'; WHITE-SPACE:=
 normal; ORPHANS: 2; LETTER-SPACING: normal; COLOR: rgb(0,0,0); WORD-SPACIN=
G: 0px; -webkit-border-horizontal-spacing: 0px; -webkit-border-vertical-spa=
cing: 0px; -webkit-text-decorations-in-effect: none; -webkit-text-size-adju=
st: auto; -webkit-text-stroke-width: 0px"><SPAN class=3D"Apple-style-span" =
style=3D"TEXT-ALIGN: left; FONT-FAMILY: Verdana; COLOR: rgb(125,125,125); F=
ONT-SIZE: 12px"></SPAN></SPAN></FONT></SPAN><SPAN style=3D"mso-fareast-font=
-family: 'Times New Roman'; mso-fareast-theme-font: minor-fareast; mso-no-p=
roof: yes">
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt">Dear=20
abhra,<o:p></o:p></P>
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt"><o:p>&nbsp;</o:p></P>
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt">Good Day !<o:p></o:p><=
/P>
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt"><o:p>&nbsp;</o:p></P>
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt">Hope you are doing gre=
at=20
!!!<o:p></o:p></P>
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt"><B><o:p>&nbsp;</o:p></=
B></P>
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt"><B>If you have&nbsp; a=
ny=20
consultant&nbsp; for the below requirement please inform me through phone o=
r=20
email.<o:p></o:p></B></P>
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt"><o:p>&nbsp;</o:p></P>
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt"><B>Job Position&nbsp;=
=20
:&nbsp;&nbsp;&nbsp; QA Lead</B><o:p></o:p></P>
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt">Location&nbsp;&nbsp;&n=
bsp;&nbsp;&nbsp;&nbsp;&nbsp;=20
:&nbsp;&nbsp;&nbsp;&nbsp; Philadelphia, PA<o:p></o:p></P>
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt">Duration&nbsp;&nbsp;&n=
bsp;&nbsp;&nbsp;=20
:&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; 6 Months<o:p></o:p></P>
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt">Rate&nbsp;&nbsp;&nbsp;=
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;=20
:&nbsp;&nbsp;&nbsp;&nbsp; $35/hr &nbsp;<o:p></o:p></P>
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt"><o:p>&nbsp;</o:p></P>
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt"><B>Required=20
skills<o:p></o:p></B></P>
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt"><o:p>&nbsp;</o:p></P>
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt">Quality Center, WinRun=
ner,=20
QuickTest Professional, LoadRunner, TCP/IP, XML, Unix<o:p></o:p></P>
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt"><SPAN style=3D"FONT-FA=
MILY: 'Times New Roman','serif'"><o:p>&nbsp;</o:p></SPAN></P>
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 12pt"><B>Experience &amp;=
=20
Qualifications: <BR></B><BR>Healthcare experience (preferable some experien=
ce a=20
Blues organization) <BR><BR>Knowledge of software development life cycle an=
d=20
processes for quality assurance guidelines. <BR><BR>Knowledge of quality=20
assurance methodologies. <BR><BR>Knowledge of quality philosophies, princip=
les,=20
methods, tools and standards of software development and testing (software=
=20
estimation techniques, risk assessment and management, configuration manage=
ment=20
and change control, software tracking constructs <BR><BR>Knowledge of Mercu=
ry=20
tools such as Quality Center, WinRunner, QuickTest Professional and LoadRun=
ner.=20
<BR><BR>Efficiency in the use of Microsoft Office (Word, Excel, Access, Pro=
ject,=20
Visio <BR><BR>Experience testing Client/Server and Web software.=20
<BR><BR>Knowledge of Internet protocols and technologies (TCP/IP, DNS, FTP,=
 etc.=20
<BR><BR>Knowledge of XML-based technologies, and encryption programming (SS=
L,=20
etc. <BR><BR>Knowledge of Windows and UNIX environments including AIX and /=
 or=20
HP-UX and Linux. <BR><BR>Knowledge of the disciplines of Software Engineeri=
ng=20
with emphasis on testing. <BR><BR>Knowledge of Internet and enterprise soft=
ware=20
technologies including web and application servers.<SPAN class=3D"txtempsty=
le">=20
</SPAN><SPAN style=3D"FONT-SIZE: 11pt"><o:p></o:p></SPAN></P>
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 12pt"><B>Job Summary:</B><S=
PAN class=3D"txtempstyle"> </SPAN><BR><BR>The Test Lead is responsible for =
the planning,=20
coordination, and execution of system and user acceptance testing, as well =
as=20
leadership of testing resources. The Test Lead is expected to function=20
independently in a testing project management role, as well as participate =
in=20
the execution of testing effort as time and resource levels require. This i=
s a=20
client-facing role that will require regular status reporting and excellent=
=20
communication skills. <BR><BR><B>NOTE:</B> PMP or other certifications are =
not=20
required, however the Testing Lead will be required to create "testing proj=
ect=20
plans" in Microsoft Project, Excel or other formats. This person will partn=
er=20
with the overall Project Manager, but will need to function independently a=
nd=20
provide overall testing status back to the PM.<SPAN class=3D"txtempstyle">=
=20
</SPAN><BR><BR><B>Essential Functions:</B><SPAN class=3D"txtempstyle">=20
</SPAN><BR><BR>Develop and maintain Test Strategy and Test Plans.=20
<BR><BR>Develop Testing "Project Plans" in support of the larger Project Pl=
an=20
<BR><BR>Manage daily testing activities across client testing resources and=
 3rd=20
parties, where applicable <BR><BR>Report testing status to Project Manageme=
nt=20
and Project Stakeholder resources <BR><BR>Ensure a high level of quality in=
 all=20
testing deliverables, including conducting QA reviews <BR><BR>Prepare, spec=
ify,=20
and execute test case scenarios as required <BR><BR>Great Communication Ski=
lls=20
<BR><BR>Identify and report project risks. <BR><BR>Ability to manage change=
=20
during the test process. <BR><BR>Administration of test control and statist=
ics=20
collection (i.e. problem reporting, defect tracking<SPAN class=3D"txtempsty=
le">=20
</SPAN><SPAN style=3D"FONT-SIZE: 11pt"><o:p></o:p></SPAN></P>
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt"><B><SPAN style=3D"FONT=
-SIZE: 12pt"><o:p>&nbsp;</o:p></SPAN></B></P>
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt"><B>Education and/or=20
Certifications:</B><SPAN class=3D"txtempstyle"> </SPAN><BR><BR>The ideal ca=
ndidate=20
will possess the following skills: <BR><BR>B.S. Degree, Computer Science, M=
IS or=20
related work experience.<SPAN class=3D"txtempstyle"> </SPAN><BR><BR><B>Tech=
nical=20
skills and abilities:</B><SPAN class=3D"txtempstyle"> </SPAN><BR><BR>Strong=
=20
interpersonal skills. <BR><BR>Strong written and verbal communication skill=
s.=20
<BR><BR>Ability to manage conflicting interests and identify compromises.=
=20
<BR><BR>Ability to deliver results under pressure. <BR><BR>Ability to trave=
l as=20
needed.<SPAN class=3D"txtempstyle"> </SPAN><BR><BR><B>JOB DESCRIPTION</B>=
=20
<B><BR></B><BR>-Manage testing timelines and deliverables. <BR><BR>-Coordin=
ate=20
testing activities by client and 3rd party resources <BR><BR>-Provide estim=
ation=20
of work in terms of schedule and duration <BR><BR>-Identify and report on=
=20
potential and actual risks to release cycle <BR><BR>-Provide business-facin=
g=20
status reports and updates on a regular basis <BR><BR>-Function as a team p=
layer=20
with a "can do" attitude <BR><BR>-Come up to speed very quickly/become=20
productive in a very short time <BR><BR>-Write and execute manual test scri=
pts=20
based upon existing product functionality, specifications, requirements and=
=20
other sources as needed. <BR><BR>-Record defects in accordance with QA best=
=20
practices; i.e. with full documentation and information including reproduct=
ion=20
steps <BR><BR>-The project timeline is aggressive and there may be a need f=
or=20
occasional work beyond normal business hours.<SPAN style=3D"FONT-FAMILY: 'T=
imes New Roman','serif'; FONT-SIZE: 11pt"><o:p></o:p></SPAN></P>
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt"><SPAN style=3D"FONT-SI=
ZE: 12pt"><o:p>&nbsp;</o:p></SPAN></P></SPAN>
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt"><SPAN style=3D"mso-far=
east-font-family: 'Times New Roman'; mso-fareast-theme-font: minor-fareast;=
 mso-no-proof: yes"></SPAN>&nbsp;</P>
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt"><SPAN style=3D"mso-far=
east-font-family: 'Times New Roman'; mso-fareast-theme-font: minor-fareast;=
 mso-no-proof: yes"></SPAN>&nbsp;</P><SPAN style=3D"mso-fareast-font-family=
: 'Times New Roman'; mso-fareast-theme-font: minor-fareast; mso-no-proof: y=
es"><FONT face=3D"Calibri">
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt"><SPAN style=3D"mso-far=
east-font-family: 'Times New Roman'; mso-fareast-theme-font: minor-fareast;=
 mso-no-proof: yes"><FONT face=3D"Calibri">Thanks for choosing JJCPL<o:p></=
o:p></FONT></SPAN></P>
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt"><SPAN style=3D"mso-far=
east-font-family: 'Times New Roman'; mso-fareast-theme-font: minor-fareast;=
 mso-no-proof: yes"><FONT face=3D"Calibri">Regards </FONT></SPAN></P>
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt"><B><I><SPAN style=3D"F=
ONT-FAMILY: 'Century Gothic','sans-serif'; COLOR: #215868; FONT-SIZE: 12pt;=
 mso-fareast-font-family: 'Times New Roman'; mso-fareast-theme-font: minor-=
fareast; mso-no-proof: yes">Ansha</SPAN></I></B></P>
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt"><SPAN style=3D"FONT-FA=
MILY: 'Century Gothic','sans-serif'; COLOR: #215868; FONT-SIZE: 12pt; mso-f=
areast-font-family: 'Times New Roman'; mso-fareast-theme-font: minor-fareas=
t; mso-no-proof: yes"><FONT face=3D"Calibri" color=3D"#000000">Sr.Recruiter=
</FONT></SPAN></P>
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt"><B><I><SPAN style=3D"F=
ONT-FAMILY: 'Century Gothic','sans-serif'; COLOR: #215868; FONT-SIZE: 12pt;=
 mso-fareast-font-family: 'Times New Roman'; mso-fareast-theme-font: minor-=
fareast; mso-no-proof: yes"><IMG height=3D"35" width=3D"122" src=3D"cid:ima=
ge_0000" alt=3D""></SPAN></I></B></P>
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt"><B><SPAN style=3D"FONT=
-FAMILY: 'Tahoma','sans-serif'; COLOR: #f2f2f2; FONT-SIZE: 7pt; mso-fareast=
-font-family: 'Times New Roman'; mso-fareast-theme-font: minor-fareast; mso=
-no-proof: yes"></SPAN></B><SPAN style=3D"COLOR: red; mso-fareast-font-fami=
ly: 'Times New Roman'; mso-fareast-theme-font: minor-fareast; mso-no-proof:=
 yes"><FONT face=3D"Calibri">&#8220;<I style=3D"mso-bidi-font-style: normal=
">we forecast you</I>&#8221; </P>
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt"></FONT></SPAN><SPAN st=
yle=3D"FONT-FAMILY: 'Bookman Old Style','serif'; FONT-SIZE: 9pt; mso-fareas=
t-font-family: 'Times New Roman'; mso-fareast-theme-font: minor-fareast; ms=
o-no-proof: yes"><o:p></o:p></SPAN></P>
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt"><SPAN style=3D"mso-far=
east-font-family: 'Times New Roman'; mso-fareast-theme-font: minor-fareast;=
 mso-no-proof: yes">512=20
600 0503</SPAN></P>
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt"><SPAN style=3D"mso-far=
east-font-family: 'Times New Roman'; mso-fareast-theme-font: minor-fareast;=
 mso-no-proof: yes"><A href=3D"http://www.jjcpl.net/"><SPAN style=3D"COLOR:=
 blue"><FONT face=3D"Calibri">http://www.jjcpl.net</FONT></SPAN></A><FONT f=
ace=3D"Calibri">=20
<o:p></o:p></FONT></SPAN></P>
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt"><SPAN style=3D"mso-far=
east-font-family: 'Times New Roman'; mso-fareast-theme-font: minor-fareast;=
 mso-no-proof: yes"><FONT face=3D"Calibri">Skype: jjcpl.rpo</FONT></SPAN></=
P>
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt"><SPAN style=3D"mso-far=
east-font-family: 'Times New Roman'; mso-fareast-theme-font: minor-fareast;=
 mso-no-proof: yes"><FONT face=3D"Calibri">Gtalk : jjcpl.rpo</FONT></SPAN><=
/P><SPAN style=3D"mso-fareast-font-family: 'Times New Roman'; mso-fareast-t=
heme-font: minor-fareast; mso-no-proof: yes">
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt"><B><SPAN style=3D"COLO=
R: #31849b; FONT-SIZE: 11pt"><FONT face=3D"Times New Roman">&nbsp;</FONT><F=
ONT face=3D"Times New Roman" color=3D"#0000ff"><A href=3D"http://jjcplrpo.b=
logspot.com">http://jjcplrpo.blogspot.com</A>=20
</FONT></SPAN></B></P></SPAN><SPAN style=3D"mso-fareast-font-family: 'Times=
 New Roman'; mso-fareast-theme-font: minor-fareast; mso-no-proof: yes">
<P class=3D"yiv1141086511msonormal" style=3D"MARGIN: 0in 0in 0pt"><STRONG><=
SPAN style=3D"FONT-FAMILY: 'Calibri','sans-serif'; COLOR: gray">Note: We re=
spect your=20
online privacy. This is not an unsolicited e-mail. If you are not intereste=
d in=20
receiving our e-mails then please reply with a</SPAN></STRONG><SPAN style=
=3D"FONT-FAMILY: 'Calibri','sans-serif'; COLOR: gray; FONT-SIZE: 10pt">=20
</SPAN><STRONG><SPAN style=3D"FONT-FAMILY: 'Calibri','sans-serif'; COLOR: r=
ed">"REMOVE"</SPAN></STRONG><SPAN style=3D"FONT-FAMILY: 'Calibri','sans-ser=
if'; COLOR: gray; FONT-SIZE: 10pt">=20
</SPAN><STRONG><SPAN style=3D"FONT-FAMILY: 'Calibri','sans-serif'; COLOR: g=
ray">in=20
the subject line. All removal requests will be honored ASAP. We sincerely=
=20
apologize for any inconvenience caused to you</SPAN></STRONG><SPAN style=3D=
"FONT-FAMILY: 'Calibri','sans-serif'; FONT-SIZE: 10pt"><o:p></o:p></SPAN></=
P></SPAN></FONT></SPAN></FONT></SPAN></BODY></HTML>


--=_reb-r7E3014F4-t4DDBFC90--


--=_reb-r44348D41-t4DDBFC90
Content-Type: image/jpeg; name="logo%20new%201.jpg"
Content-Transfer-Encoding: base64
Content-ID: <image_0000>
Content-Disposition: inline



--=_reb-r44348D41-t4DDBFC90--

