Return-Path: ajay.sharma@vlinkinfo.com
Received: from mx116.stngva01.us.mxservers.net (198.173.112.22)
	by mail19k.g19.rapidsite.net (RS ver 1.0.95vs) with SMTP id 2-0525098457
	for <abhra@altechstar.com>; Thu,  1 Sep 2011 16:20:57 -0400 (EDT)
Received: from unknown [76.74.252.24] (EHLO rs78.luxhv.com)
	by va1-mx116.stngva01.us.mxservers.net (mxl_mta-3.1.0-05)
	with ESMTP id 929ef5e4.1302313888.905547.00-003.va1-mx116.stngva01.us.mxservers.net (envelope-from <ajay.sharma@vlinkinfo.com>);
	Thu, 01 Sep 2011 16:20:57 -0400 (EDT)
Received: from ip-10-218-41-168.ec2.internal (ec2-184-72-246-77.compute-1.amazonaws.com [184.72.246.77])
	(authenticated bits=0)
	by rs78.luxhv.com (8.13.8/8.13.8) with ESMTP id p81KKpFm025968;
	Thu, 1 Sep 2011 15:20:51 -0500
Date: Thu, 1 Sep 2011 16:20:51 -0400 (EDT)
From: Ajay Sharma <ajay.sharma@vlinkinfo.com>
To: ajay.sharma@vlinkinfo.com
Message-ID: <1314908451218.18830105918178774.ajay.sharma@vlinkinfo.com>
Subject: Dot Net + Share Point opportunities for Phoenix, AZ. 5 gig.
MIME-Version: 1.0
Content-Type: multipart/mixed; 
	boundary="=_reb-r76020129-t4E5FE929"
Importance: Normal
Disposition-Notification-To: ajay.sharma@vlinkinfo.com
X-Processed-By: Rebuild v2.0-0
X-Spam: [F=0.2000000000; B=0.500(0); S=0.200(2010122901); MH=0.500(2011090135)]
X-MAIL-FROM: <ajay.sharma@vlinkinfo.com>
X-SOURCE-IP: [76.74.252.24]
X-SF-Loop: 1

This is a multi-part MIME message.

--=_reb-r76020129-t4E5FE929
Content-Type: text/html; charset=us-ascii
Content-Transfer-Encoding: 7bit

<div style="padding:3px" width="100%" align="right" id="removeTopLink"><a href="http://portal.zoniacswift.com//servlet/ZoniacServlet?api=getOutsideUnsubscribeContact&e66Ve62i57=true&m9UCc1ucRi=693&h2BNp44yL7=206455&h2ZNp64yM8=null&h2TNp86wP9=4" style="font: 11px verdana;"><font style="font: 11px verdana;" color="blue">Click here </font></a><span style="font: 11px verdana;">to unsubscribe.</span></div>


<style type="text/css">

		.font11 {color: black; font: 11px verdana}
		.font11bold {color: black; font: bold 11px verdana}	
		TABLE {font: 12px verdana}
	</style><br>
<table align="center" width="100%" cellpadding="10" cellspacing="0" border="1" style="border-bottom-color: #006aaf; border-top-color: #006aaf; border-collapse: collapse; border-right-color: #006aaf; border-left-color: #006aaf">
    <tbody>
        <tr>
            <td width="100%">
            <div><img src="cid:CUmOqVrRby" alt="" border="0"><br>
            <br>
            <br>
            Hi,</div>
            <div style="margin: 0in 0in 0pt"><font face="Calibri">We have an URGENT Dot Net opening with our Phoenix, AZ based client.</font></div>
            <div style="margin: 0in 0in 0pt">&nbsp;</div>
            <div style="margin: 0in 0in 0pt"><font face="Calibri">This is fast moving opportunity with long term duration. Need 5 resources.</font></div>
            <div style="margin: 0in 0in 0pt">&nbsp;</div>
            <div style="margin: 0in 0in 0pt"><font face="Calibri">Please discuss the requirement with your W2 consultant before sending their profiles.</font></div>
            <div style="margin: 0in 0in 0pt">&nbsp;</div>
            <div style="margin: 0in 0in 0pt"><span style="color: #1f497d"><font face="Calibri">Primary Technology - Microsoft</font></span></div>
            <div style="margin: 0in 0in 0pt"><span style="color: #1f497d"><font face="Calibri"><strong><br>
            <font size="3">Job Requirement &ndash;</font></strong> </font></span></div>
            <span style="color: #1f497d"><font face="Calibri">
            <p style="margin: 0in 0in 0pt"><br>
            *&nbsp; Dot net with SharePoint Experience</p>
            <p style="margin: 0in 0in 0pt"><span style="color: #1f497d"><font face="Calibri">*&nbsp; Strong .NET, SharePoint experience. Proactive, go getter. </font></span></p>
            <p style="margin: 0in 0in 0pt"><font face="Calibri"><span style="color: #1f497d">*&nbsp; Web development in .NET 2.0, 3.5, 4.0 (C#)&nbsp;<br>
            *&nbsp; Database (SQL Server)&nbsp;<br>
            *&nbsp; Windows development (C#, VB .NET)&nbsp;<br>
            *&nbsp; Web services User interface: AJAX, WPF SharePoint technology&nbsp;<br>
            *&nbsp; Legacy technologies (Visual Basic 6.0, Classic ASP)</span></font></p>
            <div style="margin: 0in 0in 0pt">&nbsp;</div>
            <div style="margin: 0in 0in 0pt"><span style="color: red; font-size: 12pt"><font face="Calibri" size="2">Production Support and Offshore team handling experience is must.</font></span></div>
            <div style="margin: 0in 0in 0pt"><font face="Calibri"><font size="2"><span style="color: red; font-size: 12pt">Very good communication &amp; client handling skills Required</span><span style="color: #1f497d; font-size: 12pt">.</span></font></font></div>
            <div style="margin: 0in 0in 0pt"><span style="color: #1f497d; font-size: 12pt"><font face="Calibri" size="2">Candidate should have Skype Account for web cam interview.</font></span></div>
            <div style="margin: 0in 0in 0pt">&nbsp;</div>
            <div style="margin: 0in 0in 0pt"><font face="Calibri">Your early response is highly appreciated.</font></div>
            <div>&nbsp;</div>
            <div style="margin: 0in 0in 0pt"><a name="_MailAutoSig"><font face="Calibri">Best Regards,</font></a></div>
            <div style="margin: 0in 0in 0pt">&nbsp;</div>
            <div style="margin: 0in 0in 0pt"><strong><span style="font-family: 'Tahoma','sans-serif'">Ajay Sharma</span></strong></div>
            <div style="margin: 0in 0in 0pt"><strong><span style="font-family: 'Tahoma','sans-serif'; color: #0033cc; font-size: 14pt">VLink Inc.</span></strong></div>
            <div style="margin: 0in 0in 0pt">&nbsp;</div>
            <div style="margin: 0in 0in 0pt"><span style="font-family: 'Tahoma','sans-serif'; color: gray; font-size: 9pt">(O) 860.247.1400 x 101 | (M) 602-999-3888 | (F) 860.256.8484 </span></div>
            <div style="margin: 0in 0in 0pt"><a href="http://www.vlinkinfo.com/"><span style="font-family: 'Tahoma','sans-serif'; color: blue; font-size: 9pt">www.vlinkinfo.com</span></a></div>
            <div style="margin: 0in 0in 0pt">&nbsp;</div>
            <div style="margin: 0in 0in 0pt"><span style="font-family: 'Tahoma','sans-serif'; color: gray; font-size: 7pt">Hartford, CT | New Delhi, India</span></div>
            <div style="margin: 0in 0in 0pt"><span><font face="Calibri">&nbsp; </font></span></div>
            <div style="margin: 0in 0in 0pt"><span style="font-family: 'Verdana','sans-serif'; color: black; font-size: 7pt">LEGAL NOTICE:</span></div>
            <div style="text-align: justify; margin: 0in 0in 0pt"><span style="font-family: 'Verdana','sans-serif'; color: black; font-size: 7pt"><br>
            Unless expressly stated otherwise, the information contained in this message is confidential, may be privileged, and protected from disclosure. If the reader of this message is not the intended recipient, or an employee or agent responsible for delivering this message to the intended recipient, you are hereby notified that any dissemination, distribution, or copying of this communication is strictly prohibited. If you have received this communication in error, please notify the sender immediately by replying to the message, permanently deleting it from your computer and/or destroy any copies/printouts of this message.</span></div>
            <div style="text-align: justify; margin: 0in 0in 0pt">&nbsp;</div>
            <div id="messagebody" style="font: 12px verdana">&nbsp;</div>
            <div id="Signature" style="font: 12px verdana">&nbsp;</div>
            </font></span></td>
        </tr>
    </tbody>
</table>
<div style="padding:3px" width="100%" align="center" id="removeLink"><a href="http://portal.zoniacswift.com//servlet/ZoniacServlet?api=getOutsideUnsubscribeContact&e66Ve62i57=true&m9UCc1ucRi=693&h2BNp44yL7=206455&h2ZNp64yM8=null&h2TNp86wP9=4" class="font10"><font color="blue">Click here </font></a><span class="font10">to unsubscribe from our mailing list.</span><br><span class="font10">VLINK INC, 635 Farmington Ave, , Hartford, Connecticut, 06105      Phone: 860.247.1400 Fax: </span></div>
<div style="display:block;" width="100%" align="center"><table width="100%" align="center"><tr><td align="right" width="55%"><a target="_new" href="http://www.zoniac.com" style="vertical-align: text-top;"><font style="vertical-align: text-top;">Powered by Zoniac</font></a></td><td align="left" width="45%"><IMG align="bottom" height="20" width="20" border="0" SRC="cid:0RcvHFAJ8Q"></td></tr></table></div><div id="removeLinkDummy1"></div><div id="removeLinkDummy2"></div><div id="removeLinkDummy3"></div><div id="removeLinkDummy4"></div><div id="removeLinkDummy5"></div><div id="removeLinkDummy6"></div>
<div style="padding:3px" width="100%" align="left" id="showDisclaimerData"></div>

--=_reb-r76020129-t4E5FE929
Content-Type: image/gif; name="/Companies/693//custom/customerLogo.gif"
Content-Transfer-Encoding: base64
Content-Disposition: inline; 
	filename="/Companies/693//custom/customerLogo.gif"
Content-ID: <CUmOqVrRby>



--=_reb-r76020129-t4E5FE929
Content-Type: image/gif; name="/custom/UnsubscribedPoweredByImg.gif"
Content-Transfer-Encoding: base64
Content-Disposition: inline; filename="/custom/UnsubscribedPoweredByImg.gif"
Content-ID: <0RcvHFAJ8Q>



--=_reb-r76020129-t4E5FE929--

