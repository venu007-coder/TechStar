Return-Path: sonali@vlinkinfo.com
Received: from mx34.stngva01.us.mxservers.net (204.202.242.42)
	by mail19d.g19.rapidsite.net (RS ver 1.0.95vs) with SMTP id 0-0169945785
	for <abhra@altechstar.com>; Tue, 12 Jul 2011 11:49:45 -0400 (EDT)
Received: from unknown [76.74.252.24] (EHLO rs78.luxhv.com)
	by va1-mx34.stngva01.us.mxservers.net (mxl_mta-3.1.0-05)
	with ESMTP id 81d6c1e4.2846649248.264230.00-009.va1-mx34.stngva01.us.mxservers.net (envelope-from <sonali@vlinkinfo.com>);
	Tue, 12 Jul 2011 11:49:44 -0400 (EDT)
Received: from ip-10-218-41-168.ec2.internal (ec2-184-72-246-77.compute-1.amazonaws.com [184.72.246.77])
	(authenticated bits=0)
	by rs78.luxhv.com (8.13.8/8.13.8) with ESMTP id p6CFndsC003162;
	Tue, 12 Jul 2011 10:49:40 -0500
Date: Tue, 12 Jul 2011 11:49:39 -0400 (EDT)
From: Sonali Jain <sonali@vlinkinfo.com>
To: sonali@vlinkinfo.com
Message-ID: <1310485779388.17643019147603012.sonali@vlinkinfo.com>
Subject: Oracle DBA [VLINK_1134]
MIME-Version: 1.0
Content-Type: multipart/mixed; 
	boundary="=_reb-r3EB92F03-t4E1C6D19"
Importance: Normal
X-Lux-Comment: Message p6CFndsC003162 sent by user #76870; report Spam, UCE, or Abuse to abuse@luxsci.us
X-Lux-Comment: LuxSci High Volume Email: http://luxhv.com
X-Processed-By: Rebuild v2.0-0
X-Spam: [F=0.2000000000; B=0.500(0); S=0.200(2010122901); MH=0.500(2011071228)]
X-MAIL-FROM: <sonali@vlinkinfo.com>
X-SOURCE-IP: [76.74.252.24]
X-SF-Loop: 1

This is a multi-part MIME message.

--=_reb-r3EB92F03-t4E1C6D19
Content-Type: text/html; charset=us-ascii
Content-Transfer-Encoding: 7bit

<div style="padding:3px" width="100%" align="right" id="removeTopLink"><a href="http://portal.zoniacswift.com//servlet/ZoniacServlet?api=getOutsideUnsubscribeContact&e66Ve62i57=true&m9UCc1ucRi=693&h2BNp44yL7=292640&h2ZNp64yM8=null&h2TNp86wP9=4" style="font: 11px verdana;"><font style="font: 11px verdana;" color="blue">Click here </font></a><span style="font: 11px verdana;">to unsubscribe.</span></div>


<style type="text/css">

		.font11 {color: black; font: 11px verdana}
		.font11bold {color: black; font: bold 11px verdana}
		.font12 {color: black; font: 12px verdana}
		.font12bold {color: black; font: bold 12px verdana}
		TABLE {font: 12px verdana}
		.font10 {color: black; font: 10px verdana}
		A:link.font10Link
		{  	color: black; font: 10px verdana;text-decoration: underline;
		}
		A:visited.font10Link
		{  	color: black; font: 10px verdana;text-decoration: underline;
		}
		A:active.font10Link
		{  	color: black; font: 10px verdana;text-decoration: underline;
		}
		A:hover.font10Link
		{	color: black; font: 10px verdana;text-decoration: underline;
		}
		A:link.font11Link
		{  	color: black; font: 11px verdana;text-decoration: underline;
		}
		A:visited.font11Link
		{  	color: black; font: 11px verdana;text-decoration: underline;
		}
		A:active.font11Link
		{  	color: black; font: 11px verdana;text-decoration: underline;
		}
		A:hover.font11Link
		{	color: black; font: 11px verdana;text-decoration: underline;
		}
		A:link.font12Link
		{  	color: black; font: 12px verdana;text-decoration: underline;
		}
		A:visited.font12Link
		{  	color: black; font: 12px verdana;text-decoration: underline;
		}
		A:active.font12Link
		{  	color: black; font: 12px verdana;text-decoration: underline;
		}
		A:hover.font12Link
		{	color: black; font: 12px verdana;text-decoration: underline;
		}
	</style><br>
<table align="center" width="100%" cellpadding="10" cellspacing="0" border="1" style="border-bottom-color: #51ae70; border-top-color: #51ae70; border-collapse: collapse; border-right-color: #51ae70; border-left-color: #51ae70">
    <tbody>
        <tr>
            <td>
            <div id="customerLogo"><img src="cid:Ohzphp1Dwm" alt="" border="0"><br>
            <br>
            <br>
            Hello Partner,<br>
            <br>
            Please send qualified resumes for the following position.&nbsp; MAX pay rate is $&nbsp; /hr.&nbsp;for this position.&nbsp; Be sure to include&nbsp;rate and availability information.<br>
            <br>
            Regards,<br>
            <br>
            Sonali<br>
            IT Resource Specialist<br>
            201-293-0891<br>
            <a href="mailto:sonali@vlinkinfo.com">sonali@vlinkinfo.com</a></div>
            <div id="messagebody">&nbsp;</div>
            <br>
            <a name="Top"></a>
            <table cellpadding="3" cellspacing="0" border="1" style="border-bottom-color: #51ae70; border-top-color: #51ae70; border-collapse: collapse; font: 12px verdana; color: black; border-right-color: #51ae70; border-left-color: #51ae70">
                <tbody>
                    <tr bgcolor="#e0f8e0">
                        <td align="left" width="50%" valign="top" style="border-bottom-color: #51ae70; border-top-color: #51ae70; font: bold 12px verdana; color: black; border-right-color: #51ae70; border-left-color: #51ae70">Name</td>
                        <td align="left" width="15%" valign="top" style="border-bottom-color: #51ae70; border-top-color: #51ae70; font: bold 12px verdana; color: black; border-right-color: #51ae70; border-left-color: #51ae70">Experience</td>
                        <td align="left" width="15" valign="top" style="border-bottom-color: #51ae70; border-top-color: #51ae70; font: bold 12px verdana; color: black; border-right-color: #51ae70; border-left-color: #51ae70">Openings</td>
                        <td align="left" width="20%" valign="top" style="border-bottom-color: #51ae70; border-top-color: #51ae70; font: bold 12px verdana; color: black; border-right-color: #51ae70; border-left-color: #51ae70">Location</td>
                    </tr>
                    <tr>
                        <td style="border-bottom-color: #51ae70; border-top-color: #51ae70; font: 12px verdana; color: black; border-right-color: #51ae70; border-left-color: #51ae70"><a href="#59541" class="font12Link"><span id="jobTitle">Oracle DBA</span> [ <span id="jobCode">VLINK_1134</span> ] </a></td>
                        <td style="border-bottom-color: #51ae70; border-top-color: #51ae70; font: 12px verdana; color: black; border-right-color: #51ae70; border-left-color: #51ae70">&nbsp;</td>
                        <td style="border-bottom-color: #51ae70; border-top-color: #51ae70; font: 12px verdana; color: black; border-right-color: #51ae70; border-left-color: #51ae70"><span id="numberOfOpenings">2</span></td>
                        <td style="border-bottom-color: #51ae70; border-top-color: #51ae70; font: 12px verdana; color: black; border-right-color: #51ae70; border-left-color: #51ae70"><span id="location">Albany, NY</span></td>
                    </tr>
                </tbody>
            </table>
            <div>&nbsp;</div>
            <div style="border-bottom-color: #51ae70; border-top-color: #51ae70; font: 11px verdana; color: black; border-right-color: #51ae70; border-left-color: #51ae70">Please <strong><a href="#Details" class="font11Link">scroll down</a></strong> or click on the requirement's name for more information.</div>
            <div>&nbsp;</div>
            <div id="Signature" style="font: 12px verdana">&nbsp;</div>
            </td>
        </tr>
    </tbody>
</table>
<br>
<a name="Details"></a>
<table align="center" width="100%" cellpadding="3" cellspacing="0" border="1" style="border-bottom-color: #51ae70; border-top-color: #51ae70; border-collapse: collapse; border-right-color: #51ae70; border-left-color: #51ae70">
    <tbody>
        <tr>
            <th bordercolordark="#ffffff" bordercolor="#ffffff" bordercolorlight="#51ae70" height="5" bgcolor="#51ae70" style="font: bold 14px verdana; color: white">Requirement List</th>
        </tr>
        <tr>
            <td width="100%">
            <div><a name="59541"></a>
            <table width="100%" cellpadding="3" cellspacing="0" border="1" style="border-bottom-color: #51ae70; border-top-color: #51ae70; border-collapse: collapse; border-right-color: #51ae70; border-left-color: #51ae70">
                <tbody>
                    <tr bgcolor="#e0f8e0">
                        <td align="right" nowrap="nowrap" width="5%" valign="top" style="border-bottom-color: #51ae70; border-top-color: #51ae70; font: bold 12px verdana; color: black; border-right-color: #51ae70; border-left-color: #51ae70">Requirement Title</td>
                        <td align="left" width="95%" valign="top" style="border-bottom-color: #51ae70; border-top-color: #51ae70; font: bold 12px verdana; color: black; border-right-color: #51ae70; border-left-color: #51ae70"><span id="jobCode">VLINK_1134</span> &nbsp; -&nbsp; <span id="jobTitle">Oracle DBA</span> ,&nbsp; <span id="location">Albany, NY</span> .&nbsp;</td>
                    </tr>
                    <tr>
                        <td align="right" nowrap="nowrap" width="5%" valign="top" style="border-bottom-color: #51ae70; border-top-color: #51ae70; font: bold 11px verdana; color: black; border-right-color: #51ae70; border-left-color: #51ae70">Description</td>
                        <td align="left" width="95%" valign="top" style="border-bottom-color: #51ae70; border-top-color: #51ae70; border-right-color: #51ae70; border-left-color: #51ae70"><span id="description">
                        <p style="margin: 0in 0in 0pt"><strong><font face="Times New Roman" size="3">Qualifications:</font></strong></p>
                        <p style="margin: 0in 0in 0pt"><font face="Times New Roman" size="3">Require Education proof&nbsp;<br>
                        &nbsp;<br>
                        Position Functions or Responsibilities:<br>
                        &nbsp;<br>
                        The&nbsp;application is a web-based application using .Net technology. It tracks juvenile justice youth through detention facilities into placement where they go through Intake, Residential, and CMSO (community multi-service organizations) programs and services, and also provides an automated financial/billing module.&nbsp;&nbsp; The Oracle Database Administrator (DBA) willprovide dedicated database support as needed for the extensive&nbsp;design and development work occurring to meet automation needs.&nbsp;<br>
                        &nbsp;<br>
                        Must have skills:<br>
                        &nbsp;<br>
                        1. Good communication and interpersonal skills. (10 points)<br>
                        <br>
                        2.Five years demonstrated experience as lead technical designer and administrator of Oracle relational databases.&nbsp; Experience must include all aspects of database instance creation, maintenance and configuration, schema design and development, tablespace/file management, tuning of database and sql, backup and recovery techniques, replication techniques, and software installation.&nbsp;&nbsp; Experience with Oracle releases 10 and 11. (30 points)<br>
                        <br>
                        3.Five years demonstrated experience working in Unix server environments, especially HP-UX.&nbsp; This should include a solid working knowledge of operating system principles, the ability to understand and work with system/server administration staff on database and application-related issues, and strong knowledge of shell-scripting techniques to support automation of database tasks. (25 points)<br>
                        <br>
                        4. Three years demonstrated experience supporting OLTP applications utilizing an Oracle database.&nbsp; Preference given to the candidate with experience in following areas:&nbsp; utilization of stored procedures to facillitate application database processes, Microsoft .NET Framework and best practices in interfacing with an Oracle database, and knowledge of application/database session pooling concepts in regard to establishing an efficient interface between application and database processes. (15 points)<br>
                        <br>
                        5.Three years demonstrated experience supporting data base applications requiring provisions for high availability and disaster recovery.&nbsp;&nbsp; Knowledge and experience preferred with HP Service Guard, Oracle Real Application Clusters and Data Guard products. (10 points)<br>
                        <br>
                        6.Three years demonstrated data processing experience working in public/government sector IT applications dealing with the delivery of government assistance or entitlements.&nbsp; Preference given to candidate supporting Children and/or Family Welfare programs. (10 points)</font></p>
                        <p style="margin: 0in 0in 0pt"><font face="Times New Roman" size="3">&nbsp;</font></p>
                        <p style="margin: 0in 0in 0pt"><strong><font face="Times New Roman" size="3">Required Qualifications:</font></strong></p>
                        <p style="margin: 0in 0in 0pt"><font face="Times New Roman" size="3">&nbsp;</font></p>
                        <p style="margin: 0in 0in 0pt"><strong><font face="Times New Roman" size="3">Other Required Qualifications:</font></strong></p>
                        <p style="margin: 0in 0in 0pt"><font face="Times New Roman" size="3">&nbsp;</font></p>
                        <p style="margin: 0in 0in 0pt"><strong><font face="Times New Roman" size="3">Additional Job Details:</font></strong></p>
                        <p style="margin: 0in 0in 0pt"><font face="Times New Roman" size="3">Require education proof</font></p>
                        </span></td>
                    </tr>
                    <tr>
                        <td align="right" nowrap="nowrap" width="5%" valign="top" style="border-bottom-color: #51ae70; border-top-color: #51ae70; font: bold 11px verdana; color: black; border-right-color: #51ae70; border-left-color: #51ae70">
                        <p>&nbsp;</p>
                        </td>
                        <td align="left" width="95%" valign="top" style="border-bottom-color: #51ae70; border-top-color: #51ae70; font: 11px verdana; color: black; border-right-color: #51ae70; border-left-color: #51ae70">&nbsp;<span style="border-bottom-color: #51ae70; border-top-color: #51ae70; font: bold 11px verdana; color: black; border-right-color: #51ae70; border-left-color: #51ae70">Number of Openings</span>&nbsp;:&nbsp; 2</td>
                    </tr>
                    <tr>
                        <td align="right" nowrap="nowrap" width="5%" valign="top" style="border-bottom-color: #51ae70; border-top-color: #51ae70; font: bold 11px verdana; color: black; border-right-color: #51ae70; border-left-color: #51ae70"><a href="#Top" class="font11Link">Go to Top</a></td>
                        <td align="left" width="95%" valign="top"><a href="mailto:sonali%40vlinkinfo.com?Subject=Applying%20for%20the%20job%20Oracle%20DBA[VLINK_1134]" class="font11Link">Respond to this requirement.</a></td>
                    </tr>
                </tbody>
            </table>
            </div>
            </td>
        </tr>
    </tbody>
</table>
<br>
<div>Best regards,<br>
Sonali</div>
<div>IT Resource Specialist<br>
<br>
VLink Inc | Phone: 201-293-0891 | Fax: 860 256 8484 | <a mce_onclick="if(window.location==top.location){Popup.composeWindow('pcompose.php?sendto=sonali%40vlinkinfo.com');}else{top.Popup.composeWindow('pcompose.php?sendto=sonali%40vlinkinfo.com');}; return false;" target="_blank" href="mailto:sonali@vlinkinfo.com">sonali@vlinkinfo.com</a> | <a mce_href="http://www.vlinkinfo.com/" target="_blank" href="http://www.vlinkinfo.com/">www.vlinkinfo.com</a><br>
<br>
LEGAL NOTICE:<br>
Unless expressly stated otherwise, the information contained in this message is confidential, may be privileged, and protected from disclosure. If the reader of this message is not the intended recipient, or an employee or agent responsible for delivering this message to the intended recipient, you are hereby notified that any dissemination, distribution, or copying of this communication is strictly prohibited. If you have received this communication in error, please notify the sender immediately by replying to the message, permanently deleting it from your computer and/or destroy any copies/printouts of this message.&nbsp;</div>
<br>
<div style="padding:3px" width="100%" align="center" id="removeLink"><a href="http://portal.zoniacswift.com//servlet/ZoniacServlet?api=getOutsideUnsubscribeContact&e66Ve62i57=true&m9UCc1ucRi=693&h2BNp44yL7=292640&h2ZNp64yM8=null&h2TNp86wP9=4" class="font10"><font color="blue">Click here </font></a><span class="font10">to unsubscribe from our mailing list.</span><br><span class="font10">VLINK INC, 635 Farmington Ave, , Hartford, Connecticut, 06105      Phone: 860.247.1400 Fax: </span></div>
<div style="display:block;" width="100%" align="center"><table width="100%" align="center"><tr><td align="right" width="55%"><a target="_new" href="http://www.zoniac.com" style="vertical-align: text-top;"><font style="vertical-align: text-top;">Powered by Zoniac</font></a></td><td align="left" width="45%"><IMG align="bottom" height="20" width="20" border="0" SRC="cid:SbQq61eYbE"></td></tr></table></div><div id="removeLinkDummy1"></div><div id="removeLinkDummy2"></div><div id="removeLinkDummy3"></div><div id="removeLinkDummy4"></div><div id="removeLinkDummy5"></div><div id="removeLinkDummy6"></div>
<div style="padding:3px" width="100%" align="left" id="showDisclaimerData"></div>

--=_reb-r3EB92F03-t4E1C6D19
Content-Type: image/gif; name="/Companies/693///custom/customerLogo.gif"
Content-Transfer-Encoding: base64
Content-Disposition: inline; 
	filename="/Companies/693///custom/customerLogo.gif"
Content-ID: <Ohzphp1Dwm>



--=_reb-r3EB92F03-t4E1C6D19
Content-Type: image/gif; name="/custom/UnsubscribedPoweredByImg.gif"
Content-Transfer-Encoding: base64
Content-Disposition: inline; filename="/custom/UnsubscribedPoweredByImg.gif"
Content-ID: <SbQq61eYbE>



--=_reb-r3EB92F03-t4E1C6D19--

