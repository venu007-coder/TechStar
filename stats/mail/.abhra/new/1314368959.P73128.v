Return-Path: veerendra@xperttech.com
Received: from mx13.stngva01.us.mxservers.net (204.202.242.69)
	by mail19d.g19.rapidsite.net (RS ver 1.0.95vs) with SMTP id 2-0215428957
	for <abhra@altechstar.com>; Fri, 26 Aug 2011 10:29:17 -0400 (EDT)
Received: from unknown [76.74.252.24] (EHLO rs78.luxhv.com)
	by va1-mx13.stngva01.us.mxservers.net (mxl_mta-3.1.0-05)
	with ESMTP id dbda75e4.2816736160.2001921.00-002.va1-mx13.stngva01.us.mxservers.net (envelope-from <veerendra@xperttech.com>);
	Fri, 26 Aug 2011 10:29:17 -0400 (EDT)
Received: from ip-10-218-41-168.ec2.internal (ec2-184-72-246-77.compute-1.amazonaws.com [184.72.246.77])
	(authenticated bits=0)
	by rs78.luxhv.com (8.13.8/8.13.8) with ESMTP id p7QETCCs028504;
	Fri, 26 Aug 2011 09:29:12 -0500
Date: Fri, 26 Aug 2011 10:29:11 -0400 (EDT)
From: Veerendra P <veerendra@xperttech.com>
To: veerendra@xperttech.com
Message-ID: <1314368951743.4900089497295075.veerendra@xperttech.com>
Subject: J2EE Architect - Northern California  | SURE SHOT
MIME-Version: 1.0
Content-Type: multipart/mixed; 
	boundary="=_reb-r0E0CF7AA-t4E57ADBD"
Importance: Normal
X-Processed-By: Rebuild v2.0-0
X-Spam: [F=0.2000000000; B=0.500(0); S=0.200(2010122901); MH=0.500(2011082617)]
X-MAIL-FROM: <veerendra@xperttech.com>
X-SOURCE-IP: [76.74.252.24]
X-SF-Loop: 1

This is a multi-part MIME message.

--=_reb-r0E0CF7AA-t4E57ADBD
Content-Type: text/html; charset=us-ascii
Content-Transfer-Encoding: 7bit

<div style="padding:3px" width="100%" align="right" id="removeTopLink"><a href="http://portal.zoniacswift.com//servlet/ZoniacServlet?api=getOutsideUnsubscribeContact&e66Ve62i57=true&m9UCc1ucRi=540&h2BNp44yL7=229142&h2ZNp64yM8=null&h2TNp86wP9=4" style="font: 11px verdana;"><font style="font: 11px verdana;" color="blue">Click here </font></a><span style="font: 11px verdana;">to unsubscribe.</span></div>


<style type="text/css">

		.font11 {color: black; font: 11px verdana}
		.font11bold {color: black; font: bold 11px verdana}	
		TABLE {font: 12px verdana}
	</style><br>
<table align="center" width="100%" cellpadding="10" cellspacing="0" border="1" style="border-bottom-color: #006aaf; border-top-color: #006aaf; border-collapse: collapse; border-right-color: #006aaf; border-left-color: #006aaf">
    <tbody>
        <tr>
            <td width="100%">
            <div id="customerLogo"><img src="cid:jafMMRxTNf" alt="" border="0"></div>
            <div id="messagebody" style="font: 12px verdana">&nbsp;</div>
            <div id="Signature" style="font: 12px verdana">
            <div style="margin: 0in 0in 0pt"><strong><span style="font-family: Calibri; color: navy; font-size: 16pt"><br>
            J2EE Architect</span></strong></div>
            <div style="margin: 0in 0in 0pt"><strong><span style="font-family: Calibri; color: navy; font-size: 11pt">Northern California</span></strong></div>
            <div style="margin: 0in 0in 0pt"><strong><span style="font-family: Calibri; color: navy; font-size: 11pt">6+ Months</span></strong></div>
            <div style="margin: 0in 0in 0pt">&nbsp;</div>
            <div style="margin: 0in 0in 0pt"><span style="font-family: Calibri; color: black; font-size: 11pt">We have a HOT HOT HOT need for a J2EE Architect with Spring and Hibernate to interview today and start on Monday in Northern California.&nbsp; </span></div>
            <div style="margin: 0in 0in 0pt"><span style="font-family: Calibri; color: black; font-size: 11pt">Need to have good experience working with the client for requirements gathering, etc. </span></div>
            <div style="margin: 0in 0in 0pt">&nbsp;</div>
            <div style="margin: 0in 0in 0pt">&nbsp;</div>
            <div style="margin: 0in 0in 0pt"><span style="font-family: Calibri; font-size: 11pt">Please reply me with your updated resume and your full contact details if you are interested and available. Feel free to call me @ 781-422-8844 for any queries</span></div>
            <div style="margin: 0in 0in 0pt">&nbsp;</div>
            <div style="margin: 0in 0in 0pt">&nbsp;</div>
            <div style="margin: 0in 0in 0pt">&nbsp;</div>
            <div style="margin: 0in 0in 0pt"><strong><span style="font-family: Calibri; color: #0d0d0d"><font size="3">Thanks &amp; Regards,</font></span></strong></div>
            <div style="margin: 0in 0in 0pt"><strong><font size="3">&nbsp;</font></strong></div>
            <div style="margin: 0in 0in 0pt"><strong><span style="font-family: Calibri; color: #0d0d0d"><font size="3">Veerendra</font></span></strong></div>
            <div style="margin: 0in 0in 0pt"><span style="font-family: Calibri; color: #0d0d0d; font-size: 10pt"><img src="cid:LVo7oDkKdS" alt="" height="44" width="137" border="0"><br>
            400 W Cummings Park || Suite#2850 || Woburn, MA-01801 </span></div>
            <div style="margin: 0in 0in 0pt"><span style="font-family: Calibri; color: #0d0d0d; font-size: 10pt">Email: </span><a href="mailto:veerendra@xperttech.com"><span style="font-family: Calibri; color: #0d0d0d; font-size: 10pt">veerendra@xperttech.com</span></a><span style="font-family: Calibri; color: #0d0d0d; font-size: 10pt">&nbsp;|| Phone: 781-422-8844</span></div>
            <div style="margin: 0in 0in 0pt"><span style="font-family: Calibri; color: #0d0d0d; font-size: 10pt">Fax: 978-405-5040 ||e-Fax: 781-207-0709|| </span><a href="http://www.xperttech.com/"><span style="font-family: Calibri; color: #0d0d0d; font-size: 10pt">www.xperttech.com</span></a></div>
            </div>
            </td>
        </tr>
    </tbody>
</table>
<div style="padding:3px" width="100%" align="center" id="removeLink"><a href="http://portal.zoniacswift.com//servlet/ZoniacServlet?api=getOutsideUnsubscribeContact&e66Ve62i57=true&m9UCc1ucRi=540&h2BNp44yL7=229142&h2ZNp64yM8=null&h2TNp86wP9=4" class="font10"><font color="blue">Click here </font></a><span class="font10">to unsubscribe from our mailing list.</span><br><span class="font10">XPERTTECH  Inc, 400 W Cummings Park, Suite #2850, Woburn, Massachusetts, 01801      Phone: 781 935 3775 Fax: 978-405-5040</span></div>
<div style="display:block;" width="100%" align="center"><table width="100%" align="center"><tr><td align="right" width="55%"><a target="_new" href="http://www.zoniac.com" style="vertical-align: text-top;"><font style="vertical-align: text-top;">Powered by Zoniac</font></a></td><td align="left" width="45%"><IMG align="bottom" height="20" width="20" border="0" SRC="cid:o488ZHMhwD"></td></tr></table></div><div id="removeLinkDummy1"></div><div id="removeLinkDummy2"></div><div id="removeLinkDummy3"></div><div id="removeLinkDummy4"></div><div id="removeLinkDummy5"></div><div id="removeLinkDummy6"></div>
<div style="padding:3px" width="100%" align="left" id="showDisclaimerData"></div>

--=_reb-r0E0CF7AA-t4E57ADBD
Content-Type: image/gif; name="/Companies/540//custom/customerLogo.gif"
Content-Transfer-Encoding: base64
Content-Disposition: inline; 
	filename="/Companies/540//custom/customerLogo.gif"
Content-ID: <jafMMRxTNf>



--=_reb-r0E0CF7AA-t4E57ADBD
Content-Type: image/gif; name="/Companies/540//custom/customerLogo.gif"
Content-Transfer-Encoding: base64
Content-Disposition: inline; 
	filename="/Companies/540//custom/customerLogo.gif"
Content-ID: <LVo7oDkKdS>



--=_reb-r0E0CF7AA-t4E57ADBD
Content-Type: image/gif; name="/custom/UnsubscribedPoweredByImg.gif"
Content-Transfer-Encoding: base64
Content-Disposition: inline; filename="/custom/UnsubscribedPoweredByImg.gif"
Content-ID: <o488ZHMhwD>



--=_reb-r0E0CF7AA-t4E57ADBD--

