Return-Path: sameer@xperttech.com
Received: from mx105.stngva01.us.mxservers.net (198.173.112.42)
	by mail19k.g19.rapidsite.net (RS ver 1.0.95vs) with SMTP id 1-0586813151
	for <abhra@altechstar.com>; Fri, 11 Mar 2011 12:05:51 -0500 (EST)
Received: from unknown [76.74.252.24] (EHLO rs78.luxhv.com)
	by va1-mx105.stngva01.us.mxservers.net (mxl_mta-3.1.0-05)
	with ESMTP id f665a7d4.1987038112.274715.00-013.va1-mx105.stngva01.us.mxservers.net (envelope-from <sameer@xperttech.com>);
	Fri, 11 Mar 2011 12:05:51 -0500 (EST)
Received: from ip-10-196-81-219.ec2.internal (ec2-184-72-246-77.compute-1.amazonaws.com [184.72.246.77])
	(authenticated bits=0)
	by rs78.luxhv.com (8.13.8/8.13.8) with ESMTP id p2BH5UeL017416;
	Fri, 11 Mar 2011 11:05:30 -0600
Date: Fri, 11 Mar 2011 12:05:29 -0500 (GMT-05:00)
From: Sameer Maharana <sameer@xperttech.com>
To: sameer@xperttech.com
Message-ID: <1299863129456.04357348819105078.sameer@xperttech.com>
Subject: .Net Developer (LOCALS Preferred) // Minneapolis, MN // 6+ months
 contract
MIME-Version: 1.0
Content-Type: multipart/mixed; 
	boundary="=_reb-r0D4E0192-t4D7A566F"
Importance: High
X-Lux-Comment: Message p2BH5UeL017416 sent by user #68993; report Spam, UCE, or Abuse to abuse@luxsci.us
X-Lux-Comment: LuxSci High Volume Email: http://luxhv.com
X-Processed-By: Rebuild v2.0-0
X-Spam: [F=0.2000000000; B=0.500(0); S=0.200(2010122901); MH=0.500(2011031124)]
X-MAIL-FROM: <sameer@xperttech.com>
X-SOURCE-IP: [76.74.252.24]
X-SF-Loop: 1

This is a multi-part MIME message.

--=_reb-r0D4E0192-t4D7A566F
Content-Type: text/html; charset=us-ascii
Content-Transfer-Encoding: 7bit

<div style="padding:3px" width="100%" align="right" id="removeTopLink"><a href="http://portal.zoniacswift.com//servlet/ZoniacServlet?api=getOutsideUnsubscribeContact&e66Ve62i57=true&m9UCc1ucRi=540&h2BNp44yL7=108734&h5p8pKsyMi8l0=0&h2ZNp64yM8=null&h2TNp86wP9=5&h2GQy54sI2=1746" style="font: 11px verdana;"><font style="font: 11px verdana;" color="blue">Click here </font></a><span style="font: 11px verdana;">to unsubscribe.</span></div>


<style type="text/css">

		.font11 {color: black; font: 11px verdana}
		.font11bold {color: black; font: bold 11px verdana}	
		TABLE {font: 12px verdana}
	</style><br>
<table border="1" align="center" width="100%" cellpadding="10" cellspacing="0" style="border-left-color: #006aaf; border-bottom-color: #006aaf; border-top-color: #006aaf; border-collapse: collapse; border-right-color: #006aaf">
    <tbody>
        <tr>
            <td width="100%">
            <div id="customerLogo"><img alt="" border="0" src="cid:Vy88eBBsAU"></div>
            <div style="font: 12px verdana" id="messagebody">&nbsp;</div>
            <div style="font: 12px verdana" id="Signature">
            <div style="margin: 0in 0in 0pt"><span style="font-family: Candara"><font size="3">Hi, </font></span></div>
            <div style="margin: 0in 0in 0pt"><font size="3">&nbsp;</font></div>
            <div style="margin: 0in 0in 0pt"><span style="font-family: Candara"><font size="3">I have an immediate requirement opening for a <span style="background: yellow">.Net Developer <strong>(LOCALS Preferred)</strong> position in Minneapolis, MN for 6+ month&rsquo;s contract</span>. I would appreciate if you could send your updated word format resume along with best contact details ASAP.</font></span></div>
            <div style="margin: 0in 0in 0pt"><font size="3">&nbsp;</font></div>
            <div style="margin: 0in 0in 0pt"><span style="font-family: Candara"><font size="3">Here are the requirement details:</font></span></div>
            <div style="margin: 0in 0in 0pt"><font size="3">&nbsp;</font></div>
            <div style="margin: 0in 0in 0pt"><font size="3"><strong><span style="color: blue; font-family: Candara">Position&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; : .Net Developer (</span></strong><strong><span style="color: red; font-family: Candara">LOCALS Preferred</span></strong><strong><span style="color: blue; font-family: Candara">)</span></strong></font></div>
            <div style="margin: 0in 0in 0pt"><strong><span style="color: blue; font-family: Candara"><font size="3">Location&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; : Minneapolis, MN</font></span></strong></div>
            <div style="margin: 0in 0in 0pt"><strong><span style="color: blue; font-family: Candara"><font size="3">Duration&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; : 6+ months</font></span></strong></div>
            <div style="margin: 0in 0in 0pt"><font size="3">&nbsp;</font></div>
            <div style="margin: 0in 0in 0pt"><strong><u><span style="font-family: Candara"><font size="3">DESCRIPTION:</font></span></u></strong></div>
            <div style="margin: 0in 0in 0pt"><span style="font-family: Candara"><font size="3">The contractor will be part of an 8 developer agile team working on C# web application serving 200K active users.</font></span></div>
            <div style="margin: 0in 0in 0pt"><font size="3">&nbsp;</font></div>
            <div style="margin: 0in 0in 0pt"><strong><u><span style="font-family: Candara"><font size="3">SKILLS REQUIRED:</font></span></u></strong></div>
            <ul type="disc" style="margin-top: 0in">
                <li style="margin: 0in 0in 0pt"><span style="font-family: Candara"><font size="3">C# (.NET 4.0)</font></span></li>
                <li style="margin: 0in 0in 0pt"><span style="font-family: Candara"><font size="3">MVC 3.0</font></span></li>
                <li style="margin: 0in 0in 0pt"><span style="font-family: Candara"><font size="3">IOC Controllers</font></span></li>
                <li style="margin: 0in 0in 0pt"><span style="font-family: Candara"><font size="3">JavaScript</font></span></li>
                <li style="margin: 0in 0in 0pt"><span style="font-family: Candara"><font size="3">AJAX</font></span></li>
                <li style="margin: 0in 0in 0pt"><span style="font-family: Candara"><font size="3">JQuery</font></span></li>
                <li style="margin: 0in 0in 0pt"><span style="font-family: Candara"><font size="3">SQL Server</font></span></li>
                <li style="margin: 0in 0in 0pt"><span style="font-family: Candara"><font size="3">MVP</font></span></li>
            </ul>
            <div style="margin: 0in 0in 0pt"><strong><u><span style="font-family: Candara"><font size="3">NICE TO HAVE:</font></span></u></strong></div>
            <ul type="disc" style="margin-top: 0in">
                <li style="margin: 0in 0in 0pt; color: red"><span style="font-family: Candara"><font size="3">Unity</font></span></li>
                <li style="margin: 0in 0in 0pt; color: red"><span style="font-family: Candara"><font size="3">nHibernate </font></span></li>
                <li style="margin: 0in 0in 0pt; color: red"><span style="font-family: Candara"><font size="3">Linq2SQL </font></span></li>
                <li style="margin: 0in 0in 0pt; color: red"><span style="font-family: Candara"><font size="3">MS Entity Framework</font></span></li>
                <li style="margin: 0in 0in 0pt; color: red"><span style="font-family: Candara"><font size="3">Agile development</font></span></li>
                <li style="margin: 0in 0in 0pt; color: red"><span style="font-family: Candara"><font size="3">TDD (Test driven Development)</font></span></li>
            </ul>
            <div style="margin: 0in 0in 0pt"><font size="3">&nbsp;</font></div>
            <div style="margin: 0in 0in 0pt"><a name="_MailAutoSig"><span style="font-size: 10pt; color: black; font-family: Cambria"><font size="3">Thanks/Regards,</font></span></a></div>
            <div style="margin: 0in 0in 0pt"><strong><span style="font-size: 16pt; color: green; font-family: 'Trebuchet MS'"><font size="3">SAMEER</font></span><span style="color: navy; font-family: Cambria"><font size="3">,</font></span><span style="color: navy; font-family: Cambria"><br>
            </span></strong></div>
            <div style="margin: 0in 0in 0pt"><span style="font-size: 10pt; color: black; font-family: Cambria"><font size="3">400 W Cummings Park,</font></span></div>
            <div style="margin: 0in 0in 0pt"><span style="font-size: 10pt; color: black; font-family: Cambria"><font size="3">Suite #<span style="font-size: 10pt; color: black; font-family: Cambria"> 2850</span></font></span></div>
            <div style="margin: 0in 0in 0pt"><span style="font-size: 10pt; color: black; font-family: Cambria"><font size="3">Woburn, MA-01801</font></span></div>
            <div style="margin: 0in 0in 0pt"><font size="3"><span style="font-size: 10pt; color: black; font-family: Cambria">Email:</span><u><span style="font-size: 10pt; color: blue; font-family: Cambria">sameer@xperttech.com</span></u></font></div>
            <div style="margin: 0in 0in 0pt"><span style="font-size: 10pt; color: black; font-family: Cambria"><font size="3">Ph# 781-399-4354 | Fax # 781 207 0709 | </font></span><a href="http://www.xperttech.com/" title="blocked::http://www.xperttech.com/"><span style="font-size: 10pt; font-family: Cambria"><font size="3">www.xperttech.com</font></span></a></div>
            <div style="margin: 0in 0in 0pt"><strong><font size="3">&nbsp;</font></strong></div>
            <div style="margin: 0in 0in 0pt"><strong><span style="font-size: 8pt; color: #1f497d; font-family: Arial"><font size="3">IMPORTANT NOTE</font></span><font size="3"><span style="font-size: 8pt; color: #1f497d; font-family: Arial">: If you have received this email in error or prefer not to receive such emails in future, please reply with &quot;&quot;</span><span style="font-size: 8pt; color: red; font-family: Arial">Remove</span><span style="font-size: 8pt; color: #1f497d; font-family: Arial">&quot;&quot; in the subject line and the email-id(s) to be removed. All removal requests will be honored ASAP.</span></font></strong></div>
            <div style="margin: 0in 0in 0pt"><font size="3"><span style="font-size: 8pt; color: green; font-family: Webdings">P </span><font face="Calibri"><span style="font-size: 8pt; color: green">Please consider the environment - do you really need to print this email?</span></font></font></div>
            <div style="margin: 0in 0in 0pt"><font size="3">&nbsp;</font></div>
            <div style="margin: 0in 0in 0pt"><font size="3">&nbsp;</font></div>
            </div>
            </td>
        </tr>
    </tbody>
</table>
<div style="padding:3px" width="100%" align="center" id="removeLink"><a href="http://portal.zoniacswift.com//servlet/ZoniacServlet?api=getOutsideUnsubscribeContact&e66Ve62i57=true&m9UCc1ucRi=540&h2BNp44yL7=108734&h5p8pKsyMi8l0=0&h2ZNp64yM8=null&h2TNp86wP9=5&h2GQy54sI2=1746" class="font10"><font color="blue">Click here </font></a><span class="font10">to unsubscribe from our mailing list.</span><br><span class="font10">XPERTTECH  Inc, 400 W Cummings Park, Suite #2850, Woburn, Massachusetts, 01801      Phone: 781 935 3775 Fax: 978-405-5040</span></div>
<div style="display:block;" width="100%" align="center"><table width="100%" align="center"><tr><td align="right" width="55%"><a target="_new" href="http://www.zoniac.com" style="vertical-align: text-top;"><font style="vertical-align: text-top;">Powered by Zoniac</font></a></td><td align="left" width="45%"><IMG align="bottom" height="20" width="20" border="0" SRC="cid:HLDFl1lDrY"></td></tr></table></div><div id="removeLinkDummy1"></div><div id="removeLinkDummy2"></div><div id="removeLinkDummy3"></div><div id="removeLinkDummy4"></div><div id="removeLinkDummy5"></div><div id="removeLinkDummy6"></div>

--=_reb-r0D4E0192-t4D7A566F
Content-Type: image/gif; name="/Companies/540/custom/customerLogo.gif"
Content-Transfer-Encoding: base64
Content-Disposition: inline; 
	filename="/Companies/540/custom/customerLogo.gif"
Content-ID: <Vy88eBBsAU>



--=_reb-r0D4E0192-t4D7A566F
Content-Type: image/gif; name="/custom/UnsubscribedPoweredByImg.gif"
Content-Transfer-Encoding: base64
Content-Disposition: inline; filename="/custom/UnsubscribedPoweredByImg.gif"
Content-ID: <HLDFl1lDrY>



--=_reb-r0D4E0192-t4D7A566F--

