Return-Path: andy@vlinkinfo.com
Received: from mx42.stngva01.us.mxservers.net (204.202.242.44)
	by mail19d.g19.rapidsite.net (RS ver 1.0.95vs) with SMTP id 0-0230294245
	for <abhra@altechstar.com>; Fri, 27 May 2011 09:50:45 -0400 (EDT)
Received: from unknown [76.74.252.24] (EHLO rs78.luxhv.com)
	by va1-mx42.stngva01.us.mxservers.net (mxl_mta-3.1.0-05)
	with ESMTP id 53cafdd4.72993696.1110311.00-012.va1-mx42.stngva01.us.mxservers.net (envelope-from <andy@vlinkinfo.com>);
	Fri, 27 May 2011 09:50:45 -0400 (EDT)
Received: from ip-10-218-41-168.ec2.internal (ec2-184-72-246-77.compute-1.amazonaws.com [184.72.246.77])
	(authenticated bits=0)
	by rs78.luxhv.com (8.13.8/8.13.8) with ESMTP id p4RDoeZW001020;
	Fri, 27 May 2011 08:50:40 -0500
Date: Fri, 27 May 2011 09:50:39 -0400 (EDT)
From: Andy Duggal <andy@vlinkinfo.com>
To: andy@vlinkinfo.com
Message-ID: <1306504239712.6157343505093886.andy@vlinkinfo.com>
Subject: Essbase developer needed 12 months plus contract
MIME-Version: 1.0
Content-Type: multipart/mixed; 
	boundary="=_reb-r1D666D71-t4DDFAC35"
Importance: Normal
X-Lux-Comment: Message p4RDoeZW001020 sent by user #76870; report Spam, UCE, or Abuse to abuse@luxsci.us
X-Lux-Comment: LuxSci High Volume Email: http://luxhv.com
X-Processed-By: Rebuild v2.0-0
X-Spam: [F=0.2000000000; B=0.500(0); S=0.200(2010122901); MH=0.500(2011052714)]
X-MAIL-FROM: <andy@vlinkinfo.com>
X-SOURCE-IP: [76.74.252.24]
X-SF-Loop: 1

This is a multi-part MIME message.

--=_reb-r1D666D71-t4DDFAC35
Content-Type: text/html; charset=us-ascii
Content-Transfer-Encoding: 7bit

<div style="padding:3px" width="100%" align="right" id="removeTopLink"><a href="http://portal.zoniacswift.com//servlet/ZoniacServlet?api=getOutsideUnsubscribeContact&e66Ve62i57=true&m9UCc1ucRi=693&h2BNp44yL7=187991&h2ZNp64yM8=null&h2TNp86wP9=4" style="font: 11px verdana;"><font style="font: 11px verdana;" color="blue">Click here </font></a><span style="font: 11px verdana;">to unsubscribe.</span></div>


<style type="text/css">

		.font11 {color: black; font: 11px verdana}
		.font11bold {color: black; font: bold 11px verdana}	
		TABLE {font: 12px verdana}
	</style><br>
<table align="center" width="100%" cellpadding="10" cellspacing="0" border="1" style="border-bottom-color: #006aaf; border-top-color: #006aaf; border-collapse: collapse; border-right-color: #006aaf; border-left-color: #006aaf">
    <tbody>
        <tr>
            <td width="100%">
            <div id="customerLogo"><img src="cid:X4MwvXtS8A" alt="" border="0"><br>
            We are looking for a Essbase developer for our client in Stamford, CT. 12 months plus contract, rate: $52/hr Corp to Corp Max. Immediate interviews<br>
            <br>
            <div style="margin: 0in 0in 0pt"><span style="color: #1f497d"><font face="Calibri">Responsible for the design, development and implementation of ESSBASE cubes and Financial Reporting.</font></span></div>
            <div style="margin: 0in 0in 0pt"><span style="color: #1f497d"><font face="Calibri">&nbsp;</font></span></div>
            <div style="margin: 0in 0in 0pt"><font face="Calibri"><strong><span style="color: #1f497d">Experience:</span></strong><span style="color: #1f497d">&nbsp;&nbsp;&nbsp; 5+ years of current Hyperion ESSBASE Database Development experience.</span></font></div>
            <div style="margin: 0in 0in 0pt 0.5in"><span style="color: #1f497d"><font face="Calibri">&nbsp;</font></span></div>
            <div style="margin: 0in 0in 0pt 0.5in"><span style="color: #1f497d"><font face="Calibri">Extensive developing and designing with Hyperion ESSBASE/Planning&nbsp;</font></span></div>
            <div style="margin: 0in 0in 0pt 0.5in"><span style="color: #1f497d"><font face="Calibri">&nbsp;</font></span></div>
            <div style="margin: 0in 0in 0pt 0.5in"><span style="color: #1f497d"><font face="Calibri">Experience with all aspects of Hyperion ESSBASE Database development and maintenance including Calculation Scripts (write / maintain / enhance), Load Rules, Report Scripts, Administration Services, Database tuning and optimization, Configuration Settings, Dense / Sparse Outline Configuration, ESSCMD, MaxL and MDX scripting, Batch Scripting, Automation, Back-up / Recovery, System Monitoring, Security Filters, ESSBASE Functions, importing data from multiple disparate systems (SQL based and Flat Files), etc. Experience with Financial Reporting Studio.</font></span></div>
            <div style="margin: 0in 0in 0pt"><span style="color: #1f497d"><font face="Calibri">&nbsp;</font></span></div>
            <div style="margin: 0in 0in 0pt"><font face="Calibri"><strong><span style="color: #1f497d">Skill Sets</span></strong><span style="color: #1f497d">:</span></font></div>
            <div style="text-indent: -0.25in; margin: 0in 0in 0pt 0.75in"><span style="font-family: Symbol; color: #1f497d">&middot;</span><span style="font-family: 'Times New Roman','serif'; color: #1f497d; font-size: 7pt">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; </span><span style="color: #1f497d"><font face="Calibri">2+ years of creating SQL based queries against Oracle and SQL Server Databases</font></span></div>
            <div style="text-indent: -0.25in; margin: 0in 0in 0pt 0.75in"><span style="font-family: Symbol; color: #1f497d">&middot;</span><span style="font-family: 'Times New Roman','serif'; color: #1f497d; font-size: 7pt">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; </span><span style="color: #1f497d"><font face="Calibri">Full working knowledge of Microsoft Excel, ESSBASE Excel Add-in, and SmartView </font></span></div>
            <div style="text-indent: -0.25in; margin: 0in 0in 0pt 0.75in"><span style="font-family: Symbol; color: #1f497d">&middot;</span><span style="font-family: 'Times New Roman','serif'; color: #1f497d; font-size: 7pt">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; </span><span style="color: #1f497d"><font face="Calibri">Hyperion Shared Services 9+ (ability to provision security)</font></span></div>
            <div style="text-indent: -0.25in; margin: 0in 0in 0pt 0.75in"><span style="font-family: Symbol; color: #1f497d">&middot;</span><span style="font-family: 'Times New Roman','serif'; color: #1f497d; font-size: 7pt">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; </span><span style="color: #1f497d"><font face="Calibri">Hyperion Reports (Financial Reporting) 9+ (ability to create and maintain reports forms, create applications, and maintain security)</font></span></div>
            <div style="text-indent: -0.25in; margin: 0in 0in 0pt 0.75in"><span style="font-family: Symbol; color: #1f497d">&middot;</span><span style="font-family: 'Times New Roman','serif'; color: #1f497d; font-size: 7pt">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; </span><span style="color: #1f497d"><font face="Calibri">Experience with writing Microsoft Excel VBA and the ESSBASE Excel Spreadsheet Tool-Kit / API</font></span></div>
            <div style="text-indent: -0.25in; margin: 0in 0in 0pt 0.75in"><span style="font-family: Symbol; color: #1f497d">&middot;</span><span style="font-family: 'Times New Roman','serif'; color: #1f497d; font-size: 7pt">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; </span><span style="color: #1f497d"><font face="Calibri">Very strong analytic / problem solving skills are essential</font></span></div>
            <div style="text-indent: -0.25in; margin: 0in 0in 0pt 0.75in"><span style="font-family: Symbol; color: #1f497d">&middot;</span><span style="font-family: 'Times New Roman','serif'; color: #1f497d; font-size: 7pt">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; </span><span style="color: #1f497d"><font face="Calibri">AUTOSYS Job Scheduler</font></span></div>
            <div style="text-indent: -0.25in; margin: 0in 0in 0pt 0.75in"><span style="font-family: Symbol; color: #1f497d">&middot;</span><span style="font-family: 'Times New Roman','serif'; color: #1f497d; font-size: 7pt">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; </span><span style="color: #1f497d"><font face="Calibri">Excellent written and verbal skills</font></span></div>
            <div style="margin: 0in 0in 0pt"><span style="color: #1f497d">&nbsp;</span></div>
            </div>
            <div id="messagebody" style="font: 12px verdana">&nbsp;</div>
            <div id="Signature" style="font: 12px verdana">
            <div style="margin: 0in 0in 0pt"><a name="_MailAutoSig" class="FCK__AnchorC"><span style="font-family: 'Times New Roman','serif'; font-size: 12pt">Best regards,</span></a></div>
            <div style="margin: 0in 0in 0pt"><span style="font-family: 'Times New Roman','serif'; font-size: 12pt">Andy Duggal</span></div>
            <div style="margin: 0in 0in 0pt"><span style="font-family: 'Times New Roman','serif'; font-size: 12pt">Senior Resource Manager</span></div>
            <div style="margin: 0in 0in 0pt">&nbsp;</div>
            <div style="margin: 0in 0in 0pt"><span style="font-family: 'Times New Roman','serif'; font-size: 12pt">Vlink Inc | Phone: 860 247 1400 ext 103&nbsp;|&nbsp;Cell: 201 699 9919&nbsp;| Fax: 860 256 8484 | </span><a href="mailto:andy@vlinkinfo.com" title="blocked::mailto:anish@vlinkinfo.com"><span style="font-family: 'Times New Roman','serif'; color: blue; font-size: 12pt">andy@vlinkinfo.com</span></a><span style="font-family: 'Times New Roman','serif'; font-size: 12pt"> | </span><a href="http://www.vlinkinfo.com/" title="blocked::http://www.vlinkinfo.com/"><span style="font-family: 'Times New Roman','serif'; color: blue; font-size: 12pt">www.vlinkinfo.com</span></a></div>
            <div style="margin: 0in 0in 0pt">&nbsp;<span style="font-family: 'Arial','sans-serif'; font-size: 10pt">&nbsp;&nbsp;</span><a href="http://www.linkedin.com/in/andyduggal" title="View public profile"><span><span style="font-family: 'Arial','sans-serif'; color: blue; font-size: 10pt">http://www.linkedin.com/in/andyduggal</span></span></a></div>
            <div style="margin: 0in 0in 0pt">&nbsp;</div>
            <div style="margin: 0in 0in 0pt">&nbsp;</div>
            <div style="margin: 0in 0in 0pt"><span style="font-family: 'Arial','sans-serif'; font-size: 10pt">&nbsp;&nbsp;&nbsp;&nbsp;</span>&nbsp;</div>
            <div style="margin: 0in 0in 0pt">&nbsp;</div>
            <div style="margin: 0in 0in 0pt"><span style="font-family: 'Times New Roman','serif'; font-size: 10pt">LEGAL NOTICE:</span></div>
            <div style="margin: 0in 0in 0pt"><span style="font-family: 'Times New Roman','serif'; font-size: 10pt">Unless expressly stated otherwise, the information contained in this message is confidential, may be privileged, and protected from disclosure. If the reader of this message is not the intended recipient, or an employee or agent responsible for delivering this message to the intended recipient, you are hereby notified that any dissemination, distribution, or copying of this communication is strictly prohibited. If you have received this communication in error, please notify the sender immediately by replying to the message, permanently deleting it from your computer and/or destroy any copies/printouts of this message.</span>&nbsp;</div>
            <div style="margin: 0in 0in 0pt"><span style="font-family: 'Times New Roman','serif'; color: black; font-size: 10pt">Should employment follow the receipt of this Resume/Contract, whether directly, or at an end-client site, you hereby acknowledge that such employment of the consultant&rsquo;s services will not cause displacement of any US workers similarly employed by the Company, 90 days before and continuing 90 days after the date of commencement of said services. </span></div>
            </div>
            </td>
        </tr>
    </tbody>
</table>
<div style="padding:3px" width="100%" align="center" id="removeLink"><a href="http://portal.zoniacswift.com//servlet/ZoniacServlet?api=getOutsideUnsubscribeContact&e66Ve62i57=true&m9UCc1ucRi=693&h2BNp44yL7=187991&h2ZNp64yM8=null&h2TNp86wP9=4" class="font10"><font color="blue">Click here </font></a><span class="font10">to unsubscribe from our mailing list.</span><br><span class="font10">VLINK INC, 635 Farmington Ave, , Hartford, Connecticut, 06105      Phone: 860.247.1400 Fax: </span></div>
<div style="display:block;" width="100%" align="center"><table width="100%" align="center"><tr><td align="right" width="55%"><a target="_new" href="http://www.zoniac.com" style="vertical-align: text-top;"><font style="vertical-align: text-top;">Powered by Zoniac</font></a></td><td align="left" width="45%"><IMG align="bottom" height="20" width="20" border="0" SRC="cid:YlhdSQYejo"></td></tr></table></div><div id="removeLinkDummy1"></div><div id="removeLinkDummy2"></div><div id="removeLinkDummy3"></div><div id="removeLinkDummy4"></div><div id="removeLinkDummy5"></div><div id="removeLinkDummy6"></div>
<div style="padding:3px" width="100%" align="left" id="showDisclaimerData"></div>

--=_reb-r1D666D71-t4DDFAC35
Content-Type: image/gif; name="/Companies/693//custom/customerLogo.gif"
Content-Transfer-Encoding: base64
Content-Disposition: inline; 
	filename="/Companies/693//custom/customerLogo.gif"
Content-ID: <X4MwvXtS8A>



--=_reb-r1D666D71-t4DDFAC35
Content-Type: image/gif; name="/custom/UnsubscribedPoweredByImg.gif"
Content-Transfer-Encoding: base64
Content-Disposition: inline; filename="/custom/UnsubscribedPoweredByImg.gif"
Content-ID: <YlhdSQYejo>



--=_reb-r1D666D71-t4DDFAC35--

