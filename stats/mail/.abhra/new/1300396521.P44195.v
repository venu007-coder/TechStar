Return-Path: veerendra@xperttech.com
Received: from mx111.stngva01.us.mxservers.net (198.173.112.17)
	by mail19c.g19.rapidsite.net (RS ver 1.0.95vs) with SMTP id 2-0296126517
	for <abhra@altechstar.com>; Thu, 17 Mar 2011 17:15:17 -0400 (EDT)
Received: from unknown [76.74.252.24] (EHLO rs78.luxhv.com)
	by va1-mx111.stngva01.us.mxservers.net (mxl_mta-3.1.0-05)
	with ESMTP id 5e9728d4.2177887136.239824.00-001.va1-mx111.stngva01.us.mxservers.net (envelope-from <veerendra@xperttech.com>);
	Thu, 17 Mar 2011 17:15:17 -0400 (EDT)
Received: from ip-10-196-81-219.ec2.internal (ec2-184-72-246-77.compute-1.amazonaws.com [184.72.246.77])
	(authenticated bits=0)
	by rs78.luxhv.com (8.13.8/8.13.8) with ESMTP id p2HLF3xN016651;
	Thu, 17 Mar 2011 16:15:03 -0500
Date: Thu, 17 Mar 2011 16:15:02 -0500 (GMT-05:00)
From: Veerendra P <veerendra@xperttech.com>
To: veerendra@xperttech.com
Message-ID: <1300396502857.48846222092407676.veerendra@xperttech.com>
Subject: Lead Performance Tester - Atlanta, GA (Sure Shot)
MIME-Version: 1.0
Content-Type: multipart/mixed; 
	boundary="=_reb-r2CE3E7A7-t4D8279E5"
Importance: Normal
X-Lux-Comment: Message p2HLF3xN016651 sent by user #68993; report Spam, UCE, or Abuse to abuse@luxsci.us
X-Lux-Comment: LuxSci High Volume Email: http://luxhv.com
X-Processed-By: Rebuild v2.0-0
X-Spam: [F=0.2000000000; B=0.500(0); S=0.200(2010122901); MH=0.500(2011031715)]
X-MAIL-FROM: <veerendra@xperttech.com>
X-SOURCE-IP: [76.74.252.24]
X-Loop-Detect:1

This is a multi-part MIME message.

--=_reb-r2CE3E7A7-t4D8279E5
Content-Type: text/html; charset=us-ascii
Content-Transfer-Encoding: 7bit

<div style="padding:3px" width="100%" align="right" id="removeTopLink"><a href="http://portal.zoniacswift.com//servlet/ZoniacServlet?api=getOutsideUnsubscribeContact&e66Ve62i57=true&m9UCc1ucRi=540&h2BNp44yL7=229142&h5p8pKsyMi8l0=0&h2ZNp64yM8=null&h2TNp86wP9=5&h2GQy54sI2=1667" style="font: 11px verdana;"><font style="font: 11px verdana;" color="blue">Click here </font></a><span style="font: 11px verdana;">to unsubscribe.</span></div>


<style type="text/css">

		.font11 {color: black; font: 11px verdana}
		.font11bold {color: black; font: bold 11px verdana}	
		TABLE {font: 12px verdana}
	</style><br>
<table border="1" align="center" width="100%" cellpadding="10" cellspacing="0" style="border-left-color: #006aaf; border-bottom-color: #006aaf; border-top-color: #006aaf; border-collapse: collapse; border-right-color: #006aaf">
    <tbody>
        <tr>
            <td width="100%">
            <div id="customerLogo"><img src="cid:d8nkRHvpp6" alt="" border="0"><br>
            <br>
            <div style="margin: 0in 0in 0pt"><strong><span style="font-size: 16pt; color: #1f497d"><font face="Calibri">Lead Performance Tester </font></span></strong></div>
            <div style="margin: 0in 0in 0pt"><strong><span style="font-size: 12pt; color: #1f497d"><font face="Calibri">Atlanta, GA</font></span></strong></div>
            <div style="margin: 0in 0in 0pt"><strong><span style="font-size: 12pt; color: #1f497d"><font face="Calibri">6 Months</font></span></strong></div>
            <div style="margin: 0in 0in 0pt"><strong>&nbsp;</strong></div>
            <div style="margin: 0in 0in 0pt"><strong><span style="font-size: 9pt"><font face="Calibri"><u>Performance Testing</u></font></span></strong></div>
            <div style="margin: 0in 0in 0pt"><span style="font-size: 9pt"><font face="Calibri">&nbsp;</font></span></div>
            <div style="margin: 0in 0in 0pt"><span style="font-size: 9pt"><font face="Calibri">Technical/Functional Skills: We are looking for a Performance Testing Lead having following skills for an immediate long term position:</font></span></div>
            <div style="margin: 0in 0in 0pt"><span style="font-size: 9pt"><font face="Calibri">- 7-8 years of Performance Testing experience</font></span></div>
            <div style="margin: 0in 0in 0pt"><span style="font-size: 9pt"><font face="Calibri">- Excellent communication skills</font></span></div>
            <div style="margin: 0in 0in 0pt"><span style="font-size: 9pt"><font face="Calibri">- Ability to manage Testing end to end</font></span></div>
            <div style="margin: 0in 0in 0pt"><span style="font-size: 9pt"><font face="Calibri">- Load Runner (VuGen /Controller/Generator)</font></span></div>
            <div style="margin: 0in 0in 0pt"><span style="font-size: 9pt"><font face="Calibri">- Extensive experience in handling high volume of loads</font></span></div>
            <div style="margin: 0in 0in 0pt"><span style="font-size: 9pt"><font face="Calibri">- Script development (C++, VB), execution, troubleshooting and analysis.</font></span></div>
            <div style="margin: 0in 0in 0pt"><span style="font-size: 9pt"><font face="Calibri">- Performance Center</font></span></div>
            <div style="margin: 0in 0in 0pt"><span style="font-size: 9pt"><font face="Calibri">- Web application &ndash; complete functional knowledge (end to end)</font></span></div>
            <div style="margin: 0in 0in 0pt"><span style="font-size: 9pt"><font face="Calibri">- Web Servers / Application Servers (preferably WebLogic)</font></span></div>
            <div style="margin: 0in 0in 0pt"><span style="font-size: 9pt"><font face="Calibri">- Database &ndash; Oracle / Sql Server/dB2</font></span></div>
            <div style="margin: 0in 0in 0pt"><span style="font-size: 9pt"><font face="Calibri">- Basic knowledge/experience in Java</font></span></div>
            <div style="margin: 0in 0in 0pt"><span style="font-size: 9pt"><font face="Calibri">- Basic Networking knowledge</font></span></div>
            <div style="margin: 0in 0in 0pt"><span style="font-size: 9pt"><font face="Calibri">- Basic OS knowledge</font></span></div>
            <div style="margin: 0in 0in 0pt"><span style="font-size: 9pt"><font face="Calibri">- QTP &ndash; preferable</font></span></div>
            <div style="margin: 0in 0in 0pt"><span style="font-size: 9pt"><font face="Calibri">- Quality Center - preferable</font></span></div>
            <div style="margin: 0in 0in 0pt">&nbsp;</div>
            <div style="margin: 0in 0in 0pt"><strong><u><span style="font-size: 9pt"><font face="Calibri">Roles &amp; Responsibilities:</font></span></u></strong></div>
            <div style="margin: 0in 0in 0pt">&nbsp;</div>
            <div style="margin: 0in 0in 0pt"><span style="font-size: 9pt"><font face="Calibri">Lead the Performance testing unit at Atlanta and deliver the results in specified timelines</font></span></div>
            <div style="margin: 0in 0in 0pt"><span style="font-size: 9pt"><font face="Calibri">Risk Mitigation</font></span></div>
            <div style="margin: 0in 0in 0pt"><span style="font-size: 9pt"><font face="Calibri">Maintain Testing SLAs</font></span></div>
            <div style="margin: 0in 0in 0pt"><span style="font-size: 9pt"><font face="Calibri">Collaborate with partners</font></span></div>
            <div style="margin: 0in 0in 0pt"><span style="font-size: 9pt"><font face="Calibri">Provide regular status</font></span></div>
            <div style="margin: 0in 0in 0pt">&nbsp;</div>
            <div style="margin: 0in 0in 0pt">&nbsp;</div>
            <div style="margin: 0in 0in 0pt">&nbsp;</div>
            <div style="margin: 0in 0in 0pt"><strong><span style="color: #0d0d0d"><font face="Calibri">Thanks &amp; Regards,</font></span></strong></div>
            <div style="margin: 0in 0in 0pt"><strong>&nbsp;</strong></div>
            <div style="margin: 0in 0in 0pt"><strong><span style="color: #0d0d0d"><font size="3" face="Calibri">Veerendra<br>
            <br>
            <img src="cid:j2oMjOmt42" alt="" border="0" width="163" height="37" style="width: 146px; height: 27px"></font></span></strong></div>
            <div style="margin: 0in 0in 0pt"><span style="font-size: 10pt; color: #0d0d0d"><font face="Calibri">400 W Cummings Park || Suite#2850 || Woburn, MA-01801 </font></span></div>
            <div style="margin: 0in 0in 0pt"><span style="font-size: 10pt; color: #0d0d0d"><font face="Calibri">Email: </font></span><a href="mailto:veerendra@xperttech.com"><span style="font-size: 10pt; color: #0d0d0d"><font face="Calibri">veerendra@xperttech.com</font></span></a><span style="font-size: 10pt; color: #0d0d0d"><font face="Calibri">&nbsp;|| Phone: 781-422-8844</font></span></div>
            <div style="margin: 0in 0in 0pt"><span style="font-size: 10pt; color: #0d0d0d"><font face="Calibri">Fax: 978-405-5040 ||e-Fax: 781-207-0709|| </font></span><a href="http://www.xperttech.com/"><span style="font-size: 10pt; color: #0d0d0d"><font face="Calibri">www.xperttech.com</font></span></a></div>
            </div>
            <div style="font: 12px verdana" id="messagebody">&nbsp;</div>
            <div style="font: 12px verdana" id="Signature">&nbsp;</div>
            </td>
        </tr>
    </tbody>
</table>
<div style="padding:3px" width="100%" align="center" id="removeLink"><a href="http://portal.zoniacswift.com//servlet/ZoniacServlet?api=getOutsideUnsubscribeContact&e66Ve62i57=true&m9UCc1ucRi=540&h2BNp44yL7=229142&h5p8pKsyMi8l0=0&h2ZNp64yM8=null&h2TNp86wP9=5&h2GQy54sI2=1667" class="font10"><font color="blue">Click here </font></a><span class="font10">to unsubscribe from our mailing list.</span><br><span class="font10">XPERTTECH  Inc, 400 W Cummings Park, Suite #2850, Woburn, Massachusetts, 01801      Phone: 781 935 3775 Fax: 978-405-5040</span></div>
<div style="display:block;" width="100%" align="center"><table width="100%" align="center"><tr><td align="right" width="55%"><a target="_new" href="http://www.zoniac.com" style="vertical-align: text-top;"><font style="vertical-align: text-top;">Powered by Zoniac</font></a></td><td align="left" width="45%"><IMG align="bottom" height="20" width="20" border="0" SRC="cid:uGnsPMnmr8"></td></tr></table></div><div id="removeLinkDummy1"></div><div id="removeLinkDummy2"></div><div id="removeLinkDummy3"></div><div id="removeLinkDummy4"></div><div id="removeLinkDummy5"></div><div id="removeLinkDummy6"></div>

--=_reb-r2CE3E7A7-t4D8279E5
Content-Type: image/gif; name="/Companies/540//custom/customerLogo.gif"
Content-Transfer-Encoding: base64
Content-Disposition: inline; 
	filename="/Companies/540//custom/customerLogo.gif"
Content-ID: <d8nkRHvpp6>



--=_reb-r2CE3E7A7-t4D8279E5
Content-Type: image/gif; name="/Companies/540//custom/customerLogo.gif"
Content-Transfer-Encoding: base64
Content-Disposition: inline; 
	filename="/Companies/540//custom/customerLogo.gif"
Content-ID: <j2oMjOmt42>



--=_reb-r2CE3E7A7-t4D8279E5
Content-Type: image/gif; name="/custom/UnsubscribedPoweredByImg.gif"
Content-Transfer-Encoding: base64
Content-Disposition: inline; filename="/custom/UnsubscribedPoweredByImg.gif"
Content-ID: <uGnsPMnmr8>



--=_reb-r2CE3E7A7-t4D8279E5--

