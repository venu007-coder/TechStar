Return-Path: xavier@jjcpl.net
Received: from mx67.stngva01.us.mxservers.net (204.202.242.138)
	by mail19j.g19.rapidsite.net (RS ver 1.0.95vs) with SMTP id 1-0404189326
	for <abhra@altechstar.com>; Thu, 21 Apr 2011 11:22:06 -0400 (EDT)
Received: from unknown [216.151.164.100] (EHLO mail.njtech.com)
	by va1-mx67.stngva01.us.mxservers.net (mxl_mta-3.1.0-05)
	with ESMTP id d9b40bd4.20786080.1353732.00-005.va1-mx67.stngva01.us.mxservers.net (envelope-from <xavier@jjcpl.net>);
	Thu, 21 Apr 2011 11:22:05 -0400 (EDT)
Received: (qmail 8589 invoked from network); 21 Apr 2011 15:22:05 -0000
Received: from unknown (HELO NEW2-PC) ([122.164.130.192])
  by 10.97.0.64 with SMTP; 21 Apr 2011 15:22:03 -0000
From: "xavier" <xavier@jjcpl.net> 
To: "abhra@altechstar.com" <abhra@altechstar.com>
Reply-To: xavier@jjcpl.net
Date: Thu, 21 Apr 2011 10:21:48 -0500
Subject: Functional Test Engineer 
MIME-Version: 1.0
Content-Type: multipart/related;
	type="multipart/alternative";
	boundary="=_reb-r078A4FD3-t4DB04B9E"
X-Mailer: aspNetEmail ver 3.6.0.66
Message-ID: <NEW2-PC1f727a67348440919f020ece2e58ef17@NEW2-PC>
X-Processed-By: Rebuild v2.0-0
X-Spam: [F=0.2000000000; B=0.500(0); S=0.200(2010122901); MH=0.500(2011042117)]
X-MAIL-FROM: <xavier@jjcpl.net>
X-SOURCE-IP: [216.151.164.100]
X-SF-Loop: 1

This is a multi-part MIME message.

--=_reb-r078A4FD3-t4DB04B9E
Content-Type: multipart/alternative;
	boundary="=_reb-r29A01252-t4DB04B9E"

This is a multi-part MIME message.

--=_reb-r29A01252-t4DB04B9E
Content-Type: text/plain;
	charset="iso-8859-1"
Content-Transfer-Encoding: quoted-printable

Thanks for choosing JJCPL

Regards

R.Selva Kumar

"we forecast you"=20

469 327 3112

http://www.jjcpl.net=20

Skype: jjcpl.india

Gtalk : jjcpl.rpo



--=_reb-r29A01252-t4DB04B9E
Content-Type: text/html;
	charset="iso-8859-1"
Content-Transfer-Encoding: quoted-printable

<!DOCTYPE HTML PUBLIC "-//W3C//DTD HTML 4.0 Transitional//EN">
<HTML xmlns:o=3D"urn:schemas-microsoft-com:office:office"><HEAD>

</HEAD>
<BODY>
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt"><SPAN style=3D"mso-far=
east-font-family: 'Times New Roman'; mso-fareast-theme-font: minor-fareast;=
 mso-no-proof: yes"><FONT face=3D"Calibri"><SPAN style=3D"mso-fareast-font-=
family: 'Times New Roman'; mso-fareast-theme-font: minor-fareast; mso-no-pr=
oof: yes"><FONT face=3D"Calibri"><SPAN style=3D"mso-fareast-font-family: 'T=
imes New Roman'; mso-fareast-theme-font: minor-fareast; mso-no-proof: yes">=
<FONT face=3D"Calibri">Dear abhra,</FONT></SPAN></P>
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt"><SPAN style=3D"mso-far=
east-font-family: 'Times New Roman'; mso-fareast-theme-font: minor-fareast;=
 mso-no-proof: yes"></SPAN>&nbsp;</P></FONT></SPAN></FONT></SPAN>
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt"><SPAN style=3D"mso-far=
east-font-family: 'Times New Roman'; mso-fareast-theme-font: minor-fareast;=
 mso-no-proof: yes"><FONT face=3D"Calibri"></FONT></SPAN>&nbsp;</P><SPAN st=
yle=3D"mso-fareast-font-family: 'Times New Roman'; mso-fareast-theme-font: =
minor-fareast; mso-no-proof: yes"><FONT face=3D"Calibri">
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt"><SPAN style=3D"mso-far=
east-font-family: 'Times New Roman'"><FONT face=3D"Times New Roman">Rate $4=
5/hr<o:p></o:p></FONT></SPAN></P>
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt"><SPAN style=3D"mso-far=
east-font-family: 'Times New Roman'"><FONT face=3D"Times New Roman">Locatio=
n:Reston,VA<o:p></o:p></FONT></SPAN></P>
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt"><SPAN style=3D"mso-far=
east-font-family: 'Times New Roman'"><FONT face=3D"Times New Roman">Duratio=
n :6 months<o:p></o:p></FONT></SPAN></P>
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt"><SPAN style=3D"mso-far=
east-font-family: 'Times New Roman'"><FONT face=3D"Times New Roman">&nbsp;<=
o:p></o:p></FONT></SPAN></P>
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt"><SPAN style=3D"mso-far=
east-font-family: 'Times New Roman'"><FONT style=3D"BACKGROUND-COLOR: #ffff=
00"><FONT face=3D"Microsoft Sans Serif"><FONT size=3D"6">&nbsp;F2F is a mus=
t after telephonic=20
interview</FONT></FONT></FONT></SPAN></P>
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt"><SPAN style=3D"mso-far=
east-font-family: 'Times New Roman'"><FONT style=3D"BACKGROUND-COLOR: #ffff=
00"><FONT face=3D"Microsoft Sans Serif"><FONT size=3D"6"><o:p></o:p></FONT>=
</FONT></FONT></SPAN>&nbsp;</P>
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt"><STRONG><SPAN style=3D=
"mso-fareast-font-family: 'Times New Roman'"><FONT face=3D"Times New Roman"=
>CANDIDATES MUST HAVE EXPERIENCE WITH SELENIUM IDE OR=20
SELENIUM RC </FONT></SPAN></STRONG><B><SPAN style=3D"mso-fareast-font-famil=
y: 'Times New Roman'"><BR><BR></SPAN></B><SPAN style=3D"mso-fareast-font-fa=
mily: 'Times New Roman'"><FONT face=3D"Times New Roman">We are seeking a st=
rong Functional Test Engineer for our=20
Reston, VA client. Will be responsible for creating manual test cases.=20
<BR><BR>Functional Tester will create test cases, enter bugs, satisfy=20
requirements by executing manual and automated test scripts. Report issues =
with=20
other groups (i.e. Development, UI groups). <BR><BR>Candidate must have:=20
<BR>*solid communication skills and be able to work well with Developers in=
=20
Agile methodolgy environment (standups, sprint planning, retro's, etc.=20
<BR>*-MUST write excellent test cases. Well start to get into frontend port=
ion=20
of projects and writing excellent manual test cases is very important <BR>*=
solid=20
understanding of requirements and creating manual test cases *prefer some=
=20
automated experience using Selenium IDE and Selenium RC. <BR>*able to take=
=20
Selenium scripts exported as Java files and enhance scripts once UI is read=
y for=20
automation. <BR><BR>Familiarity with browser matrix testing and ability to=
=20
evaluate pages for accessibility (508 and WCAG). <BR><BR>Preferred Skills:=
=20
Selenium RC <BR><BR>Responsible for the analysis of functional requirements=
,=20
testing applications, developing test plans, test strategies, test cases an=
d=20
test scripts as well as evaluating test results to determine compliance wit=
h=20
test plans and established business processes. <BR><BR>ESSENTIAL=20
FUNCTIONS/RESPONSIBILITIES <BR>establishes and maintaining technical testin=
g=20
methodologies and processes. <BR>Develops test strategies and plans. Overse=
es=20
the development of and/or develops and maintains test data. <BR>Develops an=
d=20
delivers verbal and written status reports. <BR>Performs system quality=20
assurance testing by executing test plans. Reports, documents, and tracks=
=20
resolutions for software Defects identified in the testing process using=20
approved tools and processes. <BR>Assists in the confirmation of problems=
=20
reported from outside sources, and assists system development staff in=20
understanding problems and desired resolution. <BR>Assists in the=20
identification, selection, and implementation of software tools and=20
technologies, and testing techniques and procedures that facilitates the SQ=
A=20
process. <BR>Performs a variety of technical tasks which includes: conducti=
ng=20
routine tasks in conformance with set procedures within the department;=20
conducting regular technical functions, interacting with users or other=20
departments; performing complex technical functions, evaluating systems or=
=20
procedures and recommending action or changes; designing, developing, modif=
ying=20
and/or testing applications, systems and procedures; establishing and=20
documenting procedures for carrying out technical duties, in compliance wit=
h=20
best practices or new technology; and being accountable for compliance with=
=20
regulatory requirements and development/oversight of strategic planning for=
=20
technical function. <BR><BR>Skills <BR>Strong experience with structured=20
Software Development Life Cycle and Quality Assurance processes <BR>Strong=
=20
experience with functional, integration, and regression testing techniques =
in=20
validating web-based applications and services. <BR>Experience with techniq=
ues=20
of problem identification, root cause analysis, issue documentation, resolu=
tion=20
support, and testing. <BR>Excellent written and verbal communication skills=
;=20
excellent interpersonal skills; and strong customer service orientation.=20
<BR><BR>Strong technical knowledge and experience with the following: <BR>5=
=20
years of experience in QA team in a functional testing capacity. <BR>Execut=
ing=20
and monitoring automation scripts. <BR>Complex SQL queries and relational D=
ata=20
Base server testing experience preferred. <BR>UNIX platforms, Windows platf=
orms,=20
and command line language. <BR>Testing J2EE, .NET based applications.=20
<BR>Testing Web 2.0 technologies and applications i.e. Ajax, Flex and RSS f=
eeds.=20
<BR><BR>The ideal candidate will also have following: <BR>- Preferably 2 ye=
ars=20
experience using agile methodology. Its important QA stands up to DEV when =
it=20
comes to process. So well need a strong QA presence <BR>- Someone whose mor=
e=20
senior or lead projects <BR>- Excellent at writing test cases. Not just wri=
ting=20
test cases to satisfy reqs, but going the extra mile to create excellent te=
st=20
cases to BREAK code.<o:p></o:p></FONT></SPAN></P>
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt"><SPAN style=3D"mso-far=
east-font-family: 'Times New Roman'"><FONT face=3D"Times New Roman">&nbsp;<=
o:p></o:p></FONT></SPAN></P>
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt"><!-- Converted from te=
xt/rtf format --></FONT></SPAN></P>
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt"><SPAN style=3D"mso-far=
east-font-family: 'Times New Roman'; mso-fareast-theme-font: minor-fareast;=
 mso-no-proof: yes"><FONT face=3D"Calibri"></FONT></SPAN>&nbsp;</P><SPAN st=
yle=3D"mso-fareast-font-family: 'Times New Roman'; mso-fareast-theme-font: =
minor-fareast; mso-no-proof: yes"><FONT face=3D"Calibri">
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt"><SPAN style=3D"mso-far=
east-font-family: 'Times New Roman'; mso-fareast-theme-font: minor-fareast;=
 mso-no-proof: yes"><FONT face=3D"Calibri">Thanks for choosing JJCPL<o:p></=
o:p></FONT></SPAN></P>
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt"><SPAN style=3D"mso-far=
east-font-family: 'Times New Roman'; mso-fareast-theme-font: minor-fareast;=
 mso-no-proof: yes"><FONT face=3D"Calibri">Regards<o:p></o:p></FONT></SPAN>=
</P>
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt"><B><I><SPAN style=3D"F=
ONT-FAMILY: 'Century Gothic','sans-serif'; COLOR: #215868; FONT-SIZE: 12pt;=
 mso-fareast-font-family: 'Times New Roman'; mso-fareast-theme-font: minor-=
fareast; mso-no-proof: yes">Xavier</SPAN></I></B></P>
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt"><IMG height=3D"35" wid=
th=3D"138" src=3D"cid:image_0000" alt=3D""></P>
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt"><B><SPAN style=3D"FONT=
-FAMILY: 'Tahoma','sans-serif'; COLOR: #f2f2f2; FONT-SIZE: 7pt; mso-fareast=
-font-family: 'Times New Roman'; mso-fareast-theme-font: minor-fareast; mso=
-no-proof: yes"></SPAN></B><SPAN style=3D"COLOR: red; mso-fareast-font-fami=
ly: 'Times New Roman'; mso-fareast-theme-font: minor-fareast; mso-no-proof:=
 yes"><FONT face=3D"Calibri">&#8220;<I style=3D"mso-bidi-font-style: normal=
">we forecast you</I>&#8221; </P>
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt"></FONT></SPAN><SPAN st=
yle=3D"FONT-FAMILY: 'Bookman Old Style','serif'; FONT-SIZE: 9pt; mso-fareas=
t-font-family: 'Times New Roman'; mso-fareast-theme-font: minor-fareast; ms=
o-no-proof: yes"><o:p>469&nbsp;327=20
3112</o:p></SPAN></P>
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt"><SPAN style=3D"mso-far=
east-font-family: 'Times New Roman'; mso-fareast-theme-font: minor-fareast;=
 mso-no-proof: yes"><A href=3D"http://www.jjcpl.net/"><SPAN style=3D"COLOR:=
 blue"><FONT face=3D"Calibri">http://www.jjcpl.net</FONT></SPAN></A><FONT f=
ace=3D"Calibri">=20
<o:p></o:p></FONT></SPAN></P>
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt"><SPAN style=3D"mso-far=
east-font-family: 'Times New Roman'; mso-fareast-theme-font: minor-fareast;=
 mso-no-proof: yes"><FONT face=3D"Calibri">Skype: jjcpl.rpo</FONT></SPAN></=
P>
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt"><SPAN style=3D"mso-far=
east-font-family: 'Times New Roman'; mso-fareast-theme-font: minor-fareast;=
 mso-no-proof: yes"><FONT face=3D"Calibri">Gtalk : jjcpl.rpo</FONT></SPAN><=
/P><SPAN style=3D"mso-fareast-font-family: 'Times New Roman'; mso-fareast-t=
heme-font: minor-fareast; mso-no-proof: yes">
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt"><B><SPAN style=3D"COLO=
R: #31849b; FONT-SIZE: 11pt"><FONT face=3D"Times New Roman" color=3D"#0000f=
f"><A href=3D"http://jjcplrpo.blogspot.com">http://jjcplrpo.blogspot.com</A=
>=20
</FONT></SPAN></B></P></SPAN><SPAN style=3D"mso-fareast-font-family: 'Times=
 New Roman'; mso-fareast-theme-font: minor-fareast; mso-no-proof: yes">
<P class=3D"yiv1141086511msonormal" style=3D"MARGIN: 0in 0in 0pt"><STRONG><=
SPAN style=3D"FONT-FAMILY: 'Calibri','sans-serif'; COLOR: gray">Note: We re=
spect your=20
online privacy. This is not an unsolicited e-mail. If you are not intereste=
d in=20
receiving our e-mails then please reply with a</SPAN></STRONG><SPAN style=
=3D"FONT-FAMILY: 'Calibri','sans-serif'; COLOR: gray; FONT-SIZE: 10pt">=20
</SPAN><STRONG><SPAN style=3D"FONT-FAMILY: 'Calibri','sans-serif'; COLOR: r=
ed">"REMOVE"</SPAN></STRONG><SPAN style=3D"FONT-FAMILY: 'Calibri','sans-ser=
if'; COLOR: gray; FONT-SIZE: 10pt">=20
</SPAN><STRONG><SPAN style=3D"FONT-FAMILY: 'Calibri','sans-serif'; COLOR: g=
ray">in=20
the subject line. All removal requests will be honored ASAP. We sincerely=
=20
apologize for any inconvenience caused to you</SPAN></STRONG><SPAN style=3D=
"FONT-FAMILY: 'Calibri','sans-serif'; FONT-SIZE: 10pt"><o:p></o:p></SPAN></=
P></SPAN></FONT></SPAN></BODY></HTML>


--=_reb-r29A01252-t4DB04B9E--


--=_reb-r078A4FD3-t4DB04B9E
Content-Type: image/jpeg; name="logo%20new%201.jpg"
Content-Transfer-Encoding: base64
Content-ID: <image_0000>
Content-Disposition: inline



--=_reb-r078A4FD3-t4DB04B9E--

