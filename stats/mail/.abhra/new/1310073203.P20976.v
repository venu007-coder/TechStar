Return-Path: veerendra@xperttech.com
Received: from mx101.stngva01.us.mxservers.net (198.173.112.38)
	by mail19e.g19.rapidsite.net (RS ver 1.0.95vs) with SMTP id 2-0738633202
	for <abhra@altechstar.com>; Thu,  7 Jul 2011 17:13:22 -0400 (EDT)
Received: from unknown [76.74.252.24] (EHLO rs78.luxhv.com)
	by va1-mx101.stngva01.us.mxservers.net (mxl_mta-3.1.0-05)
	with ESMTP id 271261e4.2454064032.5726.00-007.va1-mx101.stngva01.us.mxservers.net (envelope-from <veerendra@xperttech.com>);
	Thu, 07 Jul 2011 17:13:22 -0400 (EDT)
Received: from ip-10-218-41-168.ec2.internal (ec2-184-72-246-77.compute-1.amazonaws.com [184.72.246.77])
	(authenticated bits=0)
	by rs78.luxhv.com (8.13.8/8.13.8) with ESMTP id p67LDCls014279;
	Thu, 7 Jul 2011 16:13:12 -0500
Date: Thu, 7 Jul 2011 17:13:11 -0400 (EDT)
From: Veerendra P <veerendra@xperttech.com>
To: veerendra@xperttech.com
Message-ID: <1310073191796.9385326287244906.veerendra@xperttech.com>
Subject: SURE SHOT..!!! Ruby on Rails - Southfield, MI
MIME-Version: 1.0
Content-Type: multipart/mixed; 
	boundary="=_reb-r1440CDC3-t4E162172"
Importance: Normal
X-Lux-Comment: Message p67LDCls014279 sent by user #68993; report Spam, UCE, or Abuse to abuse@luxsci.us
X-Lux-Comment: LuxSci High Volume Email: http://luxhv.com
X-Processed-By: Rebuild v2.0-0
X-Spam: [F=0.2000000000; B=0.500(0); S=0.200(2010122901); MH=0.500(2011070738)]
X-MAIL-FROM: <veerendra@xperttech.com>
X-SOURCE-IP: [76.74.252.24]
X-SF-Loop: 1

This is a multi-part MIME message.

--=_reb-r1440CDC3-t4E162172
Content-Type: text/html; charset=us-ascii
Content-Transfer-Encoding: 7bit

<div style="padding:3px" width="100%" align="right" id="removeTopLink"><a href="http://portal.zoniacswift.com//servlet/ZoniacServlet?api=getOutsideUnsubscribeContact&e66Ve62i57=true&m9UCc1ucRi=540&h2BNp44yL7=229142&h2ZNp64yM8=null&h2TNp86wP9=4" style="font: 11px verdana;"><font style="font: 11px verdana;" color="blue">Click here </font></a><span style="font: 11px verdana;">to unsubscribe.</span></div>


<style type="text/css">

		.font11 {color: black; font: 11px verdana}
		.font11bold {color: black; font: bold 11px verdana}	
		TABLE {font: 12px verdana}
	</style><br>
<table align="center" width="100%" cellpadding="10" cellspacing="0" border="1" style="border-bottom-color: #006aaf; border-top-color: #006aaf; border-collapse: collapse; border-right-color: #006aaf; border-left-color: #006aaf">
    <tbody>
        <tr>
            <td width="100%">
            <div id="customerLogo"><img src="cid:spezgatTgd" alt="" border="0"></div>
            <div id="messagebody" style="font: 12px verdana">&nbsp;</div>
            <div id="Signature" style="font: 12px verdana"><br>
            <div style="margin: 0in 0in 0pt"><strong><span style="font-family: Calibri; color: navy; font-size: 18pt">Ruby on Rails</span></strong></div>
            <div style="margin: 0in 0in 0pt"><strong><span style="font-family: Calibri; color: navy; font-size: 11pt">Southfield</span></strong><strong><span style="font-family: Calibri; color: navy; font-size: 11pt">, MI</span></strong></div>
            <div style="margin: 0in 0in 0pt"><strong><span style="font-family: Calibri; color: navy; font-size: 11pt">12 Months</span></strong></div>
            <div style="margin: 0in 0in 0pt">&nbsp;</div>
            <div style="margin: 0in 0in 0pt"><strong><span style="font-family: Calibri; font-size: 11pt">Details:</span></strong></div>
            <div style="margin: 0in 0in 0pt"><span style="font-family: Calibri; font-size: 11pt">&bull;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; Developers needed to build new applications built on Ruby on Rails framework. </span></div>
            <div style="margin: 0in 0in 0pt"><span style="font-family: Calibri; font-size: 11pt">&bull;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; Will be involved in design and development including database and building sophisticated model classes, to writing templates, to layering on a richer user experience.&nbsp;&nbsp; </span></div>
            <div style="margin: 0in 0in 0pt"><span style="font-family: Calibri; font-size: 11pt">&bull;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; Requires a working knowledge of RoR Interactive Development Environments. </span></div>
            <div style="margin: 0in 0in 0pt"><span style="font-family: Calibri; font-size: 11pt">&bull;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; Modify existing software to correct errors, allow it to adapt to new hardware, or to improve its performance. </span></div>
            <div style="margin: 0in 0in 0pt"><span style="font-family: Calibri; font-size: 11pt">&bull;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; Develop and direct software system testing and validation procedures, programming, and documentation. </span></div>
            <div style="margin: 0in 0in 0pt"><span style="font-family: Calibri; font-size: 11pt">&bull;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; Confer with systems analysts, engineers, programmers and others to design system and to obtain information on project limitations and capabilities, performance requirements and interfaces. </span></div>
            <div style="margin: 0in 0in 0pt"><span style="font-family: Calibri; font-size: 11pt">&bull;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; Analyze user needs and software requirements to determine feasibility of design within time and cost constraints. </span></div>
            <div style="margin: 0in 0in 0pt"><span style="font-family: Calibri; font-size: 11pt">&bull;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; Design, develop and modify software systems, using scientific analysis and mathematical models to predict and measure outcome and consequences of design. </span></div>
            <div style="margin: 0in 0in 0pt"><span style="font-family: Calibri; font-size: 11pt">&bull;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; Store, retrieve and manipulate data for analysis of system capabilities and requirements. </span></div>
            <div style="margin: 0in 0in 0pt"><span style="font-family: Calibri; font-size: 11pt">&bull;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; Consult with customers about software system design and maintenance. </span></div>
            <div style="margin: 0in 0in 0pt">&nbsp;</div>
            <div style="margin: 0in 0in 0pt"><strong><span style="font-family: Calibri; font-size: 11pt">Qualifications:</span></strong></div>
            <div style="margin: 0in 0in 0pt"><span style="font-family: Calibri; font-size: 11pt">&bull;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; Must have Ruby on Rails and MYSQL experience.&nbsp;&nbsp; </span></div>
            <div style="margin: 0in 0in 0pt"><span style="font-family: Calibri; font-size: 11pt">&bull;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; Experience with deploying Ruby on Rails applications in a high-availability, clustered environment. </span></div>
            <div style="margin: 0in 0in 0pt"><span style="font-family: Calibri; font-size: 11pt">&bull;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; Expertise in:&nbsp;Rails 2.3, MySQL, JQuery, Cucumber, RSpec, UNIX/Linux, Fusion Passenger, Git </span></div>
            <div style="margin: 0in 0in 0pt"><span style="font-family: Calibri; font-size: 11pt">&bull;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; Exemplary general IT knowledge (applications development, BDD/TDD, deployment, operations, documentation, standards, best practices, security, hardware, networking, OS, DBMS, middleware, etc.) in a corporate environment. </span></div>
            <div style="margin: 0in 0in 0pt"><span style="font-family: Calibri; font-size: 11pt">&bull;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; Full life-cycle development experience, including large-scale applications </span></div>
            <div style="margin: 0in 0in 0pt"><span style="font-family: Calibri; font-size: 11pt">&bull;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; Strong DB experience with Rails including use of the ActiveRecord ORM for complex queries as well as sophisticated SQL report generation </span></div>
            <div style="margin: 0in 0in 0pt"><span style="font-family: Calibri; font-size: 11pt">&bull;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; Comfort working directly with customers for requirements gathering, analysis, and continuous feedback.</span></div>
            <div style="margin: 0in 0in 0pt">&nbsp;</div>
            <div style="margin: 0in 0in 0pt">&nbsp;</div>
            <div style="margin: 0in 0in 0pt"><strong><span style="font-family: Calibri; color: #0d0d0d"><font size="3">Thanks &amp; Regards,</font></span></strong></div>
            <div style="margin: 0in 0in 0pt"><strong><font size="3">&nbsp;</font></strong></div>
            <div style="margin: 0in 0in 0pt"><strong><span style="font-family: Calibri; color: #0d0d0d"><font size="3">Veerendra</font></span></strong></div>
            <div style="margin: 0in 0in 0pt">&nbsp;</div>
            <div style="margin: 0in 0in 0pt"><span style="font-family: Calibri; color: #0d0d0d; font-size: 10pt"><img src="cid:Z9YluzsXqx" alt="" height="43" width="144" border="0"><br>
            400 W Cummings Park || Suite#2850 || Woburn, MA-01801 </span></div>
            <div style="margin: 0in 0in 0pt"><span style="font-family: Calibri; color: #0d0d0d; font-size: 10pt">Email: </span><a href="mailto:veerendra@xperttech.com"><span style="font-family: Calibri; color: #0d0d0d; font-size: 10pt">veerendra@xperttech.com</span></a><span style="font-family: Calibri; color: #0d0d0d; font-size: 10pt">&nbsp;|| Phone: 781-422-8844</span></div>
            <div style="margin: 0in 0in 0pt"><span style="font-family: Calibri; color: #0d0d0d; font-size: 10pt">Fax: 978-405-5040 ||e-Fax: 781-207-0709|| </span><a href="http://www.xperttech.com/"><span style="font-family: Calibri; color: #0d0d0d; font-size: 10pt">www.xperttech.com</span></a></div>
            <div style="margin: 0in 0in 0pt">&nbsp;</div>
            </div>
            </td>
        </tr>
    </tbody>
</table>
<div style="padding:3px" width="100%" align="center" id="removeLink"><a href="http://portal.zoniacswift.com//servlet/ZoniacServlet?api=getOutsideUnsubscribeContact&e66Ve62i57=true&m9UCc1ucRi=540&h2BNp44yL7=229142&h2ZNp64yM8=null&h2TNp86wP9=4" class="font10"><font color="blue">Click here </font></a><span class="font10">to unsubscribe from our mailing list.</span><br><span class="font10">XPERTTECH  Inc, 400 W Cummings Park, Suite #2850, Woburn, Massachusetts, 01801      Phone: 781 935 3775 Fax: 978-405-5040</span></div>
<div style="display:block;" width="100%" align="center"><table width="100%" align="center"><tr><td align="right" width="55%"><a target="_new" href="http://www.zoniac.com" style="vertical-align: text-top;"><font style="vertical-align: text-top;">Powered by Zoniac</font></a></td><td align="left" width="45%"><IMG align="bottom" height="20" width="20" border="0" SRC="cid:Cz7AcUmjzB"></td></tr></table></div><div id="removeLinkDummy1"></div><div id="removeLinkDummy2"></div><div id="removeLinkDummy3"></div><div id="removeLinkDummy4"></div><div id="removeLinkDummy5"></div><div id="removeLinkDummy6"></div>
<div style="padding:3px" width="100%" align="left" id="showDisclaimerData"></div>

--=_reb-r1440CDC3-t4E162172
Content-Type: image/gif; name="/Companies/540//custom/customerLogo.gif"
Content-Transfer-Encoding: base64
Content-Disposition: inline; 
	filename="/Companies/540//custom/customerLogo.gif"
Content-ID: <spezgatTgd>



--=_reb-r1440CDC3-t4E162172
Content-Type: image/gif; name="/Companies/540//custom/customerLogo.gif"
Content-Transfer-Encoding: base64
Content-Disposition: inline; 
	filename="/Companies/540//custom/customerLogo.gif"
Content-ID: <Z9YluzsXqx>



--=_reb-r1440CDC3-t4E162172
Content-Type: image/gif; name="/custom/UnsubscribedPoweredByImg.gif"
Content-Transfer-Encoding: base64
Content-Disposition: inline; filename="/custom/UnsubscribedPoweredByImg.gif"
Content-ID: <Cz7AcUmjzB>



--=_reb-r1440CDC3-t4E162172--

