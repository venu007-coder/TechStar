Return-Path: avtaar@vlinkinfo.com
Received: from mx102.stngva01.us.mxservers.net (198.173.112.39)
	by mail19h.g19.rapidsite.net (RS ver 1.0.95vs) with SMTP id 3-0764435113
	for <abhra@altechstar.com>; Fri, 13 May 2011 09:58:33 -0400 (EDT)
Received: from unknown [76.74.252.24] (EHLO rs78.luxhv.com)
	by va1-mx102.stngva01.us.mxservers.net (mxl_mta-3.1.0-05)
	with ESMTP id 9093dcd4.2724199328.118906.00-002.va1-mx102.stngva01.us.mxservers.net (envelope-from <avtaar@vlinkinfo.com>);
	Fri, 13 May 2011 09:58:33 -0400 (EDT)
Received: from ip-10-218-41-168.ec2.internal (ec2-184-72-246-77.compute-1.amazonaws.com [184.72.246.77])
	(authenticated bits=0)
	by rs78.luxhv.com (8.13.8/8.13.8) with ESMTP id p4DDwSRW028864;
	Fri, 13 May 2011 08:58:28 -0500
Date: Fri, 13 May 2011 09:58:27 -0400 (EDT)
From: Avtaar Singh <avtaar@vlinkinfo.com>
To: avtaar@vlinkinfo.com
Message-ID: <1305295107429.10125767582955936.avtaar@vlinkinfo.com>
Subject: URGENT REQ: Sr. Java Developer, CT
MIME-Version: 1.0
Content-Type: multipart/mixed; 
	boundary="=_reb-r1681D759-t4DCD3909"
Importance: Normal
X-Lux-Comment: Message p4DDwSRW028864 sent by user #76870; report Spam, UCE, or Abuse to abuse@luxsci.us
X-Lux-Comment: LuxSci High Volume Email: http://luxhv.com
X-Processed-By: Rebuild v2.0-0
X-Spam: [F=0.2000000000; B=0.500(0); S=0.200(2010122901); MH=0.500(2011051313)]
X-MAIL-FROM: <avtaar@vlinkinfo.com>
X-SOURCE-IP: [76.74.252.24]
X-SF-Loop: 1

This is a multi-part MIME message.

--=_reb-r1681D759-t4DCD3909
Content-Type: text/html; charset=us-ascii
Content-Transfer-Encoding: 7bit

<div style="padding:3px" width="100%" align="right" id="removeTopLink"><a href="http://portal.zoniacswift.com//servlet/ZoniacServlet?api=getOutsideUnsubscribeContact&e66Ve62i57=true&m9UCc1ucRi=693&h2BNp44yL7=208352&h2ZNp64yM8=null&h2TNp86wP9=4" style="font: 11px verdana;"><font style="font: 11px verdana;" color="blue">Click here </font></a><span style="font: 11px verdana;">to unsubscribe.</span></div>


<style type="text/css">

		.font11 {color: black; font: 11px verdana}
		.font11bold {color: black; font: bold 11px verdana}	
		TABLE {font: 12px verdana}
	</style><br>
<table align="center" width="100%" cellpadding="10" cellspacing="0" border="1" style="border-bottom-color: #006aaf; border-top-color: #006aaf; border-collapse: collapse; border-right-color: #006aaf; border-left-color: #006aaf">
    <tbody>
        <tr>
            <td width="100%">
            <div id="customerLogo"><img alt="" src="cid:H5PEUFRSW3" border="0"></div>
            <div id="messagebody" style="font: 12px verdana">&nbsp;</div>
            <div id="Signature" style="font: 12px verdana">&nbsp;</div>
            </td>
        </tr>
    </tbody>
</table>
<br>
<br>
<br>
<div style="margin: 0in 0in 0pt"><span style="font-family: 'Tahoma','sans-serif'; color: #002060; font-size: 10pt">This is Avtar from VLINK. I&rsquo;m currently looking for suitable resources for one of my requirement given below. Please let me know if you are available and interested .</span></div>
<div style="margin: 0in 0in 0pt">&nbsp;</div>
<div style="margin: 0in 0in 0pt"><span style="font-family: 'Tahoma','sans-serif'; color: #003366; font-size: 10pt">Look forward to hear from you.</span></div>
<div style="margin: 0in 0in 0pt">&nbsp;</div>
<div style="margin: 0in 0in 0pt"><strong><span style="font-family: 'Tahoma','sans-serif'; color: #c00000; font-size: 10pt">Position: Java Developer</span></strong></div>
<div style="margin: 0in 0in 0pt"><strong><span style="font-family: 'Tahoma','sans-serif'; color: #c00000; font-size: 10pt">Duration:&nbsp;12 months plus</span></strong></div>
<div style="margin: 0in 0in 0pt"><strong><span style="font-family: 'Tahoma','sans-serif'; color: #c00000; font-size: 10pt">Location: Windsor, CT</span></strong></div>
<div style="margin: 0in 0in 0pt">&nbsp;</div>
<div style="margin: 0in 0in 0pt">&nbsp;</div>
<span style="font-family: 'Tahoma','sans-serif'; color: #c00000; font-size: 10pt">
<div style="text-indent: -22.5pt; margin: 0in 0in 0pt 22.5pt"><strong><font color="#000000">Skill set: Must have skills:</font><br>
</strong><span style="font-family: 'Arial','sans-serif'; font-size: 10pt"><font face="Tahoma" color="#000000">senior java developer with 8+ years experience in J2EE technologies&nbsp;<br>
Java1.5/1.6&nbsp;<br>
axis and jax-ws webservices&nbsp;<br>
struts 1.x/2.0, jsp,&nbsp;<br>
servlets Ant<br>
and Maven build scripts&nbsp;<br>
JUnit test cases jdbc, oracle/db2/sybase<br>
weblogic and jboss<br>
</font><font color="#000000"><font face="Tahoma"><strong><span style="font-family: 'Arial','sans-serif'"><br>
Good to have skills:</span><br>
</strong>MQ series, JMS<br>
jstl<br>
ejb<br>
eclipse<br>
Clearcase<br>
Unix shell scripts<br>
have worked with financial applications</font></font></span></div>
<div style="margin: 0in 0in 0pt">&nbsp;</div>
<div style="margin: 0in 0in 0pt">&nbsp;</div>
<div style="margin: 0in 0in 0pt">&nbsp;</div>
<strong><span style="font-family: 'Tahoma','sans-serif'; color: #c00000; font-size: 10pt">
<div style="margin: 0in 0in 0pt"><strong><span style="color: #002060"><font face="Times New Roman" size="3">Thanks &amp; Regards.</font></span></strong></div>
<div style="margin: 0in 0in 0pt"><strong><font face="Times New Roman" size="3" color="#000000">&nbsp;</font></strong></div>
<div style="margin: 0in 0in 0pt"><strong><span style="color: #002060"><font face="Times New Roman" size="3">Avtar Singh</font></span></strong></div>
<div style="margin: 0in 0in 0pt"><strong><span style="color: #002060"><font face="Times New Roman" size="3">Senior Recruiter</font></span></strong></div>
<div style="margin: 0in 0in 0pt"><strong><span style="color: #002060"><font face="Times New Roman" size="3">VLink Inc | Phone: 860 760 1703 | Fax: 860 256 8484 | </font><a href="mailto:david.krentzman@vlinkinfo.com"><span style="color: #002060"><font face="Times New Roman" size="3">avtaar@vlinkinfo.com</font></span></a><font face="Times New Roman" size="3"> | </font><a href="http://www.vlinkinfo.com/"><span style="color: #002060"><font face="Times New Roman" size="3">www.vlinkinfo.com</font></span></a></span></strong></div>
<div style="margin: 0in 0in 0pt">&nbsp;</div>
</span></strong>&nbsp;</span>
<div style="padding:3px" width="100%" align="center" id="removeLink"><a href="http://portal.zoniacswift.com//servlet/ZoniacServlet?api=getOutsideUnsubscribeContact&e66Ve62i57=true&m9UCc1ucRi=693&h2BNp44yL7=208352&h2ZNp64yM8=null&h2TNp86wP9=4" class="font10"><font color="blue">Click here </font></a><span class="font10">to unsubscribe from our mailing list.</span><br><span class="font10">VLINK INC, 635 Farmington Ave, , Hartford, Connecticut, 06105      Phone: 860.247.1400 Fax: </span></div>
<div style="display:block;" width="100%" align="center"><table width="100%" align="center"><tr><td align="right" width="55%"><a target="_new" href="http://www.zoniac.com" style="vertical-align: text-top;"><font style="vertical-align: text-top;">Powered by Zoniac</font></a></td><td align="left" width="45%"><IMG align="bottom" height="20" width="20" border="0" SRC="cid:mEm7FYRZ9x"></td></tr></table></div><div id="removeLinkDummy1"></div><div id="removeLinkDummy2"></div><div id="removeLinkDummy3"></div><div id="removeLinkDummy4"></div><div id="removeLinkDummy5"></div><div id="removeLinkDummy6"></div>
<div style="padding:3px" width="100%" align="left" id="showDisclaimerData"></div>

--=_reb-r1681D759-t4DCD3909
Content-Type: image/gif; name="/Companies/693/custom/customerLogo.gif"
Content-Transfer-Encoding: base64
Content-Disposition: inline; 
	filename="/Companies/693/custom/customerLogo.gif"
Content-ID: <H5PEUFRSW3>



--=_reb-r1681D759-t4DCD3909
Content-Type: image/gif; name="/custom/UnsubscribedPoweredByImg.gif"
Content-Transfer-Encoding: base64
Content-Disposition: inline; filename="/custom/UnsubscribedPoweredByImg.gif"
Content-ID: <mEm7FYRZ9x>



--=_reb-r1681D759-t4DCD3909--

