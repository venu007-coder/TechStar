Return-Path: franklin@jjcpl.net
Received: from mx31.stngva01.us.mxservers.net (204.202.242.105)
	by mail19d.g19.rapidsite.net (RS ver 1.0.95vs) with SMTP id 2-0151898872
	for <abhra@altechstar.com>; Fri, 29 Apr 2011 10:54:32 -0400 (EDT)
Received: from unknown [216.151.164.100] (EHLO mail.njtech.com)
	by va1-mx31.stngva01.us.mxservers.net (mxl_mta-3.1.0-05)
	with ESMTP id 821dabd4.1772084128.248402.00-010.va1-mx31.stngva01.us.mxservers.net (envelope-from <franklin@jjcpl.net>);
	Fri, 29 Apr 2011 10:54:32 -0400 (EDT)
Received: (qmail 18652 invoked from network); 29 Apr 2011 14:54:31 -0000
Received: from unknown (HELO NEW2-PC) ([122.164.101.1])
  by 10.97.0.65 with SMTP; 29 Apr 2011 14:54:28 -0000
From: "franklin" <franklin@jjcpl.net> 
To: "abhra@altechstar.com" <abhra@altechstar.com>
Reply-To: franklin@jjcpl.net
Date: Fri, 29 Apr 2011 09:54:07 -0500
Subject: Data Architect II
MIME-Version: 1.0
Content-Type: multipart/related;
	type="multipart/alternative";
	boundary="=_reb-r4ED1504B-t4DBAD128"
X-Mailer: aspNetEmail ver 3.6.0.66
Message-ID: <NEW2-PCb4f6d12128494fc3b5d3de49753f122d@NEW2-PC>
X-Processed-By: Rebuild v2.0-0
X-Spam: [F=0.2000000000; B=0.500(0); S=0.200(2010122901); MH=0.500(2011042908)]
X-MAIL-FROM: <franklin@jjcpl.net>
X-SOURCE-IP: [216.151.164.100]
X-SF-Loop: 1

This is a multi-part MIME message.

--=_reb-r4ED1504B-t4DBAD128
Content-Type: multipart/alternative;
	boundary="=_reb-r6B4F624C-t4DBAD128"

This is a multi-part MIME message.

--=_reb-r6B4F624C-t4DBAD128
Content-Type: text/html;
	charset="iso-8859-1"
Content-Transfer-Encoding: quoted-printable

<!DOCTYPE HTML PUBLIC "-//W3C//DTD HTML 4.0 Transitional//EN">
<HTML xmlns:o=3D"urn:schemas-microsoft-com:office:office"><HEAD>

</HEAD>
<BODY><SPAN style=3D"mso-fareast-font-family: 'Times New Roman'; mso-fareas=
t-theme-font: minor-fareast; mso-no-proof: yes"><FONT face=3D"Calibri"><SPA=
N style=3D"mso-fareast-font-family: 'Times New Roman'; mso-fareast-theme-fo=
nt: minor-fareast; mso-no-proof: yes"><FONT face=3D"Calibri"><SPAN style=3D=
"mso-fareast-font-family: 'Times New Roman'; mso-fareast-theme-font: minor-=
fareast; mso-no-proof: yes"><FONT face=3D"Calibri">
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt"><SPAN style=3D"FONT-FA=
MILY: 'Arial Rounded MT Bold','sans-serif'; COLOR: black; FONT-SIZE: 9pt">H=
i=20
abhra,<o:p></o:p></SPAN></P>
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt"><SPAN style=3D"FONT-FA=
MILY: 'Arial Rounded MT Bold','sans-serif'; COLOR: black; FONT-SIZE: 9pt"><=
o:p>&nbsp;</o:p></SPAN></P>
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt"><SPAN style=3D"FONT-FA=
MILY: 'Arial Rounded MT Bold','sans-serif'; COLOR: black; FONT-SIZE: 9pt">G=
ood=20
Day!!!<o:p></o:p></SPAN></P>
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt"><SPAN style=3D"FONT-FA=
MILY: 'Arial Rounded MT Bold','sans-serif'; COLOR: black; FONT-SIZE: 9pt"><=
o:p>&nbsp;</o:p></SPAN></P>
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt"><SPAN style=3D"FONT-FA=
MILY: 'Arial Rounded MT Bold','sans-serif'; COLOR: black; FONT-SIZE: 9pt">P=
osition:=20
<B>Data Architect II<o:p></o:p></B></SPAN></P>
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt"><SPAN style=3D"FONT-FA=
MILY: 'Arial Rounded MT Bold','sans-serif'; COLOR: black; FONT-SIZE: 9pt">L=
ocation:Owings=20
Mills,Maryland<o:p></o:p></SPAN></P>
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt"><SPAN style=3D"FONT-FA=
MILY: 'Arial Rounded MT Bold','sans-serif'; COLOR: black; FONT-SIZE: 9pt">D=
uration:6+=20
Months<o:p></o:p></SPAN></P>
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt"><SPAN style=3D"FONT-FA=
MILY: 'Arial Rounded MT Bold','sans-serif'; COLOR: black; FONT-SIZE: 9pt">.=
<o:p></o:p></SPAN></P>
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt"><SPAN style=3D"FONT-FA=
MILY: 'Arial Rounded MT Bold','sans-serif'; COLOR: black; FONT-SIZE: 9pt"><=
o:p>&nbsp;</o:p></SPAN></P>
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt"><SPAN style=3D"FONT-FA=
MILY: 'Arial Rounded MT Bold','sans-serif'; BACKGROUND: yellow; COLOR: blac=
k; FONT-SIZE: 9pt; mso-highlight: yellow">Familiarity=20
with Informatica Powercenter 8.</SPAN><SPAN style=3D"FONT-FAMILY: 'Arial Ro=
unded MT Bold','sans-serif'; COLOR: black; FONT-SIZE: 9pt">=20
<o:p></o:p></SPAN></P>
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt"><SPAN style=3D"FONT-FA=
MILY: 'Arial Rounded MT Bold','sans-serif'; COLOR: black; FONT-SIZE: 9pt">&=
nbsp;<o:p></o:p></SPAN></P>
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt"><B><SPAN style=3D"FONT=
-FAMILY: 'Arial Rounded MT Bold','sans-serif'; COLOR: black; FONT-SIZE: 9pt=
">Responsibilities</SPAN></B><SPAN style=3D"FONT-FAMILY: 'Arial Rounded MT =
Bold','sans-serif'; COLOR: black; FONT-SIZE: 9pt">:=20
<o:p></o:p></SPAN></P>
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt"><SPAN style=3D"FONT-FA=
MILY: 'Arial Rounded MT Bold','sans-serif'; COLOR: black; FONT-SIZE: 9pt"><=
BR>*=20
Determination of Project Discovery approaches and estimations in the data=
=20
realm<BR>* Participation in the technical solution planning process<BR>*=20
Reviewing/providing input into project schedules as they evolve.<BR>* Revie=
wing=20
and providing input into requirements traceability matrices<BR>* Documentat=
ion=20
of source and target systems from a Data Architecture category perspective =
&#8211;=20
e.g. documenting source and target systems. <BR>* Translating business=20
strategy/drivers into IT requirements with a proven ability to solve comple=
x=20
problems or situations. <BR>* Establishing/enforcing the Enterprise informa=
tion=20
architecture and the data components of a Systems Development Lifecycle e.g=
.=20
Entity relationship diagrams, Data flow diagrams, Documenting external=20
interfaces, Business process and enterprise modeling/mapping artifacts to a=
 data=20
architecture framework; stratifying data for Data Management purposes. <BR>=
*=20
identification of data conversion and translation strategies i.e. review,=
=20
interpretation, and translation of requirements and use cases - Define, mai=
ntain=20
and enforce the use of standardized enterprise data element names,=20
abbreviations, definitions, characteristics, and domains. (This includes=20
elements/definitions used in Client software applications and all derived,=
=20
summarized and calculated elements found in data reporting tools/repositori=
es.)=20
<BR>* Mapping artifacts to a data architecture framework and managing=20
data/message requirements in an Enterprise Application Integration environm=
ent=20
for Middleware layer. <BR>* Identification of data and solution patterns as=
 well=20
as taxonomies<BR>* Project estimations. This includes review and approval o=
f=20
estimations in the data realm.<BR>* Creation of lists depicting external=20
documents/references<BR>*Creation of source to target mapping at the elemen=
t=20
level including specifications for data integrity, referential integrity, C=
RUD=20
diagrams, etc.<BR>* Design of WSDL if a web service interface is part of th=
e=20
solution.<BR>* Completion of logical data designs/models; review with Busin=
ess=20
stewards - Creation of physical data models and maintaining conformance to=
=20
established Client naming standards - Facilitate the resolution and mainten=
ance=20
of enterprise common valid values and assists clients with the synchronizat=
ion=20
and integration of data across the enterprise.<BR>* Assists with business n=
aming=20
standards, developing and maintaining consistent data definitions, determin=
ing=20
data aliases, documenting standard calculations and derivations, and docume=
nting=20
the business rules of the enterprise e.g. serves in a role for the enforcem=
ent=20
and maintenance of governance policies, processes, performance measurements=
, and=20
roles relative to healthcare industry best practice, and providing both=20
technical and administrative services to execute architecture policy,=20
procedures, roles and responsibilities, concept of operations and standards=
.=20
<BR>* Support Data Profiling<BR>* Publishes architectural documents -=20
Development/enforcement of procedures to convert enterprise assets to=20
information assets from a Knowledge Management standpoint.<BR>* Assists mor=
e=20
senior level people in their effort to provide data architectural design,=
=20
problem solving and technology leadership on an enterprise level. Can be=20
assigned some independent projects.<BR><BR><B>Requirements:</B>=20
<o:p></o:p></SPAN></P>
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt"><SPAN style=3D"FONT-FA=
MILY: 'Arial Rounded MT Bold','sans-serif'; COLOR: black; FONT-SIZE: 9pt"><=
BR>*=20
College Degree with a minimum of 4 years of data architecture experience, o=
r=20
equivalent total experience. <BR>* Candidates must possess strong relationa=
l=20
database experience in the logical and design areas.<BR>* Candidate must po=
ssess=20
a strong understanding of dimensional modeling and normalization=20
strategies.<BR>* Candidat must possess a minimum of 3 years experience with=
=20
ERWin Data Modeler and have experience with ERWin Model=20
Manager<o:p></o:p></SPAN></P>
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt"><SPAN lang=3D"EN-IN" s=
tyle=3D"mso-ansi-language: EN-IN"><o:p>&nbsp;</o:p></SPAN></P>
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt"></FONT></SPAN></FONT><=
/SPAN></FONT></SPAN><SPAN style=3D"mso-fareast-font-family: 'Times New Roma=
n'; mso-fareast-theme-font: minor-fareast; mso-no-proof: yes"><FONT face=3D=
"Calibri"></FONT></SPAN></P><SPAN style=3D"mso-fareast-font-family: 'Times =
New Roman'; mso-fareast-theme-font: minor-fareast; mso-no-proof: yes"><FONT=
 face=3D"Calibri">
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt"><SPAN style=3D"mso-far=
east-font-family: 'Times New Roman'; mso-fareast-theme-font: minor-fareast;=
 mso-no-proof: yes"><FONT face=3D"Calibri">Thanks for choosing JJCPL<o:p></=
o:p></FONT></SPAN></P>
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt"><SPAN style=3D"mso-far=
east-font-family: 'Times New Roman'; mso-fareast-theme-font: minor-fareast;=
 mso-no-proof: yes"><FONT face=3D"Calibri">Regards<o:p></o:p></FONT></SPAN>=
</P>
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt"><B><I><SPAN style=3D"F=
ONT-FAMILY: 'Century Gothic','sans-serif'; COLOR: #215868; FONT-SIZE: 12pt;=
 mso-fareast-font-family: 'Times New Roman'; mso-fareast-theme-font: minor-=
fareast; mso-no-proof: yes">Franklin</SPAN></I></B></P>
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt"><B><I><SPAN style=3D"F=
ONT-FAMILY: 'Century Gothic','sans-serif'; COLOR: #215868; FONT-SIZE: 12pt;=
 mso-fareast-font-family: 'Times New Roman'; mso-fareast-theme-font: minor-=
fareast; mso-no-proof: yes"><IMG height=3D"38" width=3D"159" src=3D"cid:ima=
ge_0000" alt=3D""></SPAN></I></B></P>
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt"><B><SPAN style=3D"FONT=
-FAMILY: 'Tahoma','sans-serif'; COLOR: #f2f2f2; FONT-SIZE: 7pt; mso-fareast=
-font-family: 'Times New Roman'; mso-fareast-theme-font: minor-fareast; mso=
-no-proof: yes"></SPAN></B><SPAN style=3D"COLOR: red; mso-fareast-font-fami=
ly: 'Times New Roman'; mso-fareast-theme-font: minor-fareast; mso-no-proof:=
 yes"><FONT face=3D"Calibri">&#8220;<I style=3D"mso-bidi-font-style: normal=
">we forecast you</I>&#8221; </P>
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt"></FONT></SPAN><SPAN st=
yle=3D"FONT-FAMILY: 'Bookman Old Style','serif'; FONT-SIZE: 9pt; mso-fareas=
t-font-family: 'Times New Roman'; mso-fareast-theme-font: minor-fareast; ms=
o-no-proof: yes"><o:p>214=20
613 4091</o:p></SPAN></P>
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt"><SPAN style=3D"mso-far=
east-font-family: 'Times New Roman'; mso-fareast-theme-font: minor-fareast;=
 mso-no-proof: yes"><A href=3D"http://www.jjcpl.net/"><SPAN style=3D"COLOR:=
 blue"><FONT face=3D"Calibri">http://www.jjcpl.net</FONT></SPAN></A><FONT f=
ace=3D"Calibri">=20
<o:p></o:p></FONT></SPAN></P>
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt"><SPAN style=3D"mso-far=
east-font-family: 'Times New Roman'; mso-fareast-theme-font: minor-fareast;=
 mso-no-proof: yes"><FONT face=3D"Calibri">Skype: jjcpl.rpo</FONT></SPAN></=
P>
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt"><SPAN style=3D"mso-far=
east-font-family: 'Times New Roman'; mso-fareast-theme-font: minor-fareast;=
 mso-no-proof: yes"><FONT face=3D"Calibri">Gtalk : jjcpl.rpo</FONT></SPAN><=
/P><SPAN style=3D"mso-fareast-font-family: 'Times New Roman'; mso-fareast-t=
heme-font: minor-fareast; mso-no-proof: yes">
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt"><B><SPAN style=3D"COLO=
R: #31849b; FONT-SIZE: 11pt"><FONT face=3D"Times New Roman" color=3D"#0000f=
f"><A href=3D"http://jjcplrpo.blogspot.com">http://jjcplrpo.blogspot.com</A=
>=20
</FONT></SPAN></B></P></SPAN><SPAN style=3D"mso-fareast-font-family: 'Times=
 New Roman'; mso-fareast-theme-font: minor-fareast; mso-no-proof: yes">
<P class=3D"yiv1141086511msonormal" style=3D"MARGIN: 0in 0in 0pt"><STRONG><=
SPAN style=3D"FONT-FAMILY: 'Calibri','sans-serif'; COLOR: gray">Note: We re=
spect your=20
online privacy. This is not an unsolicited e-mail. If you are not intereste=
d in=20
receiving our e-mails then please reply with a</SPAN></STRONG><SPAN style=
=3D"FONT-FAMILY: 'Calibri','sans-serif'; COLOR: gray; FONT-SIZE: 10pt">=20
</SPAN><STRONG><SPAN style=3D"FONT-FAMILY: 'Calibri','sans-serif'; COLOR: r=
ed">"REMOVE"</SPAN></STRONG><SPAN style=3D"FONT-FAMILY: 'Calibri','sans-ser=
if'; COLOR: gray; FONT-SIZE: 10pt">=20
</SPAN><STRONG><SPAN style=3D"FONT-FAMILY: 'Calibri','sans-serif'; COLOR: g=
ray">in=20
the subject line. All removal requests will be honored ASAP. We sincerely=
=20
apologize for any inconvenience caused to you</SPAN></STRONG><SPAN style=3D=
"FONT-FAMILY: 'Calibri','sans-serif'; FONT-SIZE: 10pt"><o:p></o:p></SPAN></=
P></SPAN></FONT></SPAN></BODY></HTML>


--=_reb-r6B4F624C-t4DBAD128--


--=_reb-r4ED1504B-t4DBAD128
Content-Type: image/jpeg; name="logo%20new%201.jpg"
Content-Transfer-Encoding: base64
Content-ID: <image_0000>
Content-Disposition: inline



--=_reb-r4ED1504B-t4DBAD128--

