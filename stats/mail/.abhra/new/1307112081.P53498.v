Return-Path: nsharma@vlinkinfo.com
Received: from mx89.stngva01.us.mxservers.net (198.173.112.6)
	by mail19j.g19.rapidsite.net (RS ver 1.0.95vs) with SMTP id 0-0602592080
	for <abhra@altechstar.com>; Fri,  3 Jun 2011 10:41:20 -0400 (EDT)
Received: from unknown [76.74.252.24] (EHLO rs78.luxhv.com)
	by va1-mx89.stngva01.us.mxservers.net (mxl_mta-3.1.0-05)
	with ESMTP id 092f8ed4.2831829920.627177.00-002.va1-mx89.stngva01.us.mxservers.net (envelope-from <nsharma@vlinkinfo.com>);
	Fri, 03 Jun 2011 10:41:20 -0400 (EDT)
Received: from ip-10-218-41-168.ec2.internal (ec2-184-72-246-77.compute-1.amazonaws.com [184.72.246.77])
	(authenticated bits=0)
	by rs78.luxhv.com (8.13.8/8.13.8) with ESMTP id p53EfFME032244;
	Fri, 3 Jun 2011 09:41:15 -0500
Date: Fri, 3 Jun 2011 10:41:14 -0400 (EDT)
From: Nivedita Sharma <nsharma@vlinkinfo.com>
To: nsharma@vlinkinfo.com
Message-ID: <1307112074788.8414177388868971.nsharma@vlinkinfo.com>
Subject: Direct Client Position: Data Analyst in Newark,NJ!
MIME-Version: 1.0
Content-Type: multipart/mixed; 
	boundary="=_reb-r078C826E-t4DE8F290"
Importance: High
X-Lux-Comment: Message p53EfFME032244 sent by user #76870; report Spam, UCE, or Abuse to abuse@luxsci.us
X-Lux-Comment: LuxSci High Volume Email: http://luxhv.com
X-Processed-By: Rebuild v2.0-0
X-Spam: [F=0.2000000000; B=0.500(0); S=0.200(2010122901); MH=0.500(2011060314)]
X-MAIL-FROM: <nsharma@vlinkinfo.com>
X-SOURCE-IP: [76.74.252.24]
X-SF-Loop: 1

This is a multi-part MIME message.

--=_reb-r078C826E-t4DE8F290
Content-Type: text/html; charset=us-ascii
Content-Transfer-Encoding: 7bit

<div style="padding:3px" width="100%" align="right" id="removeTopLink"><a href="http://portal.zoniacswift.com//servlet/ZoniacServlet?api=getOutsideUnsubscribeContact&e66Ve62i57=true&m9UCc1ucRi=693&h2BNp44yL7=187992&h2ZNp64yM8=null&h2TNp86wP9=4" style="font: 11px verdana;"><font style="font: 11px verdana;" color="blue">Click here </font></a><span style="font: 11px verdana;">to unsubscribe.</span></div>


<style type="text/css">

		.font11 {color: black; font: 11px verdana}
		.font11bold {color: black; font: bold 11px verdana}	
		TABLE {font: 12px verdana}
	</style><br>
<br>
<font face="Tahoma" size="2">Hi,<br>
<br>
Happy Friday!!<br>
<br>
I have a Data Analyst position with my client in Newark,NJ.&nbsp;<br>
<br>
We are getting immediate interview request for this role.&nbsp;&nbsp;<br>
<br>
</font>
<div style="margin: 0in 0in 0pt"><font face="Tahoma" size="2">&nbsp;</font></div>
<div style="margin: 0in 0in 0pt"><font face="Tahoma"><font size="2"><strong>Job Title:</strong> Data Analyst </font></font></div>
<div style="margin: 0in 0in 0pt"><font face="Tahoma"><font size="2"><strong>Location:</strong> Newark,NJ</font></font></div>
<div style="margin: 0in 0in 0pt"><font face="Tahoma"><font size="2"><strong>Rate:$60/hr</strong> c2c</font></font></div>
<div style="margin: 0in 0in 0pt"><font face="Tahoma"><font size="2"><strong>Duration: </strong>24 months+</font></font></div>
<div style="margin: 0in 0in 0pt"><font face="Tahoma" size="2">&nbsp;</font></div>
<div style="margin: 0in 0in 0pt"><font face="Tahoma" size="2">&nbsp;</font></div>
<div style="margin: 0in 0in 0pt"><font face="Tahoma" size="2">Send resumes to </font><a href="mailto:nsharma@vlinkinfo.com"><font face="Tahoma" size="2" color="#0000ff">nsharma@vlinkinfo.com</font></a><font face="Tahoma" size="2"> or reach me at 860.247.1400 ext 107</font></div>
<div style="margin: 0in 0in 0pt"><font face="Tahoma" size="2">&nbsp;</font></div>
<div style="margin: 0in 0in 0pt"><strong><font face="Tahoma" size="2">Technical Skills:</font></strong></div>
<div style="margin: 0in 0in 0pt"><font face="Tahoma" size="2">&nbsp;</font></div>
<div style="margin: 0in 0in 0pt"><font face="Tahoma" size="2">Demonstrated experience with technical data analysis</font></div>
<div style="margin: 0in 0in 0pt"><font face="Tahoma" size="2">Technically competent to describe how he/she approaches the work as well as how to do the work with strong SQL skills and use of tools, etc.</font></div>
<div style="margin: 0in 0in 0pt"><font face="Tahoma" size="2">Experience with payer systems especially claims (NASCO)</font></div>
<div style="margin: 0in 0in 0pt"><font face="Tahoma" size="2">Mainframe</font></div>
<div style="margin: 0in 0in 0pt"><font face="Tahoma" size="2">Java</font></div>
<div style="margin: 0in 0in 0pt"><font face="Tahoma" size="2">Strong SQL skills</font></div>
<div style="margin: 0in 0in 0pt"><font face="Tahoma" size="2">SOA</font></div>
<div style="margin: 0in 0in 0pt"><font face="Tahoma" size="2">Experience with profiling and data quality</font></div>
<div style="margin: 0in 0in 0pt"><font face="Tahoma" size="2">Excellent verbal and written communication skills</font></div>
<div style="margin: 0in 0in 0pt"><font face="Tahoma" size="2">&nbsp;</font></div>
<div style="margin: 0in 0in 0pt"><font face="Tahoma" size="2">&nbsp;</font></div>
<div style="margin: 0in 0in 0pt"><strong><font face="Tahoma" size="2">Data Experience:</font></strong></div>
<div style="margin: 0in 0in 0pt"><font face="Tahoma" size="2">Data analysis of payer data specifically around code sets including ICD9, CPT, ICD9PR on claims data</font></div>
<div style="margin: 0in 0in 0pt"><font face="Tahoma" size="2">Dataflow analysis to analyze data flows and interfaces between systems including claims processing, pricing, customer service and medical informatics</font></div>
<div style="margin: 0in 0in 0pt"><font face="Tahoma" size="2">Application analysis to analyze application processing logic such as data splits, merges, de-duplication</font></div>
<br>
<br>
<br>
<br>
<br>
<br>
<br>
<br>
<table align="center" width="100%" cellpadding="10" cellspacing="0" border="1" style="border-bottom-color: #006aaf; border-top-color: #006aaf; border-collapse: collapse; border-right-color: #006aaf; border-left-color: #006aaf">
    <tbody>
        <tr>
            <td width="100%">
            <div id="customerLogo"><img alt="" src="cid:TKIcFpXUN0" border="0"></div>
            <div id="messagebody" style="font: 12px verdana">&nbsp;</div>
            <div id="Signature" style="font: 12px verdana">
            <div style="margin: 0in 0in 0pt"><font face="Tahoma" size="2">Regards,<br>
            <br>
            Nivedita Sharma</font></div>
            <div style="margin: 0in 0in 0pt"><font face="Tahoma" size="2">Account Manager</font></div>
            <span style="font-family: 'Calibri','sans-serif'; font-size: 11pt"><font face="Tahoma" size="2">VLink Inc | Phone: 860.247.1400 ext 107| Fax: 860.256.8484 | nsharma@vlinkinfo.com&nbsp;</font></span>
            <div style="margin: 0in 0in 0pt"><font face="Tahoma" size="2">&nbsp;</font></div>
            <div style="margin: 0in 0in 0pt"><span style="color: black; font-size: 10pt"><font face="Tahoma">Should employment follow the receipt of this Resume/Contract, whether directly, or at an end-client site, you hereby acknowledge that such employment of the consultant&rsquo;s services will not cause displacement of any US workers similarly employed by the Company, 90 days before and continuing 90 days after the date of commencement of said services. </font></span></div>
            </div>
            </td>
        </tr>
    </tbody>
</table>
<div style="padding:3px" width="100%" align="center" id="removeLink"><a href="http://portal.zoniacswift.com//servlet/ZoniacServlet?api=getOutsideUnsubscribeContact&e66Ve62i57=true&m9UCc1ucRi=693&h2BNp44yL7=187992&h2ZNp64yM8=null&h2TNp86wP9=4" class="font10"><font color="blue">Click here </font></a><span class="font10">to unsubscribe from our mailing list.</span><br><span class="font10">VLINK INC, 635 Farmington Ave, , Hartford, Connecticut, 06105      Phone: 860.247.1400 Fax: </span></div>
<div style="display:block;" width="100%" align="center"><table width="100%" align="center"><tr><td align="right" width="55%"><a target="_new" href="http://www.zoniac.com" style="vertical-align: text-top;"><font style="vertical-align: text-top;">Powered by Zoniac</font></a></td><td align="left" width="45%"><IMG align="bottom" height="20" width="20" border="0" SRC="cid:i8qc9GR9LN"></td></tr></table></div><div id="removeLinkDummy1"></div><div id="removeLinkDummy2"></div><div id="removeLinkDummy3"></div><div id="removeLinkDummy4"></div><div id="removeLinkDummy5"></div><div id="removeLinkDummy6"></div>
<div style="padding:3px" width="100%" align="left" id="showDisclaimerData"></div>

--=_reb-r078C826E-t4DE8F290
Content-Type: image/gif; name="/Companies/693/custom/customerLogo.gif"
Content-Transfer-Encoding: base64
Content-Disposition: inline; 
	filename="/Companies/693/custom/customerLogo.gif"
Content-ID: <TKIcFpXUN0>



--=_reb-r078C826E-t4DE8F290
Content-Type: image/gif; name="/custom/UnsubscribedPoweredByImg.gif"
Content-Transfer-Encoding: base64
Content-Disposition: inline; filename="/custom/UnsubscribedPoweredByImg.gif"
Content-ID: <i8qc9GR9LN>



--=_reb-r078C826E-t4DE8F290--

