Return-Path: selva@jjcpl.net
Received: from mx107.stngva01.us.mxservers.net (198.173.112.44)
	by mail19i.g19.rapidsite.net (RS ver 1.0.95vs) with SMTP id 3-0359363758
	for <abhra@altechstar.com>; Thu, 24 Mar 2011 15:09:18 -0400 (EDT)
Received: from unknown [216.151.164.100] (EHLO mail.njtech.com)
	by va1-mx107.stngva01.us.mxservers.net (mxl_mta-3.1.0-05)
	with ESMTP id dd69b8d4.2869386144.4283.00-005.va1-mx107.stngva01.us.mxservers.net (envelope-from <selva@jjcpl.net>);
	Thu, 24 Mar 2011 15:09:17 -0400 (EDT)
Received: (qmail 3909 invoked from network); 24 Mar 2011 19:09:17 -0000
Received: from unknown (HELO NEW2-PC) ([122.174.4.3])
  by 10.97.0.64 with SMTP; 24 Mar 2011 19:09:13 -0000
From: "selva" <selva@jjcpl.net> 
To: "abhra@altechstar.com" <abhra@altechstar.com>
Reply-To: selva@jjcpl.net
Date: Thu, 24 Mar 2011 14:08:53 -0500
Subject: Data Loss Prevention (DLP) Engineer
MIME-Version: 1.0
Content-Type: multipart/related;
	type="multipart/alternative";
	boundary="=_reb-r660FAF27-t4D8B96DE"
X-Mailer: aspNetEmail ver 3.6.0.66
Message-ID: <NEW2-PC03ef28a1f00f429b86d8b49388c6511d@NEW2-PC>
X-Processed-By: Rebuild v2.0-0
X-Spam: [F=0.2000000000; B=0.500(0); S=0.200(2010122901); MH=0.500(2011032422)]
X-MAIL-FROM: <selva@jjcpl.net>
X-SOURCE-IP: [216.151.164.100]
X-SF-Loop: 1

This is a multi-part MIME message.

--=_reb-r660FAF27-t4D8B96DE
Content-Type: multipart/alternative;
	boundary="=_reb-r0E2783F1-t4D8B96DE"

This is a multi-part MIME message.

--=_reb-r0E2783F1-t4D8B96DE
Content-Type: text/plain;
	charset="iso-8859-1"
Content-Transfer-Encoding: quoted-printable

Thanks for choosing JJCPL

Regards

R.Selva Kumar

"we forecast you"=20

469 327 3112

http://www.jjcpl.net=20

Skype: jjcpl.india

Gtalk : jjcpl.rpo



--=_reb-r0E2783F1-t4D8B96DE
Content-Type: text/html;
	charset="iso-8859-1"
Content-Transfer-Encoding: quoted-printable

<!DOCTYPE HTML PUBLIC "-//W3C//DTD HTML 4.0 Transitional//EN">
<HTML xmlns:o=3D"urn:schemas-microsoft-com:office:office"><HEAD>

</HEAD>
<BODY>
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt"><SPAN style=3D"mso-far=
east-font-family: 'Times New Roman'; mso-fareast-theme-font: minor-fareast;=
 mso-no-proof: yes"><FONT face=3D"Calibri"><SPAN style=3D"mso-fareast-font-=
family: 'Times New Roman'; mso-fareast-theme-font: minor-fareast; mso-no-pr=
oof: yes"><FONT face=3D"Calibri"><SPAN style=3D"mso-fareast-font-family: 'T=
imes New Roman'; mso-fareast-theme-font: minor-fareast; mso-no-proof: yes">=
<FONT face=3D"Calibri">Dear abhra,</FONT></SPAN></P>
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt"><SPAN style=3D"mso-far=
east-font-family: 'Times New Roman'; mso-fareast-theme-font: minor-fareast;=
 mso-no-proof: yes"></SPAN>&nbsp;</P>
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt"><SPAN style=3D"mso-far=
east-font-family: 'Times New Roman'; mso-fareast-theme-font: minor-fareast;=
 mso-no-proof: yes">If=20
you have matching resumes for the below requirement send as soon as=20
possible.</SPAN></P></FONT></SPAN></FONT></SPAN>
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt"><SPAN style=3D"mso-far=
east-font-family: 'Times New Roman'; mso-fareast-theme-font: minor-fareast;=
 mso-no-proof: yes"><FONT face=3D"Calibri"></FONT></SPAN>&nbsp;</P><SPAN st=
yle=3D"mso-fareast-font-family: 'Times New Roman'; mso-fareast-theme-font: =
minor-fareast; mso-no-proof: yes">
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt"><B><SPAN style=3D"FONT=
-SIZE: 16pt">Data Loss Prevention (DLP)=20
Engineer<o:p></o:p></SPAN></B></P>
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt"><B><SPAN style=3D"FONT=
-SIZE: 16pt">Philadelphia, PA. <BR>Rate $55hr=20
<o:p></o:p></SPAN></B></P>
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt"><o:p></o:p>&nbsp;</P>
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt"><FONT face=3D"Microsof=
t Sans Serif" size=3D"6" style=3D"BACKGROUND-COLOR: #ffff00">F2F is a=20
must </FONT></P>
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt"><FONT face=3D"Microsof=
t Sans Serif" size=3D"6" style=3D"BACKGROUND-COLOR: #ffff00"></FONT><BR>Cor=
e Requirements: <BR><BR>Provide=20
development support for client, web, and database applications in a variety=
 of=20
languages/platforms. Candidate is expected to have 5-7 years of hands-on=20
experience developing web applications leveraging server-side, client-side =
and=20
database technologies. Candidate must demonstrate proficiency in at least o=
ne of=20
the major technology platforms (ASP.NET, Java, ColdFusion) as well as JavaS=
cript=20
and SQL. <BR><BR>Candidate should have knowledge of data loss prevention=20
strategies. Candidate will develop and manage the Symantec Data Loss Preven=
tion=20
application suite. The qualified candidate will have a diverse set of skill=
s to=20
support the backend structure while implementing solutions to a hosted=20
environment for centralized, streamlined and automated business processes.=
=20
<BR><BR>Responsibilities will include problem solving, coding and scripting=
, and=20
workflow design. Candidate will also be required to produce documentation o=
f all=20
products developed and will be involved in the support of the DLP applicati=
ons=20
utilized in production. The work will often be of a collaborative nature so=
 the=20
candidate will need to be able to interact well with others inside and outs=
ide=20
of his/her work group. <BR><BR><BR>Core Responsibilities: <BR>- Monitor, an=
alyze=20
and respond to security related events on the network using established too=
ls=20
and processes <BR>- Drive and participate in efforts and projects to develo=
p,=20
refine and implement Security Operations processes <BR>- Support compliance=
=20
related scans, assessments and remediation <BR>- Deploy, maintain and tune=
=20
Security Operations systems and applications <BR>- Interface with all inter=
nal=20
teams to investigate and drive the resolution of Information Security relat=
ed=20
issues <BR>- Build and maintain relationships with operational and technolo=
gy=20
teams <BR><BR>Requirements: <BR>Understanding of, and experience building a=
nd=20
running, Security Operations processes systems- Experience with network, sy=
stem=20
and application penetration techniques and vulnerability detection and=20
remediation <BR>Understanding of network design, client server topologies a=
nd=20
high availability systems <BR>Solid working knowledge of UNIX or LINUX and=
=20
experience with MS OS <BR>Ability to participate in on-call rotation and=20
incident response <BR>Strong communication (written and verbal) and cross=
=20
organizational collaboration skills <BR>Ability to manage small to medium s=
ized=20
projects <BR>Initiative to self start, take ownership, focus on highest=20
priorities and autonomously drive issues to completion <BR>Programming and=
=20
scripting skills a strong plus <BR>CISSP or related certifications a plus=
=20
<BR>Unquestionable integrity <BR>Experience in a hands-on Security related =
role=20
in a highly available environment <BR>Experience with system and applicatio=
n=20
security, SIEM, IDS, vulnerability scanners, packet capture and analysis, D=
LP=20
and related technologies<SPAN style=3D"FONT-FAMILY: 'Book Antiqua','serif';=
 COLOR: blue; FONT-SIZE: 11pt; mso-bidi-font-family: 'Book Antiqua'"><o:p><=
/o:p></SPAN></P>
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt"></SPAN><SPAN style=3D"=
mso-fareast-font-family: 'Times New Roman'; mso-fareast-theme-font: minor-f=
areast; mso-no-proof: yes"></SPAN><SPAN style=3D"mso-fareast-font-family: '=
Times New Roman'; mso-fareast-theme-font: minor-fareast; mso-no-proof: yes"=
><FONT face=3D"Calibri"></FONT></SPAN>&nbsp;</P><SPAN style=3D"mso-fareast-=
font-family: 'Times New Roman'; mso-fareast-theme-font: minor-fareast; mso-=
no-proof: yes"><FONT face=3D"Calibri">
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt"><SPAN style=3D"mso-far=
east-font-family: 'Times New Roman'; mso-fareast-theme-font: minor-fareast;=
 mso-no-proof: yes"><FONT face=3D"Calibri">Thanks for choosing JJCPL<o:p></=
o:p></FONT></SPAN></P>
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt"><SPAN style=3D"mso-far=
east-font-family: 'Times New Roman'; mso-fareast-theme-font: minor-fareast;=
 mso-no-proof: yes"><FONT face=3D"Calibri">Regards<o:p></o:p></FONT></SPAN>=
</P>
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt"><B><I><SPAN style=3D"F=
ONT-FAMILY: 'Century Gothic','sans-serif'; COLOR: #215868; FONT-SIZE: 12pt;=
 mso-fareast-font-family: 'Times New Roman'; mso-fareast-theme-font: minor-=
fareast; mso-no-proof: yes">Selva</SPAN></I></B></P>
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt"><IMG height=3D"35" wid=
th=3D"138" src=3D"cid:image_0000" alt=3D""></P>
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt"><B><SPAN style=3D"FONT=
-FAMILY: 'Tahoma','sans-serif'; COLOR: #f2f2f2; FONT-SIZE: 7pt; mso-fareast=
-font-family: 'Times New Roman'; mso-fareast-theme-font: minor-fareast; mso=
-no-proof: yes"></SPAN></B><SPAN style=3D"COLOR: red; mso-fareast-font-fami=
ly: 'Times New Roman'; mso-fareast-theme-font: minor-fareast; mso-no-proof:=
 yes"><FONT face=3D"Calibri">&#8220;<I style=3D"mso-bidi-font-style: normal=
">we forecast you</I>&#8221; </P>
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt"></FONT></SPAN><SPAN st=
yle=3D"FONT-FAMILY: 'Bookman Old Style','serif'; FONT-SIZE: 9pt; mso-fareas=
t-font-family: 'Times New Roman'; mso-fareast-theme-font: minor-fareast; ms=
o-no-proof: yes"><o:p>469=20
327 3112</o:p></SPAN></P>
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt"><SPAN style=3D"mso-far=
east-font-family: 'Times New Roman'; mso-fareast-theme-font: minor-fareast;=
 mso-no-proof: yes"><A href=3D"http://www.jjcpl.net/"><SPAN style=3D"COLOR:=
 blue"><FONT face=3D"Calibri">http://www.jjcpl.net</FONT></SPAN></A><FONT f=
ace=3D"Calibri">=20
<o:p></o:p></FONT></SPAN></P>
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt"><SPAN style=3D"mso-far=
east-font-family: 'Times New Roman'; mso-fareast-theme-font: minor-fareast;=
 mso-no-proof: yes"><FONT face=3D"Calibri">Skype: jjcpl.rpo</FONT></SPAN></=
P>
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt"><SPAN style=3D"mso-far=
east-font-family: 'Times New Roman'; mso-fareast-theme-font: minor-fareast;=
 mso-no-proof: yes"><FONT face=3D"Calibri">Gtalk : jjcpl.rpo</FONT></SPAN><=
/P><SPAN style=3D"mso-fareast-font-family: 'Times New Roman'; mso-fareast-t=
heme-font: minor-fareast; mso-no-proof: yes">
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt"><B><SPAN style=3D"COLO=
R: #31849b; FONT-SIZE: 11pt"><FONT face=3D"Times New Roman" color=3D"#0000f=
f"><A href=3D"http://jjcplrpo.blogspot.com">http://jjcplrpo.blogspot.com</A=
>=20
</FONT></SPAN></B></P></SPAN><SPAN style=3D"mso-fareast-font-family: 'Times=
 New Roman'; mso-fareast-theme-font: minor-fareast; mso-no-proof: yes">
<P class=3D"yiv1141086511msonormal" style=3D"MARGIN: 0in 0in 0pt"><STRONG><=
SPAN style=3D"FONT-FAMILY: 'Calibri','sans-serif'; COLOR: gray">Note: We re=
spect your=20
online privacy. This is not an unsolicited e-mail. If you are not intereste=
d in=20
receiving our e-mails then please reply with a</SPAN></STRONG><SPAN style=
=3D"FONT-FAMILY: 'Calibri','sans-serif'; COLOR: gray; FONT-SIZE: 10pt">=20
</SPAN><STRONG><SPAN style=3D"FONT-FAMILY: 'Calibri','sans-serif'; COLOR: r=
ed">"REMOVE"</SPAN></STRONG><SPAN style=3D"FONT-FAMILY: 'Calibri','sans-ser=
if'; COLOR: gray; FONT-SIZE: 10pt">=20
</SPAN><STRONG><SPAN style=3D"FONT-FAMILY: 'Calibri','sans-serif'; COLOR: g=
ray">in=20
the subject line. All removal requests will be honored ASAP. We sincerely=
=20
apologize for any inconvenience caused to you</SPAN></STRONG><SPAN style=3D=
"FONT-FAMILY: 'Calibri','sans-serif'; FONT-SIZE: 10pt"><o:p></o:p></SPAN></=
P></SPAN></FONT></SPAN></BODY></HTML>


--=_reb-r0E2783F1-t4D8B96DE--


--=_reb-r660FAF27-t4D8B96DE
Content-Type: image/jpeg; name="logo%20new%201.jpg"
Content-Transfer-Encoding: base64
Content-ID: <image_0000>
Content-Disposition: inline



--=_reb-r660FAF27-t4D8B96DE--

