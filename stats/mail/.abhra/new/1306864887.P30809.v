Return-Path: sameer@xperttech.com
Received: from mx109.stngva01.us.mxservers.net (198.173.112.15)
	by mail19g.g19.rapidsite.net (RS ver 1.0.95vs) with SMTP id 1-0216474886
	for <abhra@altechstar.com>; Tue, 31 May 2011 14:01:26 -0400 (EDT)
Received: from unknown [76.74.252.24] (EHLO rs78.luxhv.com)
	by va1-mx109.stngva01.us.mxservers.net (mxl_mta-3.1.0-05)
	with ESMTP id 5fc25ed4.2667518880.1599.00-012.va1-mx109.stngva01.us.mxservers.net (envelope-from <sameer@xperttech.com>);
	Tue, 31 May 2011 14:01:25 -0400 (EDT)
Received: from ip-10-218-41-168.ec2.internal (ec2-184-72-246-77.compute-1.amazonaws.com [184.72.246.77])
	(authenticated bits=0)
	by rs78.luxhv.com (8.13.8/8.13.8) with ESMTP id p4VI1F3g008015;
	Tue, 31 May 2011 13:01:15 -0500
Date: Tue, 31 May 2011 14:01:15 -0400 (EDT)
From: Sameer Maharana <sameer@xperttech.com>
To: sameer@xperttech.com
Message-ID: <1306864875017.6545109754371621.sameer@xperttech.com>
Subject: Remedy Developer (Can work Remotely) // Commack, Long Island. NY //
 long-Term Contract
MIME-Version: 1.0
Content-Type: multipart/mixed; 
	boundary="=_reb-r6BCAFC54-t4DE52CF6"
Importance: High
X-Lux-Comment: Message p4VI1F3g008015 sent by user #68993; report Spam, UCE, or Abuse to abuse@luxsci.us
X-Lux-Comment: LuxSci High Volume Email: http://luxhv.com
X-Processed-By: Rebuild v2.0-0
X-Spam: [F=0.2000000000; B=0.500(0); S=0.200(2010122901); MH=0.500(2011053119)]
X-MAIL-FROM: <sameer@xperttech.com>
X-SOURCE-IP: [76.74.252.24]
X-SF-Loop: 1

This is a multi-part MIME message.

--=_reb-r6BCAFC54-t4DE52CF6
Content-Type: text/html; charset=us-ascii
Content-Transfer-Encoding: 7bit

<div style="padding:3px" width="100%" align="right" id="removeTopLink"><a href="http://portal.zoniacswift.com//servlet/ZoniacServlet?api=getOutsideUnsubscribeContact&e66Ve62i57=true&m9UCc1ucRi=540&h2BNp44yL7=108734&h2ZNp64yM8=null&h2TNp86wP9=4" style="font: 11px verdana;"><font style="font: 11px verdana;" color="blue">Click here </font></a><span style="font: 11px verdana;">to unsubscribe.</span></div>


<style type="text/css">

		.font11 {color: black; font: 11px verdana}
		.font11bold {color: black; font: bold 11px verdana}	
		TABLE {font: 12px verdana}
	</style><br>
<table border="1" align="center" width="100%" cellpadding="10" cellspacing="0" style="border-left-color: #006aaf; border-bottom-color: #006aaf; border-top-color: #006aaf; border-collapse: collapse; border-right-color: #006aaf">
    <tbody>
        <tr>
            <td width="100%">
            <div id="customerLogo"><img alt="" border="0" src="cid:m2prHnfXHD"></div>
            <div style="font: 12px verdana" id="messagebody">&nbsp;</div>
            <div style="font: 12px verdana" id="Signature">
            <div style="margin: 0in 0in 0pt"><span style="font-size: 11pt; font-family: 'Calibri','sans-serif'">Hi, </span></div>
            <div style="margin: 0in 0in 0pt">&nbsp;</div>
            <div style="margin: 0in 0in 0pt"><span style="font-size: 11pt; font-family: 'Calibri','sans-serif'">Hope you&rsquo;re doing great&hellip;.</span></div>
            <div style="margin: 0in 0in 0pt">&nbsp;</div>
            <div style="margin: 0in 0in 0pt"><span style="font-size: 11pt; font-family: 'Calibri','sans-serif'">I have an immediate requirement opening for a <strong><span style="background: yellow">Remedy Developer (Can work Remotely) position in Commack, Long Island. NY for a long-Term Contract.</span></strong> I would appreciate if you could please send your updated word format resume along with best contact details ASAP.</span></div>
            <div style="margin: 0in 0in 0pt">&nbsp;</div>
            <div style="margin: 0in 0in 0pt"><strong><span style="font-size: 11pt; font-family: 'Calibri','sans-serif'">Here are the requirement details:</span></strong></div>
            <div style="margin: 0in 0in 0pt"><strong>&nbsp;</strong></div>
            <div style="margin: 0in 0in 0pt"><strong><span style="font-size: 11pt; color: blue; font-family: 'Calibri','sans-serif'">Position&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; : REMEDY DEVELOPER with ANALYTICS </span></strong></div>
            <div style="margin: 0in 0in 0pt"><strong><span style="font-size: 11pt; color: blue; font-family: 'Calibri','sans-serif'">Duration &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; : Long Term</span></strong></div>
            <div style="margin: 0in 0in 0pt"><strong><span style="font-size: 11pt; color: blue; font-family: 'Calibri','sans-serif'">Location &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; : Commack, Long Island. NY</span></strong></div>
            <div style="margin: 0in 0in 0pt">&nbsp;</div>
            <div style="margin: 0in 0in 0pt"><strong><span style="font-size: 22pt; color: red; font-family: 'Calibri','sans-serif'">Can work Remotely</span></strong></div>
            <div style="margin: 0in 0in 0pt">&nbsp;</div>
            <div style="margin: 0in 0in 0pt"><span style="font-size: 11pt; font-family: 'Calibri','sans-serif'">Looking for a quick and positive response from you</span></div>
            <div style="margin: 0in 0in 0pt"><span style="font-size: 11pt; font-family: 'Calibri','sans-serif'">Have a great day</span></div>
            <div style="margin: 0in 0in 0pt">&nbsp;</div>
            <div style="margin: 0in 0in 0pt"><a name="_MailAutoSig"><span style="font-size: 10pt; color: black; font-family: 'Cambria','serif'">Thanks/Regards,</span></a></div>
            <div style="margin: 0in 0in 0pt"><strong><span style="font-size: 16pt; color: green; font-family: 'Trebuchet MS','sans-serif'">SAMEER</span><span style="font-size: 11pt; color: navy; font-family: 'Cambria','serif'">,</span><span style="font-size: 11pt; color: navy; font-family: 'Cambria','serif'"><br>
            </span></strong></div>
            <div style="margin: 0in 0in 0pt"><span style="font-size: 10pt; color: black; font-family: 'Cambria','serif'">400 W Cummings Park,</span></div>
            <div style="margin: 0in 0in 0pt"><span style="font-size: 10pt; color: black; font-family: 'Cambria','serif'">Suite # 2850</span></div>
            <div style="margin: 0in 0in 0pt"><span style="font-size: 10pt; color: black; font-family: 'Cambria','serif'">Woburn, MA-01801</span></div>
            <div style="margin: 0in 0in 0pt"><span style="font-size: 10pt; color: black; font-family: 'Cambria','serif'">Email:</span><u><span style="font-size: 10pt; color: blue; font-family: 'Cambria','serif'">sameer@xperttech.com</span></u></div>
            <div style="margin: 0in 0in 0pt"><span style="font-size: 10pt; color: black; font-family: 'Cambria','serif'">Ph# 781-399-4354 | Fax # 781 207 0709 | </span><a href="http://www.xperttech.com/" title="blocked::http://www.xperttech.com/"><span style="font-size: 10pt; color: blue; font-family: 'Cambria','serif'">www.xperttech.com</span></a></div>
            <div style="margin: 0in 0in 0pt"><strong>&nbsp;</strong></div>
            <div style="margin: 0in 0in 0pt"><strong><span style="font-size: 8pt; color: #1f497d; font-family: 'Arial','sans-serif'">IMPORTANT NOTE</span><span style="font-size: 8pt; color: #1f497d; font-family: 'Arial','sans-serif'">: If you have received this email in error or prefer not to receive such emails in future, please reply with &quot;&quot;</span><span style="font-size: 8pt; color: red; font-family: 'Arial','sans-serif'">Remove</span><span style="font-size: 8pt; color: #1f497d; font-family: 'Arial','sans-serif'">&quot;&quot; in the subject line and the email-id(s) to be removed. All removal requests will be honored ASAP.</span></strong></div>
            <div style="margin: 0in 0in 0pt"><span style="font-size: 8pt; color: green; font-family: Webdings">P </span><span style="font-size: 8pt; color: green; font-family: 'Calibri','sans-serif'">Please consider the environment - do you really need to print this email?</span></div>
            </div>
            </td>
        </tr>
    </tbody>
</table>
<div style="padding:3px" width="100%" align="center" id="removeLink"><a href="http://portal.zoniacswift.com//servlet/ZoniacServlet?api=getOutsideUnsubscribeContact&e66Ve62i57=true&m9UCc1ucRi=540&h2BNp44yL7=108734&h2ZNp64yM8=null&h2TNp86wP9=4" class="font10"><font color="blue">Click here </font></a><span class="font10">to unsubscribe from our mailing list.</span><br><span class="font10">XPERTTECH  Inc, 400 W Cummings Park, Suite #2850, Woburn, Massachusetts, 01801      Phone: 781 935 3775 Fax: 978-405-5040</span></div>
<div style="display:block;" width="100%" align="center"><table width="100%" align="center"><tr><td align="right" width="55%"><a target="_new" href="http://www.zoniac.com" style="vertical-align: text-top;"><font style="vertical-align: text-top;">Powered by Zoniac</font></a></td><td align="left" width="45%"><IMG align="bottom" height="20" width="20" border="0" SRC="cid:aAg8wY3R1G"></td></tr></table></div><div id="removeLinkDummy1"></div><div id="removeLinkDummy2"></div><div id="removeLinkDummy3"></div><div id="removeLinkDummy4"></div><div id="removeLinkDummy5"></div><div id="removeLinkDummy6"></div>
<div style="padding:3px" width="100%" align="left" id="showDisclaimerData"></div>

--=_reb-r6BCAFC54-t4DE52CF6
Content-Type: image/gif; name="/Companies/540/custom/customerLogo.gif"
Content-Transfer-Encoding: base64
Content-Disposition: inline; 
	filename="/Companies/540/custom/customerLogo.gif"
Content-ID: <m2prHnfXHD>



--=_reb-r6BCAFC54-t4DE52CF6
Content-Type: image/gif; name="/custom/UnsubscribedPoweredByImg.gif"
Content-Transfer-Encoding: base64
Content-Disposition: inline; filename="/custom/UnsubscribedPoweredByImg.gif"
Content-ID: <aAg8wY3R1G>



--=_reb-r6BCAFC54-t4DE52CF6--

