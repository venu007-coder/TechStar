Return-Path: eswari@jjcpl.net
Received: from mx07.stngva01.us.mxservers.net (204.202.242.99)
	by mail19c.g19.rapidsite.net (RS ver 1.0.95vs) with SMTP id 4-077823819
	for <abhra@altechstar.com>; Thu,  2 Jun 2011 15:43:39 -0400 (EDT)
Received: from unknown [216.151.164.100] (EHLO mail.njtech.com)
	by va1-mx07.stngva01.us.mxservers.net (mxl_mta-3.1.0-05)
	with ESMTP id be7e7ed4.1357843360.636105.00-009.va1-mx07.stngva01.us.mxservers.net (envelope-from <eswari@jjcpl.net>);
	Thu, 02 Jun 2011 15:43:39 -0400 (EDT)
Received: (qmail 18290 invoked from network); 2 Jun 2011 19:43:38 -0000
Received: from unknown (HELO NEW2-PC) ([122.164.248.253])
  by 10.97.0.64 with SMTP; 2 Jun 2011 19:43:29 -0000
From: "Eswari" <eswari@jjcpl.net> 
To: "abhra@altechstar.com" <abhra@altechstar.com>
Reply-To: eswari@jjcpl.net
Date: Thu, 02 Jun 2011 14:43:10 -0500
Subject: BizTalk Developer
MIME-Version: 1.0
Content-Type: multipart/related;
	type="multipart/alternative";
	boundary="=_reb-r320202F4-t4DE7E7EB"
X-Mailer: aspNetEmail ver 3.6.0.66
Message-ID: <NEW2-PC4966bea39dc84d8c9a8091809e331ef7@NEW2-PC>
X-Processed-By: Rebuild v2.0-0
X-Spam: [F=0.2000000000; B=0.500(0); S=0.200(2010122901); MH=0.500(2011060236)]
X-MAIL-FROM: <eswari@jjcpl.net>
X-SOURCE-IP: [216.151.164.100]
X-SF-Loop: 1

This is a multi-part MIME message.

--=_reb-r320202F4-t4DE7E7EB
Content-Type: multipart/alternative;
	boundary="=_reb-r24CAAECE-t4DE7E7EB"

This is a multi-part MIME message.

--=_reb-r24CAAECE-t4DE7E7EB
Content-Type: text/html;
	charset="iso-8859-1"
Content-Transfer-Encoding: quoted-printable

<!DOCTYPE HTML PUBLIC "-//W3C//DTD HTML 4.0 Transitional//EN">
<HTML xmlns:o=3D"urn:schemas-microsoft-com:office:office"><HEAD>

</HEAD>
<BODY>
<P class=3D"MsoNoSpacing" style=3D"MARGIN: 0in 0in 0pt"><FONT face=3D"Calib=
ri">Dearabhra,<o:p></o:p></FONT></P>
<P class=3D"MsoNoSpacing" style=3D"MARGIN: 0in 0in 0pt"><o:p><FONT face=3D"=
Calibri">&nbsp;</FONT></o:p></P>
<P class=3D"MsoNoSpacing" style=3D"MARGIN: 0in 0in 0pt"><FONT face=3D"Calib=
ri">Good=20
Day!!!!!!<o:p></o:p></FONT></P>
<P class=3D"MsoNoSpacing" style=3D"MARGIN: 0in 0in 0pt"><o:p><FONT face=3D"=
Calibri">&nbsp;</FONT></o:p></P>
<P class=3D"MsoNoSpacing" style=3D"MARGIN: 0in 0in 0pt"><FONT face=3D"Calib=
ri">Hope you=20
are doing great!!!<o:p></o:p></FONT></P>
<P class=3D"MsoNoSpacing" style=3D"MARGIN: 0in 0in 0pt"><o:p><FONT face=3D"=
Calibri">&nbsp;</FONT></o:p></P>
<P class=3D"MsoNoSpacing" style=3D"MARGIN: 0in 0in 0pt"><FONT face=3D"Calib=
ri"><B>If you=20
have any consultant regarding for the below requirement please inform me th=
rough=20
phone or email</B>.<o:p></o:p></FONT></P>
<P class=3D"MsoNoSpacing" style=3D"MARGIN: 0in 0in 0pt"><o:p><FONT face=3D"=
Calibri">&nbsp;</FONT></o:p></P>
<P class=3D"MsoNoSpacing" style=3D"MARGIN: 0in 0in 0pt"><FONT face=3D"Calib=
ri">Position:=20
BizTalk Developer<o:p></o:p></FONT></P>
<P class=3D"MsoNoSpacing" style=3D"MARGIN: 0in 0in 0pt"><FONT face=3D"Calib=
ri">Duration:=20
12+months<o:p></o:p></FONT></P>
<P class=3D"MsoNoSpacing" style=3D"MARGIN: 0in 0in 0pt"><FONT face=3D"Calib=
ri">Location:=20
NYC <o:p></o:p></FONT></P>
<P class=3D"MsoNoSpacing" style=3D"MARGIN: 0in 0in 0pt"><o:p><FONT face=3D"=
Calibri">&nbsp;</FONT></o:p></P>
<P class=3D"MsoNoSpacing" style=3D"MARGIN: 0in 0in 0pt"><FONT face=3D"Calib=
ri">Well known=20
utility company seeks a highly motivated software developer with extensive=
=20
knowledge of BizTalk Server. As a member of the development team, the candi=
date=20
performs development on new and very complex applications. Candidates shoul=
d=20
have a minimum of 3 years experience, and should have worked in a team=20
environment on systems that have moved to production.<BR></FONT><B><BR><FON=
T face=3D"Calibri">Application development experience with the=20
following:</FONT></B><FONT face=3D"Calibri"> <BR>Microsoft BizTalk Server (=
2010=20
preferred)<BR>Microsoft BizTalk ESB Toolkit (2.1 preferred) <BR></FONT><A t=
arget=3D"_blank" href=3D"http://vb.net/"><SPAN style=3D"COLOR: windowtext; =
TEXT-DECORATION: none; text-underline: none"><FONT face=3D"Calibri">VB.NET<=
/FONT></SPAN></A><FONT face=3D"Calibri"> or C# (C#=20
preferred)<BR>Microsoft .NET Framework 3.5 and 4.0<BR>Visual Studio (2010=
=20
preferred) <BR>Microsoft SQL Server, or Oracle Database (desired, but not=
=20
required) <BR></FONT><B><BR><FONT face=3D"Calibri">Understanding of the=20
following:</FONT></B><FONT face=3D"Calibri"> <BR>XML, XSD, schema developme=
nt,=20
XPath<BR>Map, orchestration, custom pipeline component development<BR>Data=
=20
translation concepts (desired, but not required) <BR>Application debugging=
=20
<BR>Application performance analysis <BR>Source control usage and concepts=
=20
<BR>Strong communication skills, both written and verbal <BR>Strong=20
organizational skills, including time management <BR>Ability to take direct=
ion=20
and work in a structured environment <BR>BS in Computer Science or equivale=
nt=20
<BR>Logica Work Management Information System (desired, but not required)=
=20
<BR><BR>Additional Skills (Skills that are a plus, but not required)=20
<BR>Mainframe (IMS, CICS, DB2, VSAM) <BR><BR>Assignment <BR>Perform BizTalk=
=20
development.<BR>Perform database development using SQL Server and/or Oracle=
=20
Database.<BR>Participate in architectural review sessions to learn new syst=
em=20
architectures.<BR>Author technical documentation.<o:p></o:p></FONT></P>
<P class=3D"MsoNoSpacing" style=3D"MARGIN: 0in 0in 0pt"><BR><FONT face=3D"C=
alibri"><B>Requirements:</B> <BR>* College Degree with a minimum of 4 years=
=20
of data architecture experience, or equivalent total experience. <BR>*=20
Candidates must possess strong relational database experience in the logica=
l and=20
design areas.<BR>* Candidate must possess a strong understanding of dimensi=
onal=20
modeling and normalization strategies.<BR>* Candidate must possess a minimu=
m of=20
3 years experience with ERWin Data Modeler and have experience with ERWin M=
odel=20
Manager.<o:p></o:p></FONT></P>
<P class=3D"MsoNoSpacing" style=3D"MARGIN: 0in 0in 0pt"><o:p><FONT face=3D"=
Calibri">&nbsp;</FONT></o:p></P>
<P class=3D"MsoNoSpacing" style=3D"MARGIN: 0in 0in 0pt"><FONT face=3D"Calib=
ri"><B>Responsibilities:</B> <BR>* Determination of Project Discovery=20
approaches and estimations in the data realm<BR>* Participation in the tech=
nical=20
solution planning process<BR>* Reviewing/providing input into project sched=
ules=20
as they evolve.<BR>* Reviewing and providing input into requirements=20
traceability matrices<BR>* Documentation of source and target systems from =
a=20
Data Architecture category perspective &#8211; e.g. documenting source and =
target=20
systems. <BR>* Translating business strategy/drivers into IT requirements w=
ith a=20
proven ability to solve complex problems or situations. <BR>*=20
Establishing/enforcing the Enterprise information architecture and the data=
=20
components of a Systems Development Lifecycle e.g. Entity relationship diag=
rams,=20
Data flow diagrams, Documenting external interfaces, Business process and=
=20
enterprise modeling/mapping artifacts to a data architecture framework;=20
stratifying data for Data Management purposes. <BR>* identification of data=
=20
conversion and translation strategies i.e. review, interpretation, and=20
translation of requirements and use cases - Define, maintain and enforce th=
e use=20
of standardized enterprise data element names, abbreviations, definitions,=
=20
characteristics, and domains. (This includes elements/definitions used in C=
lient=20
software applications and all derived, summarized and calculated elements f=
ound=20
in data reporting tools/repositories.) <BR>* Mapping artifacts to a data=20
architecture framework and managing data/message requirements in an Enterpr=
ise=20
Application Integration environment for Middleware layer. <BR>* Identificat=
ion=20
of data and solution patterns as well as taxonomies<BR>* Project estimation=
s.=20
This includes review and approval of estimations in the data realm.<BR>*=20
Creation of lists depicting external documents/references<BR>*Creation of s=
ource=20
to target mapping at the element level including specifications for data=20
integrity, referential integrity, CRUD diagrams, etc.<BR>* Design of WSDL i=
f a=20
web service interface is part of the solution.<BR>* Completion of logical d=
ata=20
designs/models; review with Business stewards - Creation of physical data m=
odels=20
and maintaining conformance to established&nbsp; Facilitate the resolution =
and=20
maintenance of enterprise common valid values and assists clients with the=
=20
synchronization and integration of data across the enterprise.<BR>* Assists=
 with=20
business naming standards, developing and maintaining consistent data=20
definitions, determining data aliases, documenting standard calculations an=
d=20
derivations, and documenting the business rules of the enterprise e.g. serv=
es in=20
a role for the enforcement and maintenance of governance policies, processe=
s,=20
performance measurements, and roles relative to healthcare industry best=20
practice, and providing both technical and administrative services to execu=
te=20
architecture policy, procedures, roles and responsibilities, concept of=20
operations and standards. <BR>* Support Data Profiling<BR>* Publishes=20
architectural documents - Development/enforcement of procedures to convert=
=20
enterprise assets to information assets from a Knowledge Management=20
standpoint.<BR>* Assists more senior level people in their effort to provid=
e=20
data architectural design, problem solving and technology leadership on an=
=20
enterprise level. Can be assigned some independent=20
projects.<o:p></o:p></FONT></P>
<P>
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt"><SPAN style=3D"mso-far=
east-font-family: 'Times New Roman'; mso-fareast-theme-font: minor-fareast;=
 mso-no-proof: yes"><FONT face=3D"Calibri">Thanks for choosing JJCPL<o:p></=
o:p></FONT></SPAN></P>
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt"><SPAN style=3D"mso-far=
east-font-family: 'Times New Roman'; mso-fareast-theme-font: minor-fareast;=
 mso-no-proof: yes"><FONT face=3D"Calibri">Regards </FONT></SPAN></P>
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt"><B><I><SPAN style=3D"F=
ONT-FAMILY: 'Century Gothic','sans-serif'; COLOR: #215868; FONT-SIZE: 12pt;=
 mso-fareast-font-family: 'Times New Roman'; mso-fareast-theme-font: minor-=
fareast; mso-no-proof: yes">Eswari</SPAN></I></B></P>
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt"><SPAN style=3D"FONT-FA=
MILY: 'Century Gothic','sans-serif'; COLOR: #215868; FONT-SIZE: 12pt; mso-f=
areast-font-family: 'Times New Roman'; mso-fareast-theme-font: minor-fareas=
t; mso-no-proof: yes"><FONT face=3D"Calibri" color=3D"#000000">Recruiter</F=
ONT></SPAN></P>
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt"><SPAN style=3D"COLOR: =
red; mso-fareast-font-family: 'Times New Roman'; mso-fareast-theme-font: mi=
nor-fareast; mso-no-proof: yes"><FONT face=3D"Calibri"><IMG height=3D"35" w=
idth=3D"138" src=3D"cid:image_0000" alt=3D""></FONT></SPAN></P></P><SPAN st=
yle=3D"COLOR: red; mso-fareast-font-family: 'Times New Roman'; mso-fareast-=
theme-font: minor-fareast; mso-no-proof: yes"><FONT face=3D"Calibri">
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt"><B><SPAN style=3D"FONT=
-FAMILY: 'Tahoma','sans-serif'; COLOR: #f2f2f2; FONT-SIZE: 7pt; mso-fareast=
-font-family: 'Times New Roman'; mso-fareast-theme-font: minor-fareast; mso=
-no-proof: yes"></SPAN></B><SPAN style=3D"COLOR: red; mso-fareast-font-fami=
ly: 'Times New Roman'; mso-fareast-theme-font: minor-fareast; mso-no-proof:=
 yes"><FONT face=3D"Calibri">&#8220;<I style=3D"mso-bidi-font-style: normal=
">we forecast you</I>&#8221;=20
</FONT></SPAN></P>
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt"></FONT></SPAN><SPAN st=
yle=3D"FONT-FAMILY: 'Bookman Old Style','serif'; FONT-SIZE: 9pt; mso-fareas=
t-font-family: 'Times New Roman'; mso-fareast-theme-font: minor-fareast; ms=
o-no-proof: yes"><o:p></o:p></SPAN></P>
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt"><SPAN style=3D"mso-far=
east-font-family: 'Times New Roman'; mso-fareast-theme-font: minor-fareast;=
 mso-no-proof: yes"><FONT face=3D"Microsoft Sans Serif" size=3D"2">469 327 =
3112</FONT></SPAN></P>
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt"><SPAN style=3D"mso-far=
east-font-family: 'Times New Roman'; mso-fareast-theme-font: minor-fareast;=
 mso-no-proof: yes"><A href=3D"http://www.jjcpl.net/"><SPAN style=3D"COLOR:=
 blue"><FONT face=3D"Calibri">http://www.jjcpl.net</FONT></SPAN></A><FONT f=
ace=3D"Calibri">=20
<o:p></o:p></FONT></SPAN></P>
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt"><SPAN style=3D"mso-far=
east-font-family: 'Times New Roman'; mso-fareast-theme-font: minor-fareast;=
 mso-no-proof: yes"><FONT face=3D"Calibri">Skype: jjcpl.rpo</FONT></SPAN></=
P>
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt"><SPAN style=3D"mso-far=
east-font-family: 'Times New Roman'; mso-fareast-theme-font: minor-fareast;=
 mso-no-proof: yes"><FONT face=3D"Calibri">Gtalk : jjcpl.rpo</FONT></SPAN><=
/P><SPAN style=3D"mso-fareast-font-family: 'Times New Roman'; mso-fareast-t=
heme-font: minor-fareast; mso-no-proof: yes">
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt"><B><SPAN style=3D"COLO=
R: #31849b; FONT-SIZE: 11pt"><FONT face=3D"Times New Roman" color=3D"#0000f=
f"><A href=3D"http://jjcplrpo.blogspot.com/">http://jjcplrpo.blogspot.com</=
A>=20
</FONT></SPAN></B></P></SPAN><SPAN style=3D"mso-fareast-font-family: 'Times=
 New Roman'; mso-fareast-theme-font: minor-fareast; mso-no-proof: yes">
<P class=3D"yiv1141086511msonormal" style=3D"MARGIN: 0in 0in 0pt"><STRONG><=
SPAN style=3D"FONT-FAMILY: 'Calibri','sans-serif'; COLOR: gray">Note: We re=
spect your=20
online privacy. This is not an unsolicited e-mail. If you are not intereste=
d in=20
receiving our e-mails then please reply with a</SPAN></STRONG><SPAN style=
=3D"FONT-FAMILY: 'Calibri','sans-serif'; COLOR: gray; FONT-SIZE: 10pt">=20
</SPAN><STRONG><SPAN style=3D"FONT-FAMILY: 'Calibri','sans-serif'; COLOR: r=
ed">"REMOVE"</SPAN></STRONG><SPAN style=3D"FONT-FAMILY: 'Calibri','sans-ser=
if'; COLOR: gray; FONT-SIZE: 10pt">=20
</SPAN><STRONG><SPAN style=3D"FONT-FAMILY: 'Calibri','sans-serif'; COLOR: g=
ray">in=20
the subject line. All removal requests will be honored ASAP. We sincerely=
=20
apologize for any inconvenience caused to you</SPAN></STRONG><SPAN style=3D=
"FONT-FAMILY: 'Calibri','sans-serif'; FONT-SIZE: 10pt"><o:p></o:p></SPAN></=
P></SPAN></BODY></HTML>


--=_reb-r24CAAECE-t4DE7E7EB--


--=_reb-r320202F4-t4DE7E7EB
Content-Type: image/jpeg; name="logo%20new%201.jpg"
Content-Transfer-Encoding: base64
Content-ID: <image_0000>
Content-Disposition: inline



--=_reb-r320202F4-t4DE7E7EB--

