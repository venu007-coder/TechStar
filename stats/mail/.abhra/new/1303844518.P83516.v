Return-Path: fredrick@jjcpl.net
Received: from mx11.stngva01.us.mxservers.net (204.202.242.100)
	by mail19c.g19.rapidsite.net (RS ver 1.0.95vs) with SMTP id 2-0213264517
	for <abhra@altechstar.com>; Tue, 26 Apr 2011 15:01:57 -0400 (EDT)
Received: from unknown [216.151.164.100] (EHLO mail.njtech.com)
	by va1-mx11.stngva01.us.mxservers.net (mxl_mta-3.1.0-05)
	with ESMTP id 4a617bd4.2483563424.41357.00-004.va1-mx11.stngva01.us.mxservers.net (envelope-from <fredrick@jjcpl.net>);
	Tue, 26 Apr 2011 15:01:56 -0400 (EDT)
Received: (qmail 16773 invoked from network); 26 Apr 2011 19:01:53 -0000
Received: from unknown (HELO NEW2-PC) ([122.164.178.52])
  by 10.97.0.65 with SMTP; 26 Apr 2011 19:01:52 -0000
From: "fredrick" <fredrick@jjcpl.net> 
To: "abhra@altechstar.com" <abhra@altechstar.com>
Reply-To: fredrick@jjcpl.net
Date: Tue, 26 Apr 2011 14:01:38 -0500
Subject: Sr. Java Developer 
MIME-Version: 1.0
Content-Type: multipart/related;
	type="multipart/alternative";
	boundary="=_reb-r66679099-t4DB716A4"
X-Mailer: aspNetEmail ver 3.6.0.66
Message-ID: <NEW2-PCf08b350887b141399e0336834cd031fb@NEW2-PC>
X-Processed-By: Rebuild v2.0-0
X-Spam: [F=0.2000000000; B=0.500(0); S=0.200(2010122901); MH=0.500(2011042625)]
X-MAIL-FROM: <fredrick@jjcpl.net>
X-SOURCE-IP: [216.151.164.100]
X-SF-Loop: 1

This is a multi-part MIME message.

--=_reb-r66679099-t4DB716A4
Content-Type: multipart/alternative;
	boundary="=_reb-r00401FD9-t4DB716A4"

This is a multi-part MIME message.

--=_reb-r00401FD9-t4DB716A4
Content-Type: text/html;
	charset="iso-8859-1"
Content-Transfer-Encoding: quoted-printable

<!DOCTYPE HTML PUBLIC "-//W3C//DTD HTML 4.0 Transitional//EN">
<HTML xmlns:o=3D"urn:schemas-microsoft-com:office:office"><HEAD>

</HEAD>
<BODY>
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt"><SPAN style=3D"mso-far=
east-font-family: 'Times New Roman'; mso-fareast-theme-font: minor-fareast;=
 mso-no-proof: yes"><FONT face=3D"Calibri"><SPAN style=3D"mso-fareast-font-=
family: 'Times New Roman'; mso-fareast-theme-font: minor-fareast; mso-no-pr=
oof: yes"><FONT face=3D"Calibri"><SPAN style=3D"mso-fareast-font-family: 'T=
imes New Roman'; mso-fareast-theme-font: minor-fareast; mso-no-proof: yes">=
<FONT face=3D"Calibri">Dear abhra,</FONT></SPAN></P>
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt"><SPAN style=3D"mso-far=
east-font-family: 'Times New Roman'; mso-fareast-theme-font: minor-fareast;=
 mso-no-proof: yes"></SPAN>&nbsp;</P>
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt"><SPAN style=3D"mso-far=
east-font-family: 'Times New Roman'; mso-fareast-theme-font: minor-fareast;=
 mso-no-proof: yes">If=20
you have matching resumes for the below requirement send as soon as=20
possible.</SPAN></P></FONT></SPAN></FONT></SPAN>
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt"><SPAN style=3D"mso-far=
east-font-family: 'Times New Roman'; mso-fareast-theme-font: minor-fareast;=
 mso-no-proof: yes"><FONT face=3D"Calibri"></FONT></SPAN>&nbsp;</P><SPAN st=
yle=3D"mso-fareast-font-family: 'Times New Roman'; mso-fareast-theme-font: =
minor-fareast; mso-no-proof: yes">
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt"><B style=3D"mso-bidi-f=
ont-weight: normal">Title: Sr. Java Developer <BR>Location:=20
Minneapolis, MN <BR>Duration: 6 months + <o:p></o:p></B></P>
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt"><B style=3D"mso-bidi-f=
ont-weight: normal">Rate $60/hr</B><BR style=3D"mso-special-character: line=
-break"><BR style=3D"mso-special-character: line-break"><o:p></o:p></P>
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt"><B style=3D"mso-bidi-f=
ont-weight: normal"><SPAN style=3D"COLOR: red">Citizens/GC/EAD or=20
any one else that can work on our W2 or 1099.<o:p></o:p></SPAN></B></P>
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt"><SPAN style=3D"FONT-FA=
MILY: 'Times New Roman','serif'; FONT-SIZE: 12pt; mso-fareast-font-family: =
'Times New Roman'; mso-ansi-language: EN-US; mso-fareast-language: EN-US; m=
so-bidi-language: AR-SA"><BR>This=20
position is for a Java / Web Services development position on an Agile=20
development team and will focus on developing Web Services using JAVA and=
=20
SOAP/Axis. Deep expertise and experience working in an Agile environment is=
=20
essential. <BR><BR>The candidate must have in-depth and hands on experience=
 that=20
covers multiple years, multiple projects using all the following technologi=
es:=20
<BR>o Java/JEE <BR>o OO Design <BR>o Developing Web Services with SOAP / Ax=
is=20
(or similar SOAP stack such as WebSphere) <BR>o Spring dependency injection=
=20
<BR>o CVS <BR>o JUnit <BR><BR>The candidate must have experience working on=
=20
multiple Agile projects over several years that practiced test driven=20
development, joint code ownership, Release and Iteration Planning, SCRUMs,=
=20
continuous integration, paired programming, and continuous refactoring.=20
Communication and collaboration skills are critically important. <BR><BR>Th=
e=20
candidate must have hands-on experience with the following technologies: <B=
R>o=20
Hibernate <BR>o Ant <BR>o SQL <BR>o Maven <BR>o Continuous Integration=20
framework/product (e.g. CruiseControl, Hudson) <BR>o WebLogic App Server or=
=20
similar product (e.g. Tomcat, Web Sphere, etc.) <BR><BR>The candidate must =
be=20
familiar with the following technologies: <BR>o Linux/Unix (e.g. basic comm=
ands,=20
configuring and installing packages like python, apache, etc.) <BR>o=20
Pair-Programming <BR>o Jmock or similar testing framework <BR>o AOP / Sprin=
g AOP=20
<BR>o WorkManager interface (or similar parallel processing framework)=20
<BR><BR>In addition to the skills called out above, the areas of expertise=
=20
listed below are needed to be a contributing member of our highly collabora=
tive=20
teams: <BR><BR>-Capable of leading/discussing multiple design alternatives =
in=20
all layers of the system, highlighting the trade-offs for all design approa=
ches=20
expressed on a per-story and task level. <BR>-A complete understanding of t=
he=20
application is needed, along with knowledge on the patterns/best practices =
used=20
throughout the entire application. <BR>-Capable of being a active participa=
nt in=20
a highly collaborative lab environment with the ability to guide technical =
and=20
functional discussions with the business and development teams. <BR>-Experi=
ence=20
in taking loosely defined user stories and working with all team members to=
=20
provide an iteratively designed and developed, unit tested implementation=
=20
meeting all business requirements. <BR>-Proven track record of researching =
and=20
implementing new technologies and Agile practices. <BR>-Able to clearly=20
communicate and lead discussions on design patterns, technical improvements=
, and=20
implementation strategies. <BR>-Experience being a leader on a team develop=
ing=20
software using Test Driven Development, working with mocking frameworks,=20
familiar with best practices on refactoring, and capable of designing and=
=20
articulating a unit testing strategy for all layers of the application.=20
<BR>-Provides direction, guidance and mentoring on all functional and techn=
ical=20
areas of the system for less experienced staff. </SPAN></SPAN></P>
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt"><SPAN style=3D"mso-far=
east-font-family: 'Times New Roman'; mso-fareast-theme-font: minor-fareast;=
 mso-no-proof: yes"><FONT face=3D"Calibri"></FONT></SPAN>&nbsp;</P><SPAN st=
yle=3D"mso-fareast-font-family: 'Times New Roman'; mso-fareast-theme-font: =
minor-fareast; mso-no-proof: yes"><FONT face=3D"Calibri">
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt"><SPAN style=3D"mso-far=
east-font-family: 'Times New Roman'; mso-fareast-theme-font: minor-fareast;=
 mso-no-proof: yes"><FONT face=3D"Calibri">Thanks for choosing JJCPL<o:p></=
o:p></FONT></SPAN></P>
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt"><SPAN style=3D"mso-far=
east-font-family: 'Times New Roman'; mso-fareast-theme-font: minor-fareast;=
 mso-no-proof: yes"><FONT face=3D"Calibri">Regards<o:p></o:p></FONT></SPAN>=
</P>
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt"><B><I><SPAN style=3D"F=
ONT-FAMILY: 'Century Gothic','sans-serif'; COLOR: #215868; FONT-SIZE: 12pt;=
 mso-fareast-font-family: 'Times New Roman'; mso-fareast-theme-font: minor-=
fareast; mso-no-proof: yes">Fredrick</SPAN></I></B></P>
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt"><B><I><SPAN style=3D"F=
ONT-FAMILY: 'Century Gothic','sans-serif'; COLOR: #215868; FONT-SIZE: 12pt;=
 mso-fareast-font-family: 'Times New Roman'; mso-fareast-theme-font: minor-=
fareast; mso-no-proof: yes"><IMG height=3D"32" width=3D"165" src=3D"cid:ima=
ge_0000" alt=3D""></SPAN></I></B></P>
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt"><B><SPAN style=3D"FONT=
-FAMILY: 'Tahoma','sans-serif'; COLOR: #f2f2f2; FONT-SIZE: 7pt; mso-fareast=
-font-family: 'Times New Roman'; mso-fareast-theme-font: minor-fareast; mso=
-no-proof: yes"></SPAN></B><SPAN style=3D"COLOR: red; mso-fareast-font-fami=
ly: 'Times New Roman'; mso-fareast-theme-font: minor-fareast; mso-no-proof:=
 yes"><FONT face=3D"Calibri">&#8220;<I style=3D"mso-bidi-font-style: normal=
">we forecast you</I>&#8221; </P>
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt"></FONT></SPAN><SPAN st=
yle=3D"FONT-FAMILY: 'Bookman Old Style','serif'; FONT-SIZE: 9pt; mso-fareas=
t-font-family: 'Times New Roman'; mso-fareast-theme-font: minor-fareast; ms=
o-no-proof: yes"><o:p>214=20
613 4090</o:p></SPAN></P>
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt"><SPAN style=3D"mso-far=
east-font-family: 'Times New Roman'; mso-fareast-theme-font: minor-fareast;=
 mso-no-proof: yes"><A href=3D"http://www.jjcpl.net/"><SPAN style=3D"COLOR:=
 blue"><FONT face=3D"Calibri">http://www.jjcpl.net</FONT></SPAN></A><FONT f=
ace=3D"Calibri">=20
<o:p></o:p></FONT></SPAN></P>
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt"><SPAN style=3D"mso-far=
east-font-family: 'Times New Roman'; mso-fareast-theme-font: minor-fareast;=
 mso-no-proof: yes"><FONT face=3D"Calibri">Skype: jjcpl.rpo</FONT></SPAN></=
P>
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt"><SPAN style=3D"mso-far=
east-font-family: 'Times New Roman'; mso-fareast-theme-font: minor-fareast;=
 mso-no-proof: yes"><FONT face=3D"Calibri">Gtalk : jjcpl.rpo</FONT></SPAN><=
/P><SPAN style=3D"mso-fareast-font-family: 'Times New Roman'; mso-fareast-t=
heme-font: minor-fareast; mso-no-proof: yes">
<P class=3D"MsoNormal" style=3D"MARGIN: 0in 0in 0pt"><B><SPAN style=3D"COLO=
R: #31849b; FONT-SIZE: 11pt"><FONT face=3D"Times New Roman" color=3D"#0000f=
f"><A href=3D"http://jjcplrpo.blogspot.com">http://jjcplrpo.blogspot.com</A=
>=20
</FONT></SPAN></B></P></SPAN><SPAN style=3D"mso-fareast-font-family: 'Times=
 New Roman'; mso-fareast-theme-font: minor-fareast; mso-no-proof: yes">
<P class=3D"yiv1141086511msonormal" style=3D"MARGIN: 0in 0in 0pt"><STRONG><=
SPAN style=3D"FONT-FAMILY: 'Calibri','sans-serif'; COLOR: gray">Note: We re=
spect your=20
online privacy. This is not an unsolicited e-mail. If you are not intereste=
d in=20
receiving our e-mails then please reply with a</SPAN></STRONG><SPAN style=
=3D"FONT-FAMILY: 'Calibri','sans-serif'; COLOR: gray; FONT-SIZE: 10pt">=20
</SPAN><STRONG><SPAN style=3D"FONT-FAMILY: 'Calibri','sans-serif'; COLOR: r=
ed">"REMOVE"</SPAN></STRONG><SPAN style=3D"FONT-FAMILY: 'Calibri','sans-ser=
if'; COLOR: gray; FONT-SIZE: 10pt">=20
</SPAN><STRONG><SPAN style=3D"FONT-FAMILY: 'Calibri','sans-serif'; COLOR: g=
ray">in=20
the subject line. All removal requests will be honored ASAP. We sincerely=
=20
apologize for any inconvenience caused to you</SPAN></STRONG><SPAN style=3D=
"FONT-FAMILY: 'Calibri','sans-serif'; FONT-SIZE: 10pt"><o:p></o:p></SPAN></=
P></SPAN></FONT></SPAN></BODY></HTML>


--=_reb-r00401FD9-t4DB716A4--


--=_reb-r66679099-t4DB716A4
Content-Type: image/jpeg; name="logo%20new%201.jpg"
Content-Transfer-Encoding: base64
Content-ID: <image_0000>
Content-Disposition: inline



--=_reb-r66679099-t4DB716A4--

