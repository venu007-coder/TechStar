Return-Path: nsharma@vlinkinfo.com
Received: from mx34.stngva01.us.mxservers.net (204.202.242.42)
	by mail19e.g19.rapidsite.net (RS ver 1.0.95vs) with SMTP id 3-0856067998
	for <abhra@altechstar.com>; Fri, 19 Aug 2011 11:33:18 -0400 (EDT)
Received: from unknown [76.74.252.24] (EHLO rs78.luxhv.com)
	by va1-mx34.stngva01.us.mxservers.net (mxl_mta-3.1.0-05)
	with ESMTP id e328e4e4.2911038368.229242.00-003.va1-mx34.stngva01.us.mxservers.net (envelope-from <nsharma@vlinkinfo.com>);
	Fri, 19 Aug 2011 11:33:18 -0400 (EDT)
Received: from ip-10-218-41-168.ec2.internal (ec2-184-72-246-77.compute-1.amazonaws.com [184.72.246.77])
	(authenticated bits=0)
	by rs78.luxhv.com (8.13.8/8.13.8) with ESMTP id p7JFX6BE005689;
	Fri, 19 Aug 2011 10:33:06 -0500
Date: Fri, 19 Aug 2011 11:33:05 -0400 (EDT)
From: Nivedita Sharma <nsharma@vlinkinfo.com>
To: nsharma@vlinkinfo.com
Message-ID: <1313767985812.5577092394061746.nsharma@vlinkinfo.com>
Subject: Direct Client Position: Data Architect-PA!!
MIME-Version: 1.0
Content-Type: multipart/mixed; 
	boundary="=_reb-r04BA7DB0-t4E4E823E"
Importance: Normal
X-Processed-By: Rebuild v2.0-0
X-Spam: [F=0.2000000000; B=0.500(0); S=0.200(2010122901); MH=0.500(2011081914)]
X-MAIL-FROM: <nsharma@vlinkinfo.com>
X-SOURCE-IP: [76.74.252.24]
X-SF-Loop: 1

This is a multi-part MIME message.

--=_reb-r04BA7DB0-t4E4E823E
Content-Type: text/html; charset=us-ascii
Content-Transfer-Encoding: 7bit

<div style="padding:3px" width="100%" align="right" id="removeTopLink"><a href="http://portal.zoniacswift.com//servlet/ZoniacServlet?api=getOutsideUnsubscribeContact&e66Ve62i57=true&m9UCc1ucRi=693&h2BNp44yL7=187992&h2ZNp64yM8=null&h2TNp86wP9=4" style="font: 11px verdana;"><font style="font: 11px verdana;" color="blue">Click here </font></a><span style="font: 11px verdana;">to unsubscribe.</span></div>


<style type="text/css">

		.font11 {color: black; font: 11px verdana}
		.font11bold {color: black; font: bold 11px verdana}	
		TABLE {font: 12px verdana}
	</style><br>
<table align="center" width="100%" cellpadding="10" cellspacing="0" border="1" style="border-bottom-color: #006aaf; border-top-color: #006aaf; border-collapse: collapse; border-right-color: #006aaf; border-left-color: #006aaf">
    <tbody>
        <tr>
            <td width="100%">
            <div id="customerLogo"><img src="cid:41FPQp1QyP" alt="" border="0"></div>
            <div id="messagebody" style="font: 12px verdana">&nbsp;</div>
            <div id="Signature" style="font: 12px verdana">
            <div style="margin: 0in 0in 0pt"><font face="Tahoma" size="2"><br>
            <div style="margin: 0in 0in 0pt">&nbsp;</div>
            <div style="margin: 0in 0in 0pt"><font face="Arial">Hi,</font></div>
            <div style="margin: 0in 0in 0pt"><font face="Arial">&nbsp;</font></div>
            <div style="margin: 0in 0in 0pt"><font face="Arial">I have a Data Architect with my direct client in PA.&nbsp;</font></div>
            <div style="margin: 0in 0in 0pt"><font face="Arial">&nbsp;</font></div>
            <div style="margin: 0in 0in 0pt"><font face="Arial">Job Title: Data Architect</font></div>
            <div style="margin: 0in 0in 0pt"><font face="Arial">Location:<span style="line-height: 115%; font-family: 'Arial','sans-serif'; color: black; font-size: 10pt">West Chester, PA.</span></font></div>
            <div style="margin: 0in 0in 0pt"><font face="Arial">Duration: 6+ months</font></div>
            <div style="margin: 0in 0in 0pt"><font face="Arial">Rate: DOE</font></div>
            <div style="margin: 0in 0in 0pt"><font face="Arial">Interview Process: Phone to Hire </font></div>
            <div style="margin: 0in 0in 0pt"><font face="Arial">&nbsp;</font></div>
            <div style="margin: 0in 0in 0pt"><font face="Arial">Send resumes to </font><a href="mailto:nsharma@vlinkinfo.com"><font face="Arial" color="#0000ff">nsharma@vlinkinfo.com</font></a><font face="Arial"> or reach me at 860-247-1400 Ext 107 </font></div>
            <div style="margin: 0in 0in 0pt"><font face="Arial">&nbsp;</font></div>
            <div style="margin: 0in 0in 10pt"><font face="Arial">Requirements:</font></div>
            <div style="margin: 0in 0in 0pt"><font face="Arial">* <strong><span style="color: red">At least 5 years experience with data modeling and data architecture (relational databases)</span></strong></font></div>
            <div style="margin: 0in 0in 0pt"><font face="Arial">* Substantial experience in SQL and or PL/SQL</font></div>
            <div style="margin: 0in 0in 0pt"><font face="Arial">* Relational database SQL scripting</font></div>
            <div style="margin: 0in 0in 0pt"><font face="Arial">* Excellent data model and patterns experience discovering patterns in the data model to enable coalescing similar data (e.g. addresses)</font></div>
            <div style="margin: 0in 0in 0pt"><font face="Arial">* Ability to create present-case data models and migrate them into future-case data models</font></div>
            <div style="margin: 0in 0in 0pt"><font face="Arial">* Strong Object / Relational mapping experience and ETL principles/knowledge</font></div>
            <div style="margin: 0in 0in 0pt"><font face="Arial">* Data warehouse and business intelligence experience</font></div>
            <div style="margin: 0in 0in 0pt"><font face="Arial">* Experience in Database Administration with Oracle or SQL Server</font></div>
            <div style="margin: 0in 0in 0pt"><font face="Arial">* Experience in object relational mapping, UML modeling and OO modeling</font></div>
            <br>
            <br>
            Regards,<br>
            <br>
            Nivedita Sharma</font></div>
            <span style="font-family: 'Calibri','sans-serif'; font-size: 11pt"><font face="Tahoma" size="2">VLink Inc | Phone: 860.247.1400 ext 107| Cell: 860-992-7599|Fax: 860.256.8484 | nsharma@vlinkinfo.com&nbsp;</font></span>
            <div style="margin: 0in 0in 0pt"><font face="Tahoma" size="2">&nbsp;</font></div>
            <div style="margin: 0in 0in 0pt"><span style="color: black; font-size: 10pt"><font face="Tahoma">Should employment follow the receipt of this Resume/Contract, whether directly, or at an end-client site, you hereby acknowledge that such employment of the consultant&rsquo;s services will not cause displacement of any US workers similarly employed by the Company, 90 days before and continuing 90 days after the date of commencement of said services. </font></span></div>
            </div>
            </td>
        </tr>
    </tbody>
</table>
<div style="padding:3px" width="100%" align="center" id="removeLink"><a href="http://portal.zoniacswift.com//servlet/ZoniacServlet?api=getOutsideUnsubscribeContact&e66Ve62i57=true&m9UCc1ucRi=693&h2BNp44yL7=187992&h2ZNp64yM8=null&h2TNp86wP9=4" class="font10"><font color="blue">Click here </font></a><span class="font10">to unsubscribe from our mailing list.</span><br><span class="font10">VLINK INC, 635 Farmington Ave, , Hartford, Connecticut, 06105      Phone: 860.247.1400 Fax: </span></div>
<div style="display:block;" width="100%" align="center"><table width="100%" align="center"><tr><td align="right" width="55%"><a target="_new" href="http://www.zoniac.com" style="vertical-align: text-top;"><font style="vertical-align: text-top;">Powered by Zoniac</font></a></td><td align="left" width="45%"><IMG align="bottom" height="20" width="20" border="0" SRC="cid:XwBUV7J5xW"></td></tr></table></div><div id="removeLinkDummy1"></div><div id="removeLinkDummy2"></div><div id="removeLinkDummy3"></div><div id="removeLinkDummy4"></div><div id="removeLinkDummy5"></div><div id="removeLinkDummy6"></div>
<div style="padding:3px" width="100%" align="left" id="showDisclaimerData"></div>

--=_reb-r04BA7DB0-t4E4E823E
Content-Type: image/gif; name="/Companies/693//custom/customerLogo.gif"
Content-Transfer-Encoding: base64
Content-Disposition: inline; 
	filename="/Companies/693//custom/customerLogo.gif"
Content-ID: <41FPQp1QyP>



--=_reb-r04BA7DB0-t4E4E823E
Content-Type: image/gif; name="/custom/UnsubscribedPoweredByImg.gif"
Content-Transfer-Encoding: base64
Content-Disposition: inline; filename="/custom/UnsubscribedPoweredByImg.gif"
Content-ID: <XwBUV7J5xW>



--=_reb-r04BA7DB0-t4E4E823E--

