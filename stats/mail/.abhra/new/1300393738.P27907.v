Return-Path: sameer@xperttech.com
Received: from mx58.stngva01.us.mxservers.net (204.202.242.48)
	by mail19a.g19.rapidsite.net (RS ver 1.0.95vs) with SMTP id 3-0547923738
	for <abhra@altechstar.com>; Thu, 17 Mar 2011 16:28:58 -0400 (EDT)
Received: from unknown [76.74.252.24] (EHLO rs78.luxhv.com)
	by va1-mx58.stngva01.us.mxservers.net (mxl_mta-3.1.0-05)
	with ESMTP id 90f628d4.2035280800.519349.00-001.va1-mx58.stngva01.us.mxservers.net (envelope-from <sameer@xperttech.com>);
	Thu, 17 Mar 2011 16:28:57 -0400 (EDT)
Received: from ip-10-196-81-219.ec2.internal (ec2-184-72-246-77.compute-1.amazonaws.com [184.72.246.77])
	(authenticated bits=0)
	by rs78.luxhv.com (8.13.8/8.13.8) with ESMTP id p2HKShKU028655;
	Thu, 17 Mar 2011 15:28:44 -0500
Date: Thu, 17 Mar 2011 15:28:43 -0500 (GMT-05:00)
From: Sameer Maharana <sameer@xperttech.com>
To: sameer@xperttech.com
Message-ID: <1300393723750.1324163634602693.sameer@xperttech.com>
Subject: Java core (j2ee) developer // 12+ months // Minneapolis, MN
MIME-Version: 1.0
Content-Type: multipart/mixed; 
	boundary="=_reb-r03358336-t4D826F0A"
Importance: High
X-Lux-Comment: Message p2HKShKU028655 sent by user #68993; report Spam, UCE, or Abuse to abuse@luxsci.us
X-Lux-Comment: LuxSci High Volume Email: http://luxhv.com
X-Processed-By: Rebuild v2.0-0
X-Spam: [F=0.2000000000; B=0.500(0); S=0.200(2010122901); MH=0.500(2011031714)]
X-MAIL-FROM: <sameer@xperttech.com>
X-SOURCE-IP: [76.74.252.24]
X-SF-Loop: 1

This is a multi-part MIME message.

--=_reb-r03358336-t4D826F0A
Content-Type: text/html; charset=us-ascii
Content-Transfer-Encoding: 7bit

<div style="padding:3px" width="100%" align="right" id="removeTopLink"><a href="http://portal.zoniacswift.com//servlet/ZoniacServlet?api=getOutsideUnsubscribeContact&e66Ve62i57=true&m9UCc1ucRi=540&h2BNp44yL7=108734&h5p8pKsyMi8l0=0&h2ZNp64yM8=null&h2TNp86wP9=5&h2GQy54sI2=1746" style="font: 11px verdana;"><font style="font: 11px verdana;" color="blue">Click here </font></a><span style="font: 11px verdana;">to unsubscribe.</span></div>


<style type="text/css">

		.font11 {color: black; font: 11px verdana}
		.font11bold {color: black; font: bold 11px verdana}	
		TABLE {font: 12px verdana}
	</style><br>
<table border="1" align="center" width="100%" cellpadding="10" cellspacing="0" style="border-left-color: #006aaf; border-bottom-color: #006aaf; border-top-color: #006aaf; border-collapse: collapse; border-right-color: #006aaf">
    <tbody>
        <tr>
            <td width="100%">
            <div id="customerLogo"><img alt="" border="0" src="cid:I0j99dN4xj"></div>
            <div style="font: 12px verdana" id="messagebody">&nbsp;</div>
            <div style="font: 12px verdana" id="Signature">
            <div><font size="3" face="Candara"><span style="font-size: 11pt; font-family: Candara">Hi, </span></font></div>
            <div><font size="3" face="Candara">&nbsp;</font></div>
            <div><font size="3" face="Candara"><span style="font-size: 11pt; font-family: Candara">I have an immediate requirement opening for a <strong><span style="font-weight: bold; background: yellow">Java core (j2ee) developer position in Minneapolis, MN for 12+ month&rsquo;s contract</span></strong>. I would appreciate if you could send your updated word format resume along with best contact details ASAP.</span></font></div>
            <div><font size="3" face="Candara">&nbsp;</font></div>
            <div><font size="3" face="Candara"><span style="font-size: 11pt; font-family: Candara">Here are the requirement details:</span></font></div>
            <div><font size="3" face="Candara">&nbsp;</font></div>
            <div><strong><font size="3" color="#0000ff" face="Candara"><span style="font-weight: bold; font-size: 11pt; color: blue; font-family: Candara">Position&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; : Java core (j2ee) developer</span></font></strong></div>
            <div><strong><font size="3" color="#0000ff" face="Candara"><span style="font-weight: bold; font-size: 11pt; color: blue; font-family: Candara">Duration &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; : 12+ month&rsquo;s contract</span></font></strong></div>
            <div><strong><font size="3" color="#0000ff" face="Candara"><span style="font-weight: bold; font-size: 11pt; color: blue; font-family: Candara">Location &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; : Minneapolis, MN</span></font></strong></div>
            <div><font size="3" face="Candara">&nbsp;</font></div>
            <div><strong><u><font size="3" face="Candara"><span style="font-weight: bold; font-size: 11pt; font-family: Candara">Required Skills:</span></font></u></strong></div>
            <ul type="disc" style="margin-top: 0in">
                <li><font size="3"><font face="Candara"><span style="font-size: 11pt; font-family: Candara">Application development in distributed setting (6 years)</span></font> </font></li>
                <li><font size="3"><font face="Candara"><span style="font-size: 11pt; font-family: Candara">J2EE (5 years)</span></font> </font></li>
                <li><font size="3"><font face="Candara"><span style="font-size: 11pt; font-family: Candara">Unix (4 years)</span></font> </font></li>
                <li><font size="3"><font face="Candara"><span style="font-size: 11pt; font-family: Candara">Agile (3 years)</span></font> </font></li>
                <li><font size="3"><font face="Candara"><span style="font-size: 11pt; font-family: Candara">OOAD (4 years, formal training)</span></font> </font></li>
                <li><font size="3"><font face="Candara"><span style="font-size: 11pt; font-family: Candara">Design pattern usage</span></font> </font></li>
                <li><font size="3"><font face="Candara"><span style="font-size: 11pt; font-family: Candara">Mainframe to Distributed System Integration</span></font> </font></li>
                <li><font size="3"><font face="Candara"><span style="font-size: 11pt; font-family: Candara">High volume systems (5k transactions/second)</span></font> </font></li>
                <li><font size="3"><font face="Candara"><span style="font-size: 11pt; font-family: Candara">High volume systems (5k transactions/second) - Tuning</span></font> </font></li>
                <li><font size="3"><font face="Candara"><span style="font-size: 11pt; font-family: Candara">Oracle development (3 years)</span></font> </font></li>
                <li><font size="3"><font face="Candara"><span style="font-size: 11pt; font-family: Candara">Ant</span></font> </font></li>
                <li><font size="3"><font face="Candara"><span style="font-size: 11pt; font-family: Candara">Maven</span></font> </font></li>
                <li><font size="3"><font face="Candara"><span style="font-size: 11pt; font-family: Candara">Subversion</span></font> </font></li>
                <li><font size="3"><font face="Candara"><span style="font-size: 11pt; font-family: Candara">Roles with notable Business stakeholder interaction</span></font> </font></li>
                <li><font size="3" face="Candara"><span style="font-size: 11pt; font-family: Candara">Effort estimation around j2ee solutions</span></font></li>
            </ul>
            </div>
            </td>
        </tr>
    </tbody>
</table>
<div style="padding:3px" width="100%" align="center" id="removeLink"><a href="http://portal.zoniacswift.com//servlet/ZoniacServlet?api=getOutsideUnsubscribeContact&e66Ve62i57=true&m9UCc1ucRi=540&h2BNp44yL7=108734&h5p8pKsyMi8l0=0&h2ZNp64yM8=null&h2TNp86wP9=5&h2GQy54sI2=1746" class="font10"><font color="blue">Click here </font></a><span class="font10">to unsubscribe from our mailing list.</span><br><span class="font10">XPERTTECH  Inc, 400 W Cummings Park, Suite #2850, Woburn, Massachusetts, 01801      Phone: 781 935 3775 Fax: 978-405-5040</span></div>
<div style="display:block;" width="100%" align="center"><table width="100%" align="center"><tr><td align="right" width="55%"><a target="_new" href="http://www.zoniac.com" style="vertical-align: text-top;"><font style="vertical-align: text-top;">Powered by Zoniac</font></a></td><td align="left" width="45%"><IMG align="bottom" height="20" width="20" border="0" SRC="cid:ReGGe36Tcm"></td></tr></table></div><div id="removeLinkDummy1"></div><div id="removeLinkDummy2"></div><div id="removeLinkDummy3"></div><div id="removeLinkDummy4"></div><div id="removeLinkDummy5"></div><div id="removeLinkDummy6"></div>

--=_reb-r03358336-t4D826F0A
Content-Type: image/gif; name="/Companies/540/custom/customerLogo.gif"
Content-Transfer-Encoding: base64
Content-Disposition: inline; 
	filename="/Companies/540/custom/customerLogo.gif"
Content-ID: <I0j99dN4xj>



--=_reb-r03358336-t4D826F0A
Content-Type: image/gif; name="/custom/UnsubscribedPoweredByImg.gif"
Content-Transfer-Encoding: base64
Content-Disposition: inline; filename="/custom/UnsubscribedPoweredByImg.gif"
Content-ID: <ReGGe36Tcm>



--=_reb-r03358336-t4D826F0A--

