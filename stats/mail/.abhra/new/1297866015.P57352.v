Return-Path: andy@vlinkinfo.com
Received: from mx99.stngva01.us.mxservers.net (198.173.112.36)
	by mail19h.g19.rapidsite.net (RS ver 1.0.95vs) with SMTP id 4-0889976014
	for <abhra@altechstar.com>; Wed, 16 Feb 2011 09:20:14 -0500 (EST)
Received: from unknown [76.74.252.24] (EHLO rs78.luxhv.com)
	by va1-mx99.stngva01.us.mxservers.net (mxl_mta-3.1.0-05)
	with ESMTP id e1ddb5d4.2464152480.228554.00-009.va1-mx99.stngva01.us.mxservers.net (envelope-from <andy@vlinkinfo.com>);
	Wed, 16 Feb 2011 09:20:14 -0500 (EST)
Received: from ip-10-196-81-219.ec2.internal (ec2-184-72-246-77.compute-1.amazonaws.com [184.72.246.77])
	(authenticated bits=0)
	by rs78.luxhv.com (8.13.8/8.13.8) with ESMTP id p1GEK7HX025705;
	Wed, 16 Feb 2011 08:20:07 -0600
Date: Wed, 16 Feb 2011 09:20:07 -0500 (GMT-05:00)
From: Andy Duggal <andy@vlinkinfo.com>
To: andy@vlinkinfo.com
Message-ID: <1297866007299.8506289918225757.andy@vlinkinfo.com>
Subject: Datastage Technical specialist- designer contract requirement
MIME-Version: 1.0
Content-Type: multipart/mixed; 
	boundary="=_reb-r6CD946BF-t4D5BDD1E"
Importance: Normal
X-Lux-Comment: Message p1GEK7HX025705 sent by user #76870; report Spam, UCE, or Abuse to abuse@luxsci.us
X-Lux-Comment: LuxSci High Volume Email: http://luxhv.com
X-Processed-By: Rebuild v2.0-0
X-Spam: [F=0.2000000000; B=0.500(0); S=0.200(2010122901); MH=0.500(2011021613)]
X-MAIL-FROM: <andy@vlinkinfo.com>
X-SOURCE-IP: [76.74.252.24]
X-SF-Loop: 1

This is a multi-part MIME message.

--=_reb-r6CD946BF-t4D5BDD1E
Content-Type: text/html; charset=us-ascii
Content-Transfer-Encoding: 7bit

<div style="padding:3px" width="100%" align="right" id="removeTopLink"><a href="http://portal.zoniacswift.com//servlet/ZoniacServlet?api=getOutsideUnsubscribeContact&e66Ve62i57=true&m9UCc1ucRi=693&h2BNp44yL7=187991&h5p8pKsyMi8l0=0&h2ZNp64yM8=null&h2TNp86wP9=5&h2GQy54sI2=1530" style="font: 11px verdana;"><font style="font: 11px verdana;" color="blue">Click here </font></a><span style="font: 11px verdana;">to unsubscribe.</span></div>


<style type="text/css">

		.font11 {color: black; font: 11px verdana}
		.font11bold {color: black; font: bold 11px verdana}	
		TABLE {font: 12px verdana}
	</style><br>
<table align="center" width="100%" cellpadding="10" cellspacing="0" border="1" style="border-bottom-color: #006aaf; border-top-color: #006aaf; border-collapse: collapse; border-right-color: #006aaf; border-left-color: #006aaf">
    <tbody>
        <tr>
            <td width="100%">
            <div id="customerLogo"><img src="cid:NRftTGrUmu" alt="" border="0"><br>
            <br>
            <br>
            12 months plus project in Hartford, CT, Rate: Negotiable<br>
            <div style="margin: 0in 0in 0pt"><span style="font-family: 'Arial','sans-serif'; color: blue; font-size: 10pt">ETL design and technical leadership<br>
            <br>
            AIX environment<br>
            Healthcare Payer industry experience preferred<br>
            Must have experience with DataStage in a DB2 environment<br>
            Excellent comm skills to facilitate communication with developers and business users<br>
            <br>
            A minimum of 7 years experience providing hands-on leadership for complex ETL delivery initiatives for data warehouses and related applications using industry standard toolsets.<br>
            <br>
            * Prior experience with all phases of system delivery and implementation.<br>
            * Working knowledge of Industry Systems/Software Engineering processes and techniques.<br>
            * Prior experience in a development environment that includes some of the following tools and technologies: DB2/UDB version 9.5, AIX, IBM DataStage version 7.5, ZEKE scheduler, Clearcase, Clearquest, SQL, DB2 Stored Procedures.<br>
            * Exposure to other toolsets used to deliver ETL solutions is a plus (Informatica, Abinitio, SAS, Oracle, Teradata, etc.)<br>
            * A minimum of 5 years experience working within structured project methodologies.<br>
            * Experience with Healthcare payer business.<br>
            * Extensive knowledge of the system development life cycle, the client area's functions and systems, and application program development alternatives.<br>
            * Technical leadership skills.<br>
            * Excellent communication skills. <br>
            * Analytical ability.<br>
            * Ability to work effectively with clients and IT management and staff.</span></div>
            </div>
            <div id="messagebody" style="font: 12px verdana">&nbsp;</div>
            <div id="Signature" style="font: 12px verdana">
            <div style="margin: 0in 0in 0pt"><a name="_MailAutoSig" class="FCK__AnchorC"><span style="font-family: 'Times New Roman','serif'; font-size: 12pt">Best regards,</span></a></div>
            <div style="margin: 0in 0in 0pt"><span style="font-family: 'Times New Roman','serif'; font-size: 12pt">Andy Duggal</span></div>
            <div style="margin: 0in 0in 0pt"><span style="font-family: 'Times New Roman','serif'; font-size: 12pt">Senior Resource Manager</span></div>
            <div style="margin: 0in 0in 0pt">&nbsp;</div>
            <div style="margin: 0in 0in 0pt"><span style="font-family: 'Times New Roman','serif'; font-size: 12pt">Vlink Inc | Phone: 860 247 1400 ext 103&nbsp;|&nbsp;Cell: 201 699 9919&nbsp;| Fax: 860 256 8484 | </span><a href="mailto:andy@vlinkinfo.com" title="blocked::mailto:anish@vlinkinfo.com"><span style="font-family: 'Times New Roman','serif'; color: blue; font-size: 12pt">andy@vlinkinfo.com</span></a><span style="font-family: 'Times New Roman','serif'; font-size: 12pt"> | </span><a href="http://www.vlinkinfo.com/" title="blocked::http://www.vlinkinfo.com/"><span style="font-family: 'Times New Roman','serif'; color: blue; font-size: 12pt">www.vlinkinfo.com</span></a></div>
            <div style="margin: 0in 0in 0pt">&nbsp;<span style="font-family: 'Arial','sans-serif'; font-size: 10pt">&nbsp;&nbsp;</span><a href="http://www.linkedin.com/in/andyduggal" title="View public profile"><span><span style="font-family: 'Arial','sans-serif'; color: blue; font-size: 10pt">http://www.linkedin.com/in/andyduggal</span></span></a></div>
            <div style="margin: 0in 0in 0pt">&nbsp;</div>
            <div style="margin: 0in 0in 0pt">&nbsp;</div>
            <div style="margin: 0in 0in 0pt"><span style="font-family: 'Arial','sans-serif'; font-size: 10pt">&nbsp;&nbsp;&nbsp;&nbsp;</span>&nbsp;</div>
            <div style="margin: 0in 0in 0pt">&nbsp;</div>
            <div style="margin: 0in 0in 0pt"><span style="font-family: 'Times New Roman','serif'; font-size: 10pt">LEGAL NOTICE:</span></div>
            <div style="margin: 0in 0in 0pt"><span style="font-family: 'Times New Roman','serif'; font-size: 10pt">Unless expressly stated otherwise, the information contained in this message is confidential, may be privileged, and protected from disclosure. If the reader of this message is not the intended recipient, or an employee or agent responsible for delivering this message to the intended recipient, you are hereby notified that any dissemination, distribution, or copying of this communication is strictly prohibited. If you have received this communication in error, please notify the sender immediately by replying to the message, permanently deleting it from your computer and/or destroy any copies/printouts of this message.</span>&nbsp;</div>
            <div style="margin: 0in 0in 0pt"><span style="font-family: 'Times New Roman','serif'; color: black; font-size: 10pt">Should employment follow the receipt of this Resume/Contract, whether directly, or at an end-client site, you hereby acknowledge that such employment of the consultant&rsquo;s services will not cause displacement of any US workers similarly employed by the Company, 90 days before and continuing 90 days after the date of commencement of said services. </span></div>
            </div>
            </td>
        </tr>
    </tbody>
</table>
<div style="padding:3px" width="100%" align="center" id="removeLink"><a href="http://portal.zoniacswift.com//servlet/ZoniacServlet?api=getOutsideUnsubscribeContact&e66Ve62i57=true&m9UCc1ucRi=693&h2BNp44yL7=187991&h5p8pKsyMi8l0=0&h2ZNp64yM8=null&h2TNp86wP9=5&h2GQy54sI2=1530" class="font10"><font color="blue">Click here </font></a><span class="font10">to unsubscribe from our mailing list.</span><br><span class="font10">VLINK INC, 635 Farmington Ave, , Hartford, Connecticut, 06105      Phone: 860.247.1400 Fax: </span></div>
<div style="display:block;" width="100%" align="center"><table width="100%" align="center"><tr><td align="right" width="55%"><a target="_new" href="http://www.zoniac.com" style="vertical-align: text-top;"><font style="vertical-align: text-top;">Powered by Zoniac</font></a></td><td align="left" width="45%"><IMG align="bottom" height="20" width="20" border="0" SRC="cid:XeLmc74zZr"></td></tr></table></div><div id="removeLinkDummy1"></div><div id="removeLinkDummy2"></div><div id="removeLinkDummy3"></div><div id="removeLinkDummy4"></div><div id="removeLinkDummy5"></div><div id="removeLinkDummy6"></div>

--=_reb-r6CD946BF-t4D5BDD1E
Content-Type: image/gif; name="/Companies/693//custom/customerLogo.gif"
Content-Transfer-Encoding: base64
Content-Disposition: inline; 
	filename="/Companies/693//custom/customerLogo.gif"
Content-ID: <NRftTGrUmu>



--=_reb-r6CD946BF-t4D5BDD1E
Content-Type: image/gif; name="/custom/UnsubscribedPoweredByImg.gif"
Content-Transfer-Encoding: base64
Content-Disposition: inline; filename="/custom/UnsubscribedPoweredByImg.gif"
Content-ID: <XeLmc74zZr>



--=_reb-r6CD946BF-t4D5BDD1E--

